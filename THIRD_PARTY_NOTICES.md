...
Gondolin is MIT-licensed; see `LICENSE`. Third-party components, datasets, generated fixtures,
and optional local assets may be licensed under different terms; this file records those
provenance and release-hygiene notes.
...
These assets are not redistributed by Gondolin unless explicitly noted. Users
...
The repository includes small generated fixtures so tutorials and tests can run
without network access:

| Fixture | Path | Origin / notice |
| --- | --- | --- |
| Toy regression CSV/NPY | `NN/Examples/Data/toy_regression.csv`, `NN/Examples/Data/toy_regression_X.npy`, `NN/Examples/Data/toy_regression_y.npy` | Synthetic Gondolin tutorial data, covered by the repository MIT license. |
| Toy CIFAR-10-like NPY | `NN/Examples/Data/toy_cifar10like_X.npy`, `NN/Examples/Data/toy_cifar10like_y.npy` | Synthetic Gondolin fixture generated by `NN/Examples/Data/gen_toy_cifar10like.py`, covered by the repository MIT license. |
| Robustness digits JSON fixtures | `NN/Examples/Verification/Robustness/digits_test.json`, `NN/Examples/Verification/Robustness/digits_linear_weights.json`, `NN/Examples/Verification/Robustness/digits_linear_margin_cert.json` | Derived demo artifacts produced from scikit-learn's built-in `load_digits` dataset and local training/certification scripts. scikit-learn is BSD-3-Clause licensed; keep scikit-learn and original digits-dataset attribution when reusing these fixtures outside Gondolin. |
...
| --- | --- | --- |
| RL widget logs and policies | `data/rl/*.json` | Produced by `gondolin ppo_gridworld`, `gondolin ppo_cartpole`, and related widget demos. |
| FNO prediction CSV/PNG files | `data/real/fno*/predictions*.csv`, `data/real/fno*/predictions*.png` | Produced by `gondolin fno1d_burgers` and `NN/Examples/Data/plot_fno1d_burgers.py`. |
...
- `Two-Stage_Neural_Controller_Training/`
  - This path is gitignored in Gondolin and is not part of the public library
...
4. Keep generated Gondolin logs, plots, and caches out of source releases.
