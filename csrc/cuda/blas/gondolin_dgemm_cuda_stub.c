...
// CPU fallback for `gondolin_dgemm_cuda`.
//
...
// `gondolin_dgemm_cuda.cu`; tests should be able to compare CUDA and stub behavior on small cases.
LEAN_EXPORT lean_obj_res gondolin_dgemm_cuda(b_lean_obj_arg AObj, b_lean_obj_arg BObj,
                                             uint32_t m, uint32_t n, uint32_t p) {
  size_t M = (size_t)m, N = (size_t)n, P = (size_t)p;
  size_t aSz = checked_mul_size(M, N, "gondolin_dgemm_cuda_stub: A size overflow");
  size_t bSz = checked_mul_size(N, P, "gondolin_dgemm_cuda_stub: B size overflow");
  size_t cSz = checked_mul_size(M, P, "gondolin_dgemm_cuda_stub: C size overflow");

  lean_object* A = (lean_object*)AObj;
  lean_object* B = (lean_object*)BObj;

  if (lean_sarray_size(A) != aSz) {
    lean_internal_panic("gondolin_dgemm_cuda_stub: A.size mismatch");
  }
  if (lean_sarray_size(B) != bSz) {
    lean_internal_panic("gondolin_dgemm_cuda_stub: B.size mismatch");
  }
