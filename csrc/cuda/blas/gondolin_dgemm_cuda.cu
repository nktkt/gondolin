...
template <typename T>
static inline void gondolin_cuda_free_checked(T** ptr, const char* msg) {
  if (*ptr) {
...
extern "C" LEAN_EXPORT lean_obj_res gondolin_dgemm_cuda(b_lean_obj_arg AObj, b_lean_obj_arg BObj,
                                                        uint32_t m, uint32_t n, uint32_t p) {
  // Copy inputs from Lean FloatArray.
  lean_object* A = (lean_object*)AObj;
  lean_object* B = (lean_object*)BObj;

  size_t aSz = checked_mul_size((size_t)m, (size_t)n, "gondolin_dgemm_cuda: A size overflow");
  size_t bSz = checked_mul_size((size_t)n, (size_t)p, "gondolin_dgemm_cuda: B size overflow");
  size_t cSz = checked_mul_size((size_t)m, (size_t)p, "gondolin_dgemm_cuda: C size overflow");

  if (lean_sarray_size(A) != aSz) {
    lean_internal_panic("gondolin_dgemm_cuda: A.size mismatch");
  }
  if (lean_sarray_size(B) != bSz) {
    lean_internal_panic("gondolin_dgemm_cuda: B.size mismatch");
  }
  if (m > (uint32_t)INT_MAX || n > (uint32_t)INT_MAX || p > (uint32_t)INT_MAX) {
    lean_internal_panic("gondolin_dgemm_cuda: dimensions exceed cuBLAS int range");
  }
  ...
  if (err != cudaSuccess) {
    gondolin_cuda_free_checked(&dA, "cudaFree dA after dB alloc failure failed");
    checkCuda(err, "cudaMalloc dB failed");
  }
  err = cudaMalloc((void**)&dC, cSz * sizeof(double));
  if (err != cudaSuccess) {
    gondolin_cuda_free_checked(&dB, "cudaFree dB after dC alloc failure failed");
    gondolin_cuda_free_checked(&dA, "cudaFree dA after dC alloc failure failed");
    checkCuda(err, "cudaMalloc dC failed");
  }

  err = cudaMemcpy(dA, hA, aSz * sizeof(double), cudaMemcpyHostToDevice);
  if (err != cudaSuccess) {
    gondolin_cuda_free_checked(&dC, "cudaFree dC after A copy failure failed");
    gondolin_cuda_free_checked(&dB, "cudaFree dB after A copy failure failed");
    gondolin_cuda_free_checked(&dA, "cudaFree dA after A copy failure failed");
    checkCuda(err, "cudaMemcpy A failed");
  }
  err = cudaMemcpy(dB, hB, bSz * sizeof(double), cudaMemcpyHostToDevice);
  if (err != cudaSuccess) {
    gondolin_cuda_free_checked(&dC, "cudaFree dC after B copy failure failed");
    gondolin_cuda_free_checked(&dB, "cudaFree dB after B copy failure failed");
    gondolin_cuda_free_checked(&dA, "cudaFree dA after B copy failure failed");
  ...
  if (stat != CUBLAS_STATUS_SUCCESS) {
    gondolin_cuda_free_checked(&dC, "cudaFree dC after cublasDgemm failure failed");
    gondolin_cuda_free_checked(&dB, "cudaFree dB after cublasDgemm failure failed");
    gondolin_cuda_free_checked(&dA, "cudaFree dA after cublasDgemm failure failed");
  ...
  if (err != cudaSuccess) {
    gondolin_cuda_free_checked(&dC, "cudaFree dC after C copy failure failed");
    gondolin_cuda_free_checked(&dB, "cudaFree dB after C copy failure failed");
    gondolin_cuda_free_checked(&dA, "cudaFree dA after C copy failure failed");
    checkCuda(err, "cudaMemcpy C failed");
  }
  lean_sarray_set_size(out, cSz);

  gondolin_cuda_free_checked(&dA, "cudaFree dA failed");
  gondolin_cuda_free_checked(&dB, "cudaFree dB failed");
  gondolin_cuda_free_checked(&dC, "cudaFree dC failed");
