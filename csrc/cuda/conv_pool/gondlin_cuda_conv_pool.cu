...
#include "gondlin_cuda_conv_pool_common.h"
...
// CUDA implementation of Gondlin float32 conv/pool kernels over `Cuda.Buffer`.
//
...
// - max-pool padding is ignored (`-inf` for max), matching the Gondlin/PyTorch convention;
// - atomic backward paths have deterministic alternatives controlled by the runtime flag;
// - the CPU fallback in `gondlin_cuda_conv_pool_stub.c` must stay semantically aligned.

static constexpr int kBlock = 256;
static constexpr int kMaxRank = GONDLIN_CUDA_CONV_POOL_MAX_RANK;
...
static inline bool gondlin_cuda_deterministic_reductions_enabled() {
  return gondlin_cuda_get_deterministic_reductions() != 0;
}
...
__global__ void maxpool2d_bwd_kernel(const float* input, const float* gradOutput, float* dInput,
  ...
  // Tie-breaking matches Gondlin spec: keep the first (row-major) argmax.
  for (int ky = 0; ky < kH; ++ky) {
    int ih = oh * stride + ky - padding;
    for (int kx = 0; kx < kW; ++kx) {
      int iw = ow * stride + kx - padding;
...
extern "C" LEAN_EXPORT lean_obj_res gondlin_cuda_conv2d_fwd(
  ...
  gondlin_cuda_buffer* input = gondlin_cuda_buffer_unbox(inputObj);
  gondlin_cuda_buffer* kernel = gondlin_cuda_buffer_unbox(kernelObj);
  gondlin_cuda_buffer* bias = gondlin_cuda_buffer_unbox(biasObj);

  if (kH == 0 || kW == 0) {
    lean_internal_panic("gondlin_cuda_conv2d_fwd: kH/kW must be > 0");
  }
  ...
  checkBufSize(input, inElems, "gondlin_cuda_conv2d_fwd: input.size mismatch");
  checkBufSize(kernel, kElems, "gondlin_cuda_conv2d_fwd: kernel.size mismatch");
  checkBufSize(bias, bElems, "gondlin_cuda_conv2d_fwd: bias.size mismatch");

  gondlin_cuda_buffer* out = gondlin_cuda_buffer_alloc(outElems);
  if (outElems == 0) {
...
extern "C" LEAN_EXPORT lean_obj_res gondlin_cuda_conv2d_bwd(
  ...
  gondlin_cuda_buffer* input = gondlin_cuda_buffer_unbox(inputObj);
  gondlin_cuda_buffer* kernel = gondlin_cuda_buffer_unbox(kernelObj);
  gondlin_cuda_buffer* gradOutput = gondlin_cuda_buffer_unbox(gradObj);

  if (kH == 0 || kW == 0) {
    lean_internal_panic("gondlin_cuda_conv2d_bwd: kH/kW must be > 0");
  }
  ...
  checkBufSize(input, inElems, "gondlin_cuda_conv2d_bwd: input.size mismatch");
  checkBufSize(kernel, kElems, "gondlin_cuda_conv2d_bwd: kernel.size mismatch");
  checkBufSize(gradOutput, outElems, "gondlin_cuda_conv2d_bwd: gradOutput.size mismatch");

  gondlin_cuda_buffer* dKernel = gondlin_cuda_buffer_alloc(kElems);
  gondlin_cuda_buffer* dBias = gondlin_cuda_buffer_alloc((size_t)outC);
  gondlin_cuda_buffer* dInput = gondlin_cuda_buffer_alloc(inElems);
  ...
  lean_ctor_set(inner, 0, gondlin_cuda_buffer_box(dBias));
  lean_ctor_set(inner, 1, gondlin_cuda_buffer_box(dInput));

  lean_object* outer = lean_alloc_ctor(0, 2, 0);
  lean_ctor_set(outer, 0, gondlin_cuda_buffer_box(dKernel));
  lean_ctor_set(outer, 1, inner);
  return outer;
}

extern "C" LEAN_EXPORT lean_obj_res gondlin_cuda_convtranspose2d_fwd(
    b_lean_obj_arg inputObj, b_lean_obj_arg kernelObj, b_lean_obj_arg biasObj,
    uint32_t inC, uint32_t inH, uint32_t inW,
    uint32_t outC, uint32_t kH, uint32_t kW,
    uint32_t stride, uint32_t padding) {
  ...
  if (kH == 0 || kW == 0) {
    lean_internal_panic("gondlin_cuda_convtranspose2d_fwd: kH/kW must be > 0");
  }
  ...
  checkBufSize(input, inElems, "gondlin_cuda_convtranspose2d_fwd: input.size mismatch");
  checkBufSize(kernel, kElems, "gondlin_cuda_convtranspose2d_fwd: kernel.size mismatch");
  checkBufSize(bias, bElems, "gondlin_cuda_convtranspose2d_fwd: bias.size mismatch");
...
extern "C" LEAN_EXPORT lean_obj_res gondlin_cuda_convtranspose2d_bwd(
    b_lean_obj_arg inputObj, b_lean_obj_arg kernelObj, b_lean_obj_arg gradObj,
    uint32_t inC, uint32_t inH, uint32_t inW,
    uint32_t outC, uint32_t kH, uint32_t kW,
    uint32_t stride, uint32_t padding) {
  ...
  if (kH == 0 || kW == 0) {
    lean_internal_panic("gondlin_cuda_convtranspose2d_bwd: kH/kW must be > 0");
  }
  ...
  checkBufSize(input, inElems, "gondlin_cuda_convtranspose2d_bwd: input.size mismatch");
  checkBufSize(kernel, kElems, "gondlin_cuda_convtranspose2d_bwd: kernel.size mismatch");
  checkBufSize(gradOutput, outElems, "gondlin_cuda_convtranspose2d_bwd: gradOutput.size mismatch");
...
extern "C" LEAN_EXPORT lean_obj_res gondlin_cuda_maxpool2d_fwd(
  ...
  if (kH == 0 || kW == 0) {
    lean_internal_panic("gondlin_cuda_maxpool2d_fwd: kH/kW must be > 0");
  }
  ...
  checkBufSize(input, inElems, "gondlin_cuda_maxpool2d_fwd: input.size mismatch");
...
extern "C" LEAN_EXPORT lean_obj_res gondlin_cuda_maxpool2d_bwd(
  ...
  if (kH == 0 || kW == 0) {
    lean_internal_panic("gondlin_cuda_maxpool2d_bwd: kH/kW must be > 0");
  }
  ...
  checkBufSize(input, inElems, "gondlin_cuda_maxpool2d_bwd: input.size mismatch");
  checkBufSize(gradOutput, outElems, "gondlin_cuda_maxpool2d_bwd: gradOutput.size mismatch");

  gondlin_cuda_buffer* dInput = gondlin_cuda_buffer_alloc(inElems);
  const bool det = gondlin_cuda_deterministic_reductions_enabled();
  ...
  return gondlin_cuda_buffer_box(dInput);
}

extern "C" LEAN_EXPORT lean_obj_res gondlin_cuda_smooth_maxpool2d_fwd(
  ...
  if (kH == 0 || kW == 0) {
    lean_internal_panic("gondlin_cuda_smooth_maxpool2d_fwd: kH/kW must be > 0");
  }
  ...
  checkBufSize(input, inElems, "gondlin_cuda_smooth_maxpool2d_fwd: input.size mismatch");
  ...
  int grid = grid_for(outElems);
  const float betaF =
      checked_smoothmax_beta(beta, "gondlin_cuda_smooth_maxpool2d_fwd: beta must be finite and nonzero");
...
extern "C" LEAN_EXPORT lean_obj_res gondlin_cuda_smooth_maxpool2d_bwd(
  ...
  if (kH == 0 || kW == 0) {
    lean_internal_panic("gondlin_cuda_smooth_maxpool2d_bwd: kH/kW must be > 0");
  }
  ...
  checkBufSize(input, inElems, "gondlin_cuda_smooth_maxpool2d_bwd: input.size mismatch");
  checkBufSize(gradOutput, outElems, "gondlin_cuda_smooth_maxpool2d_bwd: gradOutput.size mismatch");

  gondlin_cuda_buffer* dInput = gondlin_cuda_buffer_alloc(inElems);
  const float betaF =
      checked_smoothmax_beta(beta, "gondlin_cuda_smooth_maxpool2d_bwd: beta must be finite and nonzero");
  const bool det = gondlin_cuda_deterministic_reductions_enabled();
  if (!det && dInput->size > 0) {
    // Atomic path accumulates into dInput; must clear first.
    checkCuda(cudaMemset(dInput->data, 0, dInput->size * sizeof(float)), "cudaMemset dInput failed");
...
extern "C" LEAN_EXPORT lean_obj_res gondlin_cuda_avgpool2d_fwd(
    b_lean_obj_arg inputObj,
    uint32_t inC, uint32_t inH, uint32_t inW,
    uint32_t kH, uint32_t kW,
    uint32_t stride, uint32_t padding) {
  ...
  if (kH == 0 || kW == 0) {
    lean_internal_panic("gondlin_cuda_avgpool2d_fwd: kH/kW must be > 0");
  }
  ...
  checkBufSize(input, inElems, "gondlin_cuda_avgpool2d_fwd: input.size mismatch");
...
extern "C" LEAN_EXPORT lean_obj_res gondlin_cuda_avgpool2d_bwd(
  ...
  if (kH == 0 || kW == 0) {
    lean_internal_panic("gondlin_cuda_avgpool2d_bwd: kH/kW must be > 0");
  }
  ...
  checkBufSize(gradOutput, outElems, "gondlin_cuda_avgpool2d_bwd: gradOutput.size mismatch");
...
extern "C" LEAN_EXPORT lean_obj_res gondlin_cuda_conv_fwd(
    b_lean_obj_arg inputObj, b_lean_obj_arg kernelObj, b_lean_obj_arg biasObj,
  ...
  const int rank = read_rank_checked(inSpatialObj, "gondlin_cuda_conv_fwd: bad inSpatial");
  if (read_rank_checked(kernelSpatialObj, "gondlin_cuda_conv_fwd: bad kernelSpatial") != rank ||
      read_rank_checked(strideObj, "gondlin_cuda_conv_fwd: bad stride") != rank ||
      read_rank_checked(paddingObj, "gondlin_cuda_conv_fwd: bad padding") != rank) {
    lean_internal_panic("gondlin_cuda_conv_fwd: array rank mismatch");
  }
  if (rank <= 0) {
    lean_internal_panic("gondlin_cuda_conv_fwd: rank must be > 0");
  }
  ...
  read_u32_array(inSpatialObj, hInSpatial, rank, "gondlin_cuda_conv_fwd: bad inSpatial");
  read_u32_array(kernelSpatialObj, hKSpatial, rank, "gondlin_cuda_conv_fwd: bad kernelSpatial");
  read_u32_array(strideObj, hStride, rank, "gondlin_cuda_conv_fwd: bad stride");
  read_u32_array(paddingObj, hPadding, rank, "gondlin_cuda_conv_fwd: bad padding");

  for (int ax = 0; ax < rank; ++ax) {
    if (hKSpatial[ax] == 0) {
      lean_internal_panic("gondlin_cuda_conv_fwd: kernel dims must be > 0");
    }
    if (hStride[ax] == 0) {
      lean_internal_panic("gondlin_cuda_conv_fwd: stride dims must be > 0");
    }
  ...
  checkBufSize(input, inElems, "gondlin_cuda_conv_fwd: input.size mismatch");
  checkBufSize(kernel, kElems, "gondlin_cuda_conv_fwd: kernel.size mismatch");
  checkBufSize(bias, bElems, "gondlin_cuda_conv_fwd: bias.size mismatch");
  ...
  gondlin_cuda_free_checked((void**)&dInSpatial, "cudaFree convnd inSpatial failed");
  gondlin_cuda_free_checked((void**)&dOutSpatial, "cudaFree convnd outSpatial failed");
  gondlin_cuda_free_checked((void**)&dKSpatial, "cudaFree convnd kSpatial failed");
  gondlin_cuda_free_checked((void**)&dStride, "cudaFree convnd stride failed");
  gondlin_cuda_free_checked((void**)&dPadding, "cudaFree convnd padding failed");

  return gondlin_cuda_buffer_box(out);
}

extern "C" LEAN_EXPORT lean_obj_res gondlin_cuda_conv_bwd(
    b_lean_obj_arg inputObj, b_lean_obj_arg kernelObj, b_lean_obj_arg gradObj,
    b_lean_obj_arg inSpatialObj, b_lean_obj_arg kernelSpatialObj,
    b_lean_obj_arg strideObj, b_lean_obj_arg paddingObj,
    uint32_t inC, uint32_t outC) {
  ...
  const int rank = read_rank_checked(inSpatialObj, "gondlin_cuda_conv_bwd: bad inSpatial");
  if (read_rank_checked(kernelSpatialObj, "gondlin_cuda_conv_bwd: bad kernelSpatial") != rank ||
      read_rank_checked(strideObj, "gondlin_cuda_conv_bwd: bad stride") != rank ||
      read_rank_checked(paddingObj, "gondlin_cuda_conv_bwd: bad padding") != rank) {
    lean_internal_panic("gondlin_cuda_conv_bwd: array rank mismatch");
  }
  if (rank <= 0) {
    lean_internal_panic("gondlin_cuda_conv_bwd: rank must be > 0");
  }
  ...
  read_u32_array(inSpatialObj, hInSpatial, rank, "gondlin_cuda_conv_bwd: bad inSpatial");
  read_u32_array(kernelSpatialObj, hKSpatial, rank, "gondlin_cuda_conv_bwd: bad kernelSpatial");
  read_u32_array(strideObj, hStride, rank, "gondlin_cuda_conv_bwd: bad stride");
  read_u32_array(paddingObj, hPadding, rank, "gondlin_cuda_conv_bwd: bad padding");

  for (int ax = 0; ax < rank; ++ax) {
    if (hKSpatial[ax] == 0) {
      lean_internal_panic("gondlin_cuda_conv_bwd: kernel dims must be > 0");
    }
    if (hStride[ax] == 0) {
      lean_internal_panic("gondlin_cuda_conv_bwd: stride dims must be > 0");
    }
    hOutSpatial[ax] = outDim(hInSpatial[ax], hKSpatial[ax], hStride[ax], hPadding[ax]);
  }
  ...
  checkBufSize(input, inElems, "gondlin_cuda_conv_bwd: input.size mismatch");
  checkBufSize(kernel, kElems, "gondlin_cuda_conv_bwd: kernel.size mismatch");
  checkBufSize(gradOutput, outElems, "gondlin_cuda_conv_bwd: gradOutput.size mismatch");
  ...
  gondlin_cuda_free_checked((void**)&dInSpatial, "cudaFree convnd_bwd inSpatial failed");
  gondlin_cuda_free_checked((void**)&dOutSpatial, "cudaFree convnd_bwd outSpatial failed");
  gondlin_cuda_free_checked((void**)&dKSpatial, "cudaFree convnd_bwd kSpatial failed");
  gondlin_cuda_free_checked((void**)&dStride, "cudaFree convnd_bwd stride failed");
  gondlin_cuda_free_checked((void**)&dPadding, "cudaFree convnd_bwd padding failed");
...
extern "C" LEAN_EXPORT lean_obj_res gondlin_cuda_convtranspose_fwd(
    b_lean_obj_arg inputObj, b_lean_obj_arg kernelObj, b_lean_obj_arg biasObj,
    b_lean_obj_arg inSpatialObj, b_lean_obj_arg kernelSpatialObj,
    b_lean_obj_arg strideObj, b_lean_obj_arg paddingObj,
    uint32_t inC, uint32_t outC) {
  ...
  const int rank = read_rank_checked(inSpatialObj, "gondlin_cuda_convtranspose_fwd: bad inSpatial");
  if (read_rank_checked(kernelSpatialObj, "gondlin_cuda_convtranspose_fwd: bad kernelSpatial") != rank ||
      read_rank_checked(strideObj, "gondlin_cuda_convtranspose_fwd: bad stride") != rank ||
      read_rank_checked(paddingObj, "gondlin_cuda_convtranspose_fwd: bad padding") != rank) {
    lean_internal_panic("gondlin_cuda_convtranspose_fwd: array rank mismatch");
  }
  if (rank <= 0) {
    lean_internal_panic("gondlin_cuda_convtranspose_fwd: rank must be > 0");
  }
  ...
  read_u32_array(inSpatialObj, hInSpatial, rank, "gondlin_cuda_convtranspose_fwd: bad inSpatial");
  read_u32_array(kernelSpatialObj, hKSpatial, rank, "gondlin_cuda_convtranspose_fwd: bad kernelSpatial");
  read_u32_array(strideObj, hStride, rank, "gondlin_cuda_convtranspose_fwd: bad stride");
  read_u32_array(paddingObj, hPadding, rank, "gondlin_cuda_convtranspose_fwd: bad padding");

  for (int ax = 0; ax < rank; ++ax) {
    if (hKSpatial[ax] == 0) {
      lean_internal_panic("gondlin_cuda_convtranspose_fwd: kernel dims must be > 0");
    }
    if (hStride[ax] == 0) {
      lean_internal_panic("gondlin_cuda_convtranspose_fwd: stride dims must be > 0");
    }
  ...
  checkBufSize(input, inElems, "gondlin_cuda_convtranspose_fwd: input.size mismatch");
  checkBufSize(kernel, kElems, "gondlin_cuda_convtranspose_fwd: kernel.size mismatch");
  checkBufSize(bias, bElems, "gondlin_cuda_convtranspose_fwd: bias.size mismatch");
  ...
  gondlin_cuda_free_checked((void**)&dInSpatial, "cudaFree convtranspose inSpatial failed");
  gondlin_cuda_free_checked((void**)&dOutSpatial, "cudaFree convtranspose outSpatial failed");
  gondlin_cuda_free_checked((void**)&dKSpatial, "cudaFree convtranspose kSpatial failed");
  gondlin_cuda_free_checked((void**)&dStride, "cudaFree convtranspose stride failed");
  gondlin_cuda_free_checked((void**)&dPadding, "cudaFree convtranspose padding failed");

  return gondlin_cuda_buffer_box(out);
}

extern "C" LEAN_EXPORT lean_obj_res gondlin_cuda_convtranspose_bwd(
    b_lean_obj_arg inputObj, b_lean_obj_arg kernelObj, b_lean_obj_arg gradObj,
    b_lean_obj_arg inSpatialObj, b_lean_obj_arg kernelSpatialObj,
    b_lean_obj_arg strideObj, b_lean_obj_arg paddingObj,
    uint32_t inC, uint32_t outC) {
  ...
  const int rank = read_rank_checked(inSpatialObj, "gondlin_cuda_convtranspose_bwd: bad inSpatial");
  if (read_rank_checked(kernelSpatialObj, "gondlin_cuda_convtranspose_bwd: bad kernelSpatial") != rank ||
      read_rank_checked(strideObj, "gondlin_cuda_convtranspose_bwd: bad stride") != rank ||
      read_rank_checked(paddingObj, "gondlin_cuda_convtranspose_bwd: bad padding") != rank) {
    lean_internal_panic("gondlin_cuda_convtranspose_bwd: array rank mismatch");
  }
  if (rank <= 0) {
    lean_internal_panic("gondlin_cuda_convtranspose_bwd: rank must be > 0");
  }
  ...
  read_u32_array(inSpatialObj, hInSpatial, rank, "gondlin_cuda_convtranspose_bwd: bad inSpatial");
  read_u32_array(kernelSpatialObj, hKSpatial, rank, "gondlin_cuda_convtranspose_bwd: bad kernelSpatial");
  read_u32_array(strideObj, hStride, rank, "gondlin_cuda_convtranspose_bwd: bad stride");
  read_u32_array(paddingObj, hPadding, rank, "gondlin_cuda_convtranspose_bwd: bad padding");

  for (int ax = 0; ax < rank; ++ax) {
    if (hKSpatial[ax] == 0) {
      lean_internal_panic("gondlin_cuda_convtranspose_bwd: kernel dims must be > 0");
    }
    if (hStride[ax] == 0) {
      lean_internal_panic("gondlin_cuda_convtranspose_bwd: stride dims must be > 0");
    }
    hOutSpatial[ax] = outDimTranspose(hInSpatial[ax], hKSpatial[ax], hStride[ax], hPadding[ax]);
  }
  ...
  checkBufSize(input, inElems, "gondlin_cuda_convtranspose_bwd: input.size mismatch");
  checkBufSize(kernel, kElems, "gondlin_cuda_convtranspose_bwd: kernel.size mismatch");
  checkBufSize(gradOutput, outElems, "gondlin_cuda_convtranspose_bwd: gradOutput.size mismatch");
  ...
  gondlin_cuda_free_checked((void**)&dInSpatial, "cudaFree convtranspose_bwd inSpatial failed");
  gondlin_cuda_free_checked((void**)&dOutSpatial, "cudaFree convtranspose_bwd outSpatial failed");
  gondlin_cuda_free_checked((void**)&dKSpatial, "cudaFree convtranspose_bwd kSpatial failed");
  gondlin_cuda_free_checked((void**)&dStride, "cudaFree convtranspose_bwd stride failed");
  gondlin_cuda_free_checked((void**)&dPadding, "cudaFree convtranspose_bwd padding failed");
...
extern "C" LEAN_EXPORT lean_obj_res gondlin_cuda_maxpool_fwd(
    b_lean_obj_arg inputObj,
  ...
  const int rank = read_rank_checked(inSpatialObj, "gondlin_cuda_maxpool_fwd: bad inSpatial");
  if (read_rank_checked(kernelObj, "gondlin_cuda_maxpool_fwd: bad kernel") != rank ||
      read_rank_checked(strideObj, "gondlin_cuda_maxpool_fwd: bad stride") != rank ||
      read_rank_checked(paddingObj, "gondlin_cuda_maxpool_fwd: bad padding") != rank) {
    lean_internal_panic("gondlin_cuda_maxpool_fwd: array rank mismatch");
  }
  if (rank <= 0) {
    lean_internal_panic("gondlin_cuda_maxpool_fwd: rank must be > 0");
  }
  ...
  read_u32_array(inSpatialObj, hInSpatial, rank, "gondlin_cuda_maxpool_fwd: bad inSpatial");
  read_u32_array(kernelObj, hKSpatial, rank, "gondlin_cuda_maxpool_fwd: bad kernel");
  read_u32_array(strideObj, hStride, rank, "gondlin_cuda_maxpool_fwd: bad stride");
  read_u32_array(paddingObj, hPadding, rank, "gondlin_cuda_maxpool_fwd: bad padding");

  for (int ax = 0; ax < rank; ++ax) {
    if (hKSpatial[ax] == 0) {
      lean_internal_panic("gondlin_cuda_maxpool_fwd: kernel dims must be > 0");
    }
    if (hStride[ax] == 0) {
      lean_internal_panic("gondlin_cuda_maxpool_fwd: stride dims must be > 0");
    }
    hOutSpatial[ax] = outDim(hInSpatial[ax], hKSpatial[ax], hStride[ax], hPadding[ax]);
  }
  ...
  checkBufSize(input, inElems, "gondlin_cuda_maxpool_fwd: input.size mismatch");
  ...
  gondlin_cuda_free_checked((void**)&dInSpatial, "cudaFree maxpoolnd inSpatial failed");
  gondlin_cuda_free_checked((void**)&dOutSpatial, "cudaFree maxpoolnd outSpatial failed");
  gondlin_cuda_free_checked((void**)&dKSpatial, "cudaFree maxpoolnd kSpatial failed");
  gondlin_cuda_free_checked((void**)&dStride, "cudaFree maxpoolnd stride failed");
  gondlin_cuda_free_checked((void**)&dPadding, "cudaFree maxpoolnd padding failed");

  return gondlin_cuda_buffer_box(out);
}

extern "C" LEAN_EXPORT lean_obj_res gondlin_cuda_maxpool_bwd(
    b_lean_obj_arg inputObj, b_lean_obj_arg gradObj,
    b_lean_obj_arg inSpatialObj, b_lean_obj_arg kernelObj,
    b_lean_obj_arg strideObj, b_lean_obj_arg paddingObj,
    uint32_t inC) {
  ...
  const int rank = read_rank_checked(inSpatialObj, "gondlin_cuda_maxpool_bwd: bad inSpatial");
  if (read_rank_checked(kernelObj, "gondlin_cuda_maxpool_bwd: bad kernel") != rank ||
      read_rank_checked(strideObj, "gondlin_cuda_maxpool_bwd: bad stride") != rank ||
      read_rank_checked(paddingObj, "gondlin_cuda_maxpool_bwd: bad padding") != rank) {
    lean_internal_panic("gondlin_cuda_maxpool_bwd: array rank mismatch");
  }
  if (rank <= 0) {
    lean_internal_panic("gondlin_cuda_maxpool_bwd: rank must be > 0");
  }
  ...
  read_u32_array(inSpatialObj, hInSpatial, rank, "gondlin_cuda_maxpool_bwd: bad inSpatial");
  read_u32_array(kernelObj, hKSpatial, rank, "gondlin_cuda_maxpool_bwd: bad kernel");
  read_u32_array(strideObj, hStride, rank, "gondlin_cuda_maxpool_bwd: bad stride");
  read_u32_array(paddingObj, hPadding, rank, "gondlin_cuda_maxpool_bwd: bad padding");

  for (int ax = 0; ax < rank; ++ax) {
    if (hKSpatial[ax] == 0) {
      lean_internal_panic("gondlin_cuda_maxpool_bwd: kernel dims must be > 0");
    }
    if (hStride[ax] == 0) {
      lean_internal_panic("gondlin_cuda_maxpool_bwd: stride dims must be > 0");
    }
    hOutSpatial[ax] = outDim(hInSpatial[ax], hKSpatial[ax], hStride[ax], hPadding[ax]);
  }
  ...
  checkBufSize(input, inElems, "gondlin_cuda_maxpool_bwd: input.size mismatch");
  checkBufSize(gradOutput, outElems, "gondlin_cuda_maxpool_bwd: gradOutput.size mismatch");
  ...
  gondlin_cuda_free_checked((void**)&dInSpatial, "cudaFree maxpoolnd_bwd inSpatial failed");
  gondlin_cuda_free_checked((void**)&dOutSpatial, "cudaFree maxpoolnd_bwd outSpatial failed");
  gondlin_cuda_free_checked((void**)&dKSpatial, "cudaFree maxpoolnd_bwd kSpatial failed");
  gondlin_cuda_free_checked((void**)&dStride, "cudaFree maxpoolnd_bwd stride failed");
  gondlin_cuda_free_checked((void**)&dPadding, "cudaFree maxpoolnd_bwd padding failed");

  return gondlin_cuda_buffer_box(dInput);
}

extern "C" LEAN_EXPORT lean_obj_res gondlin_cuda_avgpool_fwd(
    b_lean_obj_arg inputObj,
    b_lean_obj_arg inSpatialObj, b_lean_obj_arg kernelObj,
    b_lean_obj_arg strideObj, b_lean_obj_arg paddingObj,
    uint32_t inC) {
  ...
  const int rank = read_rank_checked(inSpatialObj, "gondlin_cuda_avgpool_fwd: bad inSpatial");
  if (read_rank_checked(kernelObj, "gondlin_cuda_avgpool_fwd: bad kernel") != rank ||
      read_rank_checked(strideObj, "gondlin_cuda_avgpool_fwd: bad stride") != rank ||
      read_rank_checked(paddingObj, "gondlin_cuda_avgpool_fwd: bad padding") != rank) {
    lean_internal_panic("gondlin_cuda_avgpool_fwd: array rank mismatch");
  }
  if (rank <= 0) {
    lean_internal_panic("gondlin_cuda_avgpool_fwd: rank must be > 0");
  }
  ...
  read_u32_array(inSpatialObj, hInSpatial, rank, "gondlin_cuda_avgpool_fwd: bad inSpatial");
  read_u32_array(kernelObj, hKSpatial, rank, "gondlin_cuda_avgpool_fwd: bad kernel");
  read_u32_array(strideObj, hStride, rank, "gondlin_cuda_avgpool_fwd: bad stride");
  read_u32_array(paddingObj, hPadding, rank, "gondlin_cuda_avgpool_fwd: bad padding");

  for (int ax = 0; ax < rank; ++ax) {
    if (hKSpatial[ax] == 0) {
      lean_internal_panic("gondlin_cuda_avgpool_fwd: kernel dims must be > 0");
    }
    if (hStride[ax] == 0) {
      lean_internal_panic("gondlin_cuda_avgpool_fwd: stride dims must be > 0");
    }
    hOutSpatial[ax] = outDim(hInSpatial[ax], hKSpatial[ax], hStride[ax], hPadding[ax]);
  }
  ...
  checkBufSize(input, inElems, "gondlin_cuda_avgpool_fwd: input.size mismatch");
  ...
  gondlin_cuda_free_checked((void**)&dInSpatial, "cudaFree avgpoolnd inSpatial failed");
  gondlin_cuda_free_checked((void**)&dOutSpatial, "cudaFree avgpoolnd outSpatial failed");
  gondlin_cuda_free_checked((void**)&dKSpatial, "cudaFree avgpoolnd kSpatial failed");
  gondlin_cuda_free_checked((void**)&dStride, "cudaFree avgpoolnd stride failed");
  gondlin_cuda_free_checked((void**)&dPadding, "cudaFree avgpoolnd padding failed");

  return gondlin_cuda_buffer_box(out);
}

extern "C" LEAN_EXPORT lean_obj_res gondlin_cuda_avgpool_bwd(
    b_lean_obj_arg gradObj,
    b_lean_obj_arg inSpatialObj, b_lean_obj_arg kernelObj,
    b_lean_obj_arg strideObj, b_lean_obj_arg paddingObj,
    uint32_t inC) {
  ...
  const int rank = read_rank_checked(inSpatialObj, "gondlin_cuda_avgpool_bwd: bad inSpatial");
  if (read_rank_checked(kernelObj, "gondlin_cuda_avgpool_bwd: bad kernel") != rank ||
      read_rank_checked(strideObj, "gondlin_cuda_avgpool_bwd: bad stride") != rank ||
      read_rank_checked(paddingObj, "gondlin_cuda_avgpool_bwd: bad padding") != rank) {
    lean_internal_panic("gondlin_cuda_avgpool_bwd: array rank mismatch");
  }
  if (rank <= 0) {
    lean_internal_panic("gondlin_cuda_avgpool_bwd: rank must be > 0");
  }
  ...
  read_u32_array(inSpatialObj, hInSpatial, rank, "gondlin_cuda_avgpool_bwd: bad inSpatial");
  read_u32_array(kernelObj, hKSpatial, rank, "gondlin_cuda_avgpool_bwd: bad kernel");
  read_u32_array(strideObj, hStride, rank, "gondlin_cuda_avgpool_bwd: bad stride");
  read_u32_array(paddingObj, hPadding, rank, "gondlin_cuda_avgpool_bwd: bad padding");

  for (int ax = 0; ax < rank; ++ax) {
    if (hKSpatial[ax] == 0) {
      lean_internal_panic("gondlin_cuda_avgpool_bwd: kernel dims must be > 0");
    }
    if (hStride[ax] == 0) {
      lean_internal_panic("gondlin_cuda_avgpool_bwd: stride dims must be > 0");
    }
    hOutSpatial[ax] = outDim(hInSpatial[ax], hKSpatial[ax], hStride[ax], hPadding[ax]);
  }
  ...
  checkBufSize(gradOutput, outElems, "gondlin_cuda_avgpool_bwd: gradOutput.size mismatch");
  ...
  gondlin_cuda_free_checked((void**)&dInSpatial, "cudaFree avgpoolnd_bwd inSpatial failed");
  gondlin_cuda_free_checked((void**)&dOutSpatial, "cudaFree avgpoolnd_bwd outSpatial failed");
  gondlin_cuda_free_checked((void**)&dKSpatial, "cudaFree avgpoolnd_bwd kSpatial failed");
  gondlin_cuda_free_checked((void**)&dStride, "cudaFree avgpoolnd_bwd stride failed");
  gondlin_cuda_free_checked((void**)&dPadding, "cudaFree avgpoolnd_bwd padding failed");

  return gondlin_cuda_buffer_box(dInput);
}

extern "C" LEAN_EXPORT lean_obj_res gondlin_cuda_smooth_maxpool_fwd(
    b_lean_obj_arg inputObj, double beta,
    b_lean_obj_arg inSpatialObj, b_lean_obj_arg kernelObj,
    b_lean_obj_arg strideObj, b_lean_obj_arg paddingObj,
    uint32_t inC) {
  ...
  const int rank = read_rank_checked(inSpatialObj, "gondlin_cuda_smooth_maxpool_fwd: bad inSpatial");
  if (read_rank_checked(kernelObj, "gondlin_cuda_smooth_maxpool_fwd: bad kernel") != rank ||
      read_rank_checked(strideObj, "gondlin_cuda_smooth_maxpool_fwd: bad stride") != rank ||
      read_rank_checked(paddingObj, "gondlin_cuda_smooth_maxpool_fwd: bad padding") != rank) {
    lean_internal_panic("gondlin_cuda_smooth_maxpool_fwd: array rank mismatch");
  }
  if (rank <= 0) {
    lean_internal_panic("gondlin_cuda_smooth_maxpool_fwd: rank must be > 0");
  }
  ...
  read_u32_array(inSpatialObj, hInSpatial, rank, "gondlin_cuda_smooth_maxpool_fwd: bad inSpatial");
  read_u32_array(kernelObj, hKSpatial, rank, "gondlin_cuda_smooth_maxpool_fwd: bad kernel");
  read_u32_array(strideObj, hStride, rank, "gondlin_cuda_smooth_maxpool_fwd: bad stride");
  read_u32_array(paddingObj, hPadding, rank, "gondlin_cuda_smooth_maxpool_fwd: bad padding");

  for (int ax = 0; ax < rank; ++ax) {
    if (hKSpatial[ax] == 0) {
      lean_internal_panic("gondlin_cuda_smooth_maxpool_fwd: kernel dims must be > 0");
    }
    if (hStride[ax] == 0) {
      lean_internal_panic("gondlin_cuda_smooth_maxpool_fwd: stride dims must be > 0");
    }
    hOutSpatial[ax] = outDim(hInSpatial[ax], hKSpatial[ax], hStride[ax], hPadding[ax]);
  }
  ...
  checkBufSize(input, inElems, "gondlin_cuda_smooth_maxpool_fwd: input.size mismatch");
  ...
  const int grid = grid_for(outElems);
  const float betaF =
      checked_smoothmax_beta(beta, "gondlin_cuda_smooth_maxpool_fwd: beta must be finite and nonzero");
  ...
  gondlin_cuda_free_checked((void**)&dInSpatial, "cudaFree smoothmaxpoolnd inSpatial failed");
  gondlin_cuda_free_checked((void**)&dOutSpatial, "cudaFree smoothmaxpoolnd outSpatial failed");
  gondlin_cuda_free_checked((void**)&dKSpatial, "cudaFree smoothmaxpoolnd kSpatial failed");
  gondlin_cuda_free_checked((void**)&dStride, "cudaFree smoothmaxpoolnd stride failed");
  gondlin_cuda_free_checked((void**)&dPadding, "cudaFree smoothmaxpoolnd padding failed");

  return gondlin_cuda_buffer_box(out);
}

extern "C" LEAN_EXPORT lean_obj_res gondlin_cuda_smooth_maxpool_bwd(
    b_lean_obj_arg inputObj, b_lean_obj_arg gradObj, double beta,
    b_lean_obj_arg inSpatialObj, b_lean_obj_arg kernelObj,
    b_lean_obj_arg strideObj, b_lean_obj_arg paddingObj,
    uint32_t inC) {
  ...
  const int rank = read_rank_checked(inSpatialObj, "gondlin_cuda_smooth_maxpool_bwd: bad inSpatial");
  if (read_rank_checked(kernelObj, "gondlin_cuda_smooth_maxpool_bwd: bad kernel") != rank ||
      read_rank_checked(strideObj, "gondlin_cuda_smooth_maxpool_bwd: bad stride") != rank ||
      read_rank_checked(paddingObj, "gondlin_cuda_smooth_maxpool_bwd: bad padding") != rank) {
    lean_internal_panic("gondlin_cuda_smooth_maxpool_bwd: array rank mismatch");
  }
  if (rank <= 0) {
    lean_internal_panic("gondlin_cuda_smooth_maxpool_bwd: rank must be > 0");
  }
  ...
  read_u32_array(inSpatialObj, hInSpatial, rank, "gondlin_cuda_smooth_maxpool_bwd: bad inSpatial");
  read_u32_array(kernelObj, hKSpatial, rank, "gondlin_cuda_smooth_maxpool_bwd: bad kernel");
  read_u32_array(strideObj, hStride, rank, "gondlin_cuda_smooth_maxpool_bwd: bad stride");
  read_u32_array(paddingObj, hPadding, rank, "gondlin_cuda_smooth_maxpool_bwd: bad padding");

  for (int ax = 0; ax < rank; ++ax) {
    if (hKSpatial[ax] == 0) {
      lean_internal_panic("gondlin_cuda_smooth_maxpool_bwd: kernel dims must be > 0");
    }
    if (hStride[ax] == 0) {
      lean_internal_panic("gondlin_cuda_smooth_maxpool_bwd: stride dims must be > 0");
    }
    hOutSpatial[ax] = outDim(hInSpatial[ax], hKSpatial[ax], hStride[ax], hPadding[ax]);
  }
  ...
  checkBufSize(input, inElems, "gondlin_cuda_smooth_maxpool_bwd: input.size mismatch");
  checkBufSize(gradOutput, outElems, "gondlin_cuda_smooth_maxpool_bwd: gradOutput.size mismatch");

  gondlin_cuda_buffer* dInput = gondlin_cuda_buffer_alloc(inElems);
  const float betaF =
      checked_smoothmax_beta(beta, "gondlin_cuda_smooth_maxpool_bwd: beta must be finite and nonzero");
  const bool det = gondlin_cuda_deterministic_reductions_enabled();
  if (!det && dInput->size > 0) {
    // Atomic path accumulates into dInput; must clear first.
    checkCuda(cudaMemset(dInput->data, 0, dInput->size * sizeof(float)), "cudaMemset dInput failed");
  ...
  gondlin_cuda_free_checked((void**)&dInSpatial, "cudaFree smoothmaxpoolnd_bwd inSpatial failed");
  gondlin_cuda_free_checked((void**)&dOutSpatial, "cudaFree smoothmaxpoolnd_bwd outSpatial failed");
  gondlin_cuda_free_checked((void**)&dKSpatial, "cudaFree smoothmaxpoolnd_bwd kSpatial failed");
  gondlin_cuda_free_checked((void**)&dStride, "cudaFree smoothmaxpoolnd_bwd stride failed");
  gondlin_cuda_free_checked((void**)&dPadding, "cudaFree smoothmaxpoolnd_bwd padding failed");
