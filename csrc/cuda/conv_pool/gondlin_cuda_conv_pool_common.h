...
// The formulas intentionally mirror Gondlin's Nat-style specs: subtractions truncate at zero
// instead of becoming negative, and host wrappers reject invalid kernel/stride metadata before work
// reaches the device.

enum { GONDLIN_CUDA_CONV_POOL_MAX_RANK = 8 };

static inline void checkBufSize(gondlin_cuda_buffer* b, size_t elems, const char* msg) {
  if (b->size != elems) {
...
static inline uint32_t outDim(uint32_t in, uint32_t k, uint32_t stride, uint32_t padding) {
  if (stride == 0) {
    lean_internal_panic("gondlin_cuda_conv_pool: stride must be > 0");
  }
  // Gondlin spec uses Nat subtraction (truncated at 0).
...
static inline uint32_t outDimTranspose(uint32_t in, uint32_t k, uint32_t stride, uint32_t padding) {
  ...
  if (t > (uint64_t)UINT32_MAX) {
    ...
    lean_internal_panic("gondlin_cuda_conv_pool: outDimTranspose overflow");
#else
    lean_internal_panic("gondlin_cuda_conv_pool_stub: outDimTranspose overflow");
#endif
  }
  return (uint32_t)t;
}
...
static inline int read_rank_checked(b_lean_obj_arg arrObj, const char* msg) {
  ...
  if (n > (size_t)GONDLIN_CUDA_CONV_POOL_MAX_RANK) {
    lean_internal_panic(msg);
  }
  return (int)n;
}
...
#if defined(__CUDACC__)
#define GONDLIN_CUDA_CONV_POOL_HD __host__ __device__
#else
#define GONDLIN_CUDA_CONV_POOL_HD
#endif

GONDLIN_CUDA_CONV_POOL_HD static inline int64_t floor_div_i64(int64_t a, int64_t b) {
...
GONDLIN_CUDA_CONV_POOL_HD static inline int64_t ceil_div_i64(int64_t a, int64_t b) {
  // b must be > 0.
  return -floor_div_i64(-a, b);
}

#undef GONDLIN_CUDA_CONV_POOL_HD
