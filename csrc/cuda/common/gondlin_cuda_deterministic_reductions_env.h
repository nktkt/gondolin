...
//
// - `GONDLIN_CUDA_DETERMINISTIC_REDUCTIONS=1` (preferred)
// - `GONDLIN_DETERMINISTIC_REDUCTIONS=1` (alias)
static inline uint32_t gondlin_read_deterministic_reductions_env() {
  const char* v = getenv("GONDLIN_CUDA_DETERMINISTIC_REDUCTIONS");
  if (!v || !*v) {
    v = getenv("GONDLIN_DETERMINISTIC_REDUCTIONS");
  }
  if (!v || !*v) {
    return 0u;
  }
