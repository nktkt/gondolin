...
// - Lean owns an external object that points at `gondlin_cuda_buffer`;
...
// Helpers implemented by `gondlin_cuda_tensor.cu` / `gondlin_cuda_tensor_stub.c`.
...
//
// Some kernels use `atomicAdd`, which is fast but can be non-deterministic. When enabled, Gondlin
