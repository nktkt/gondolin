...
This module is the DocGen-facing home for Gondolin's native CUDA/C runtime notes.

DocGen documents Lean modules, not C or CUDA translation units. Instead of publishing a separate
Jekyll source browser, Gondolin keeps the native-source map here, beside the Lean FFI modules that
...
The CUDA backend is a validated implementation of Gondolin's float32 eager runtime. Lean does not
...
Gondolin's proof-side CUDA contract therefore lives one level up: Lean states pure kernel specs,
...
- `csrc/cuda/common/gondolin_cuda_buffer.h`
...
- `csrc/cuda/common/gondolin_cuda_deterministic_reductions_env.h`
  Environment-variable parser for deterministic CUDA reduction mode.

- `csrc/cuda/common/gondolin_cuda_rng_common.h`
...
- `csrc/cuda/tensor/gondolin_cuda_tensor.cu`
...
- `csrc/cuda/tensor/gondolin_cuda_tensor_stub.c`
  Portable CPU implementation of the tensor-buffer FFI symbols used when Gondolin is built without
  CUDA.

- `csrc/cuda/kernels/gondolin_cuda_kernels.cu`
...
- `csrc/cuda/kernels/gondolin_cuda_kernels_stub.c`
  Portable CPU mirror of the general tensor-kernel FFI surface.

- `csrc/cuda/conv_pool/gondolin_cuda_conv_pool_common.h`
  Shared convolution/pooling shape arithmetic and rank limits.

- `csrc/cuda/conv_pool/gondolin_cuda_conv_pool.cu`
...
- `csrc/cuda/conv_pool/gondolin_cuda_conv_pool_stub.c`
  Portable CPU mirror of the convolution and pooling FFI surface.

- `csrc/cuda/blas/gondolin_dgemm_cuda.cu`
...
- `csrc/cuda/blas/gondolin_dgemm_cuda_stub.c`
...
FFI modules.
-/
def docAnchor : String := "Gondolin CUDA native source map"
