...
This module is the DocGen-facing home for Gondlin's native CUDA/C runtime notes.

DocGen documents Lean modules, not C or CUDA translation units. Instead of publishing a separate
Jekyll source browser, Gondlin keeps the native-source map here, beside the Lean FFI modules that
...
The CUDA backend is a validated implementation of Gondlin's float32 eager runtime. Lean does not
...
Gondlin's proof-side CUDA contract therefore lives one level up: Lean states pure kernel specs,
...
- `csrc/cuda/common/gondlin_cuda_buffer.h`
...
- `csrc/cuda/common/gondlin_cuda_deterministic_reductions_env.h`
  Environment-variable parser for deterministic CUDA reduction mode.

- `csrc/cuda/common/gondlin_cuda_rng_common.h`
...
- `csrc/cuda/tensor/gondlin_cuda_tensor.cu`
...
- `csrc/cuda/tensor/gondlin_cuda_tensor_stub.c`
  Portable CPU implementation of the tensor-buffer FFI symbols used when Gondlin is built without
  CUDA.

- `csrc/cuda/kernels/gondlin_cuda_kernels.cu`
...
- `csrc/cuda/kernels/gondlin_cuda_kernels_stub.c`
  Portable CPU mirror of the general tensor-kernel FFI surface.

- `csrc/cuda/conv_pool/gondlin_cuda_conv_pool_common.h`
  Shared convolution/pooling shape arithmetic and rank limits.

- `csrc/cuda/conv_pool/gondlin_cuda_conv_pool.cu`
...
- `csrc/cuda/conv_pool/gondlin_cuda_conv_pool_stub.c`
  Portable CPU mirror of the convolution and pooling FFI surface.

- `csrc/cuda/blas/gondlin_dgemm_cuda.cu`
...
- `csrc/cuda/blas/gondlin_dgemm_cuda_stub.c`
...
FFI modules.
-/
def docAnchor : String := "Gondlin CUDA native source map"
