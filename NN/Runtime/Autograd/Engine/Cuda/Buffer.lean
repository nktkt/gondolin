...
Gondlin's CUDA runtime uses `atomicAdd` in a few kernels to accumulate float32 results. This is
...
Gondlin therefore exposes an opt-in deterministic mode that replaces those atomic accumulation
...
@[extern "gondlin_cuda_set_deterministic_reductions"]
opaque setDeterministicReductionsRaw (on : UInt32) : Unit

@[extern "gondlin_cuda_get_deterministic_reductions_u"]
opaque getDeterministicReductionsRaw (u : UInt32) : UInt32

@[extern "gondlin_cuda_set_deterministic_reductions_checked"]
...
-/
@[extern "gondlin_cuda_buffer_release"]
...
@[extern "gondlin_cuda_buffer_zeros"]
opaque zeros (n : UInt32) : Buffer

/-- Allocate a length-`n` buffer filled with `v` (host `Float`, cast to float32). -/
@[extern "gondlin_cuda_buffer_full"]
opaque full (n : UInt32) (v : Float) : Buffer

/-!
### Deterministic RNG (device-side)

These are low-level building blocks used by Gondlin's seeded RNG ops (`rand_uniform`,
`bernoulli_mask`) when running on the eager CUDA backend.

They intentionally use the same SplitMix64-style mixing as `Gondlin.Random` so results are
...
-/
@[extern "gondlin_cuda_buffer_max_bwd"]
opaque maxBwd (a b dLdy : Buffer) : Buffer × Buffer

@[extern "gondlin_cuda_buffer_min"]
...
-/
@[extern "gondlin_cuda_buffer_scale"]
...
-/
@[extern "gondlin_cuda_buffer_axpy"]
opaque axpy (a b : Buffer) (c : Float) : Buffer

/-- Reductions (return a length-1 buffer). -/
@[extern "gondlin_cuda_buffer_reduce_sum"]
opaque reduceSum (b : Buffer) : Buffer

@[extern "gondlin_cuda_buffer_reduce_mean"]
