...
This is runtime/executable functionality intended for Gondolin ergonomics; it is separate from
...
def gradsPrefix {α : Type} [DecidableEq Shape] :
  ...
  | s :: ss, grads, off => do
      let any ← match grads[off]? with
        | some v => pure v
        | none => throw <| IO.userError "gondolin(hvp): gradient array too small"
        if h : any.s = s then
          ...
          throw <| IO.userError <|
            s!"gondolin(hvp): gradient shape mismatch at idx={off} (expected "
...
/--
Compile a scalar Gondolin program to a reusable `CompiledScalar` (static SSA/DAG + output node).
-/
def compileLoss {α : Type} [Context α] [DecidableEq Shape]
    {paramShapes inputShapes : List Shape}
    (loss :
      ∀ {β : Type}, [Context β] → [DecidableEq Shape] →
        Gondolin.Program β (paramShapes ++ inputShapes) Shape.scalar) :
...
/--
Compile a Gondolin program to a reusable `CompiledOut` (static SSA/DAG + output node).
...
def compileOut {α : Type} [Context α] [DecidableEq Shape]
    ...
    (f :
      ∀ {β : Type}, [Context β] → [DecidableEq Shape] →
        Gondolin.Program β (paramShapes ++ inputShapes) τ) :
...
def hessian1 {α : Type} [Context α] [DecidableEq Shape]
    ...
    (f :
      ∀ {β : Type}, [Context β] → [DecidableEq Shape] →
        Gondolin.Program β [σ] Shape.scalar)
    (x : Tensor α σ) :
