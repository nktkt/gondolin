...
Gondolin module wrappers with PyTorch-style ergonomics.

Gondolin already provides the core ingredients:
...
/-- List-shaped version of `castTensor` for Gondolin's `TList` parameter bundles. -/
...
structure ScalarModuleDef (paramShapes inputShapes : List Shape) where
  ...
  loss :
    ∀ {α : Type}, [Context α] → [DecidableEq Shape] →
      Gondolin.Program α (paramShapes ++ inputShapes) Shape.scalar
...
def initOptim {α : Type} [Context α] [DecidableEq Shape]
    ...
    (opt : Gondolin.Optim.Optimizer α paramShapes) :
    IO opt.State :=
...
def stepWith {α : Type} [Context α] [DecidableEq Shape]
    {paramShapes inputShapes : List Shape}
    (m : ScalarModule α paramShapes inputShapes)
    (opt : Gondolin.Optim.Optimizer α paramShapes) (st : opt.State)
...
def trainWith {α : Type} [Context α] [DecidableEq Shape] [ToString α]
    {paramShapes inputShapes : List Shape}
    (m : ScalarModule α paramShapes inputShapes)
    (opt : Gondolin.Optim.Optimizer α paramShapes) (st0 : opt.State)
    (steps : Nat) (samples : List (Torch.TList α inputShapes))
    (logEvery : Nat := 1) : IO opt.State :=
  Gondolin.trainCycleOptim (α := α) (paramShapes := paramShapes) (inputShapes := inputShapes)
