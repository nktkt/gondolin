...
Important note about Gondolin’s layer architecture:
- Gondolin layers are scalar-polymorphic but **do not change scalar type** mid-model.
...
def reshapeVecToMat1 (grid : Nat) :
  ...
  { paramShapes := []
    ...
    forward := fun _ {α} _ _ =>
      fun {m} _ _ =>
        fun x => Gondolin.reshape (m := m) (α := α) (s₁ := s₁) (s₂ := s₂) x h
  }
      ...
      fun {m} _ _ =>
        fun w b x =>
          (show m (RefTy (m := m) (α := α) (mat grid outC)) from do
            ...
            let bb ←
              Gondolin.broadcastTo (m := m) (α := α)
                (s₁ := bShape) (s₂ := mat grid outC) Shape.BroadcastTo.proof b
            Gondolin.add (m := m) (α := α) (s := mat grid outC) y bb)
  }
      ...
      fun {m} _ _ =>
        fun wLow wHigh wSkip bSkip x =>
          (show m (RefTy (m := m) (α := α) (mat grid width)) from do
            ...
            let fR ← Gondolin.const (m := m) (α := α) (s := mat grid grid) f
            let fiR ← Gondolin.const (m := m) (α := α) (s := mat grid grid) fi
            ...
            let yMid ← Gondolin.const (m := m) (α := α) (s := mat midLen width) mid0

            -- Concatenate [low, mid, high] back to a `grid×width` spectrum
            let yLowMid ←
              Gondolin.concatDim0 (m := m) (α := α)
            ...
            let ySkip ← Gondolin.add (m := m) (α := α) (s := mat grid width) ySkip0 bSkipB
            let y ← Gondolin.add (m := m) (α := α) (s := mat grid width) ySpec ySkip

            match activation with
            | .tanh => Gondolin.tanh (m := m) (α := α) (s := mat grid width) y
            | .relu => Gondolin.relu (m := m) (α := α) (s := mat grid width) y)
  }
...
def block
    ...
    (hModes : 2 * modes ≤ grid) :
    LayerDef (mat grid width) (mat grid width) :=
  let wShape : Shape := _root_.Runtime.Autograd.Gondolin.NN.FNO1D.spectralWShape modes width
  let wSkipShape : Shape := mat width width
  let bSkipShape : Shape := vec width
    ...
    forward := fun _ {α} _ _ =>
      fun {m} _ _ =>
        fun wLowRe wLowIm wHighRe wHighIm wSkip bSkip x =>
          (show m (_root_.Runtime.Autograd.Gondolin.NN.Seq.RefT (m := m) (α := α) (mat grid width)) from do
            ...
            let mulModes
                (xr xi : _root_.Runtime.Autograd.Gondolin.NN.Seq.RefT (m := m) (α := α) (mat modes width))
                (wr wi : _root_.Runtime.Autograd.Gondolin.NN.Seq.RefT (m := m) (α := α)
                  (_root_.Runtime.Autograd.Gondolin.NN.FNO1D.spectralWShape modes width)) := do
              let hIn :
            ...
            let yHatRe : _root_.Runtime.Autograd.Gondolin.NN.Seq.RefT (m := m) (α := α) (mat grid width) := by
              have hSum : (modes + midLen) + modes = grid := by
                dsimp [midLen]
                calc
                  (modes + (grid - 2 * modes)) + modes
            ...
            let yHatIm : _root_.Runtime.Autograd.Gondolin.NN.Seq.RefT (m := m) (α := α) (mat grid width) := by
              have hSum : (modes + midLen) + modes = grid := by
                dsimp [midLen]
                calc
                  (modes + (grid - 2 * modes)) + modes
...
def blocksSeq (grid width modes blocks : Nat) (seed : Nat) (hModes : 2 * modes ≤ grid) :
    _root_.Runtime.Autograd.Gondolin.NN.Seq (mat grid width) (mat grid width) :=
  match blocks with
  | 0 => .id (mat grid width)
  | Nat.succ k =>
...
def model
    ...
    (hModes : 2 * modes ≤ grid) :
    Seq (vec grid) (vec grid) :=
  let reshapeIn := _root_.Runtime.Autograd.Gondolin.NN.FNO1D.reshapeVecToMat1 (grid := grid)
  let lift :=
    _root_.Runtime.Autograd.Gondolin.NN.FNO1D.matAffine (grid := grid) (inC := 1) (outC := width)
      (seedW := seed) (seedB := seed + 1)
  let proj :=
    _root_.Runtime.Autograd.Gondolin.NN.FNO1D.matAffine (grid := grid) (inC := width) (outC := 1)
      (seedW := seed + 1000) (seedB := seed + 1001)
  let reshapeOut := _root_.Runtime.Autograd.Gondolin.NN.FNO1D.reshapeMatToVec1 (grid := grid)
  Seq.cons reshapeIn <|
    Seq.cons lift <|
