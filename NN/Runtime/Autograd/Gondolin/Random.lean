...
Deterministic RNG utilities for Gondolin (seed-threaded, *pure*).
...
whose results depend on hidden runtime state. For Gondolin, that is a poor fit:
...
Today this is used by *Session-level* stochastic layers (e.g. `Gondolin.Session.dropout`), where
...
and then using the seeded RNG from that point onward (`Gondolin.Session.initRngFromIO`).
-/
