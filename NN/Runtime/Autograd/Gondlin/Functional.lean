...
Gondlin functional helpers in the style of `torch.*` building blocks.

These are *derived* ops built from the small primitive `Gondlin.Ops` surface, so they work for:
