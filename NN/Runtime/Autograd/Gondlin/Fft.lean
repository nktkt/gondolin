...
Gondlin’s layer/model definitions are scalar-polymorphic: a model runs over whatever scalar type
`α` you instantiate it with (e.g. `Float`, `IEEE32Exec`, `ℝ`, etc.).  A “real FFT” would normally
*change* the scalar type (real → complex), but Gondlin’s `LayerDef` does not support changing the
scalar type mid-model.

So this module provides **complex-domain** transforms: `fft` and `ifft` as layers that assume the
chosen scalar type `α` already behaves like a complex field (for example `Gondlin.Complex
...
This is intended to be instantiated with Gondlin’s own complex scalar
`Gondlin.Complex β` (for some base scalar `β`).  For real-only scalar backends, the formulas are
not meaningful.
-/
      ...
      fun {m} _ _ =>
        fun x =>
          (show m (RefTy (m := m) (α := α) sIn) from do
            let xMat ← Gondlin.reshape (m := m) (α := α) (s₁ := sIn) (s₂ := sMat) x hSz
            let f : Tensor α (mat n n) := dftMatrix (α := α) n
            let fR ← Gondlin.const (m := m) (α := α) (s := mat n n) f
            let yMat ←
              Gondlin.matmul (m := m) (α := α)
                (mDim := n) (nDim := n) (pDim := cols) fR xMat
            Gondlin.reshape (m := m) (α := α) (s₁ := sMat) (s₂ := sIn) yMat hSz.symm)
  }
...
def permuteBySwaps {α : Type} [Context α] [DecidableEq Shape]
  ...
  | [] => pure x
  | d :: ds => do
      let y ← Gondlin.swapAdjacentAtDepth (m := m) (α := α) (s := x.fst) d x.snd
