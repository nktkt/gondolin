...
`Gondlin.NN`: a compact `torch.nn`-style builder layer.
...
- `scalarModuleDef*` helpers bundle a `Seq` model together with a scalar loss, producing a
  `Gondlin.Module.ScalarModuleDef` that the runtime training code can execute.

## PyTorch analogies

- `LayerDef` is like a small `nn.Module` definition, except parameters are an explicit list instead
  of fields, and the forward pass is a typed Gondlin program.
...
The surface here is intentionally narrow: it supports Gondlin's executable model constructors and
...
Gondlin keeps "train vs eval" behavior explicit. This affects layers like dropout and
...
structure LayerDef (σ τ : Shape) where
  ...
  /--
  Forward pass as a typed Gondlin program.
  ...
  forward :
    Mode → ∀ {α : Type}, [Context α] → [DecidableEq Shape] →
      Gondlin.Program α (paramShapes ++ [σ]) τ
...
def evalTensor {σ τ : Shape} (l : LayerDef σ τ) (mode : Mode)
  ...
  let compiled ← _root_.Runtime.Autograd.Gondlin.Autodiff.compileOut (α := α)
...
def programWithMode {σ τ : Shape} (mode : Mode) (model : Seq σ τ)
    {α : Type} [Context α] [DecidableEq Shape] :
    Gondlin.Program α (paramShapes model ++ [σ]) τ :=
  fun {m} _ _ =>
  ...
  def compileOutWithMode {σ τ : Shape}
    ...
    _root_.Runtime.Autograd.Gondlin.Autodiff.compileOut (α := α)
...
The resulting `ScalarModuleDef` can be handed to Gondlin’s runtime training code: it knows how to
...
def scalarModuleDefWithMode {σ τ : Shape} (mode : Mode) (model : Seq σ τ)
    (loss : ∀ {α : Type}, [Context α] → [DecidableEq Shape] → Gondlin.Program α [τ, τ]
      Shape.scalar) :
    Gondlin.Module.ScalarModuleDef (paramShapes model) [σ, τ] :=
...
def mseScalarModuleDefWithMode {σ τ : Shape} (mode : Mode) (model : Seq σ τ)
    (reduction : Gondlin.Loss.Reduction := .mean) :
    Gondlin.Module.ScalarModuleDef (paramShapes model) [σ, τ] :=
  scalarModuleDefWithMode mode (model := model) (loss := fun {α} _ _ =>
    fun {m} _ _ =>
      fun yhat y => Gondlin.Loss.mse (m := m) (α := α) (s := τ) yhat y (reduction := reduction))

/-- Training-mode MSE wrapper. -/
def mseScalarModuleDef {σ τ : Shape} (model : Seq σ τ) (reduction : Gondlin.Loss.Reduction :=
  .mean) :
    Gondlin.Module.ScalarModuleDef (paramShapes model) [σ, τ] :=
  ...
  scalarModuleDefWithMode mode (model := model) (loss := fun {α} _ _ =>
    fun {m} _ _ =>
      fun logits targetOneHot =>
        Gondlin.Loss.crossEntropyOneHot (m := m) (α := α) (s := τ) logits targetOneHot
          (reduction := reduction))
...
This is implemented by unrolling a fixed number of steps (`seqLen`) using existing Gondlin ops,
    ...
    forward := fun _ {α} _ _ =>
      fun {m} _ _ =>
        fun w b xs => show m (Ref (.dim seqLen (.dim hiddenSize .scalar))) from do
          ...
          let h0 ← Gondlin.const (m := m) (α := α) (s := .dim hiddenSize .scalar) h0T
          let out0 ← Gondlin.const (m := m) (α := α) (s := .dim seqLen (.dim hiddenSize .scalar))
            out0T
          let (_, out) ← (List.finRange seqLen).foldlM (init := (h0, out0)) (fun st t => do
            let (hPrev, outPrev) := st
            let x_t ← Gondlin.gatherRow (m := m) (α := α) (rows := seqLen) (cols := inputSize) xs t
            let concat ← Gondlin.concatVectors (m := m) (α := α)
              (nDim := inputSize) (mDim := hiddenSize) x_t hPrev
            let pre ← Gondlin.linear (m := m) (α := α)
              (inDim := inputSize + hiddenSize) (outDim := hiddenSize)
              w b concat
            let h_t ← Gondlin.tanh (m := m) (α := α) (s := .dim hiddenSize .scalar) pre
            let outNext ← Gondlin.scatterAddRow (m := m) (α := α)
    ...
    forward := fun _ {α} _ _ =>
      fun {m} _ _ =>
        fun wReset bReset wUpdate bUpdate wNew bNew xs =>
          ...
          let out0 ← Gondlin.const (m := m) (α := α) (s := .dim seqLen (.dim hiddenSize .scalar))
            out0T
          let ones ← Gondlin.const (m := m) (α := α) (s := .dim hiddenSize .scalar) onesT
          let (_, out) ← (List.finRange seqLen).foldlM (init := (h0, out0)) (fun st t => do
            let (hPrev, outPrev) := st
            let x_t ← Gondlin.gatherRow (m := m) (α := α) (rows := seqLen) (cols := inputSize) xs t
            let concat ← Gondlin.concatVectors (m := m) (α := α)
              (nDim := inputSize) (mDim := hiddenSize) x_t hPrev
            let r_pre ← Gondlin.linear (m := m) (α := α)
              (inDim := inputSize + hiddenSize) (outDim := hiddenSize)
              wReset bReset concat
            let r ← Gondlin.sigmoid (m := m) (α := α) (s := .dim hiddenSize .scalar) r_pre
            let z_pre ← Gondlin.linear (m := m) (α := α)
              (inDim := inputSize + hiddenSize) (outDim := hiddenSize)
              wUpdate bUpdate concat
            let z ← Gondlin.sigmoid (m := m) (α := α) (s := .dim hiddenSize .scalar) z_pre
            let r_hPrev ← Gondlin.mul (m := m) (α := α) (s := .dim hiddenSize .scalar) r hPrev
            let concat2 ← Gondlin.concatVectors (m := m) (α := α)
              (nDim := inputSize) (mDim := hiddenSize) x_t r_hPrev
            let n_pre ← Gondlin.linear (m := m) (α := α)
              (inDim := inputSize + hiddenSize) (outDim := hiddenSize)
              wNew bNew concat2
            let n ← Gondlin.tanh (m := m) (α := α) (s := .dim hiddenSize .scalar) n_pre
            let oneMinusZ ← Gondlin.sub (m := m) (α := α) (s := .dim hiddenSize .scalar) ones z
            let newContrib ← Gondlin.mul (m := m) (α := α) (s := .dim hiddenSize .scalar) oneMinusZ n
            let hiddenContrib ← Gondlin.mul (m := m) (α := α) (s := .dim hiddenSize .scalar) z hPrev
            let h_t ← Gondlin.add (m := m) (α := α) (s := .dim hiddenSize .scalar) newContrib hiddenContrib
            let outNext ← Gondlin.scatterAddRow (m := m) (α := α)
              (rows := seqLen) (cols := hiddenSize) outPrev h_t t
            pure (h_t, outNext))
          pure out
...
The recurrence is unrolled with ordinary Gondlin differentiable ops, so the same definition trains
      ...
      fun {m} _ _ =>
        fun wIn bIn wDelta bDelta wGate bGate xs =>
          ...
          let (_, out) ← (List.finRange seqLen).foldlM (init := (h0, out0)) (fun st t => do
            let (hPrev, outPrev) := st
            let x_t ← Gondlin.gatherRow (m := m) (α := α) (rows := seqLen) (cols := inputSize) xs t
            let uPre ← Gondlin.linear (m := m) (α := α)
            ...
            let concat ← Gondlin.concatVectors (m := m) (α := α)
              (nDim := inputSize) (mDim := hiddenSize) x_t hPrev
            let deltaPre ← Gondlin.linear (m := m) (α := α)
              (inDim := inputSize + hiddenSize) (outDim := hiddenSize)
              wDelta bDelta concat
            let delta ← Gondlin.sigmoid (m := m) (α := α) (s := .dim hiddenSize .scalar) deltaPre
            let oneMinusDelta ← Gondlin.sub (m := m) (α := α) (s := .dim hiddenSize .scalar)
              ones delta
            let keep ← Gondlin.mul (m := m) (α := α) (s := .dim hiddenSize .scalar)
              delta hPrev
            let write ← Gondlin.mul (m := m) (α := α) (s := .dim hiddenSize .scalar)
              oneMinusDelta u
            let h_t ← Gondlin.add (m := m) (α := α) (s := .dim hiddenSize .scalar)
              keep write
            let gatePre ← Gondlin.linear (m := m) (α := α)
            ...
            let y_t ← Gondlin.mul (m := m) (α := α) (s := .dim hiddenSize .scalar)
              h_t gate
            let outNext ← Gondlin.scatterAddRow (m := m) (α := α)
      ...
      fun {m} _ _ =>
        fun wF bF wI bI wC bC wO bO xs =>
          ...
          let out0T : Tensor α (.dim seqLen (.dim hiddenSize .scalar)) :=
            Spec.fill (α := α) (0 : α) (.dim seqLen (.dim hiddenSize .scalar))
          let h0 ← Gondlin.const (m := m) (α := α) (s := .dim hiddenSize .scalar) h0T
          let c0 ← Gondlin.const (m := m) (α := α) (s := .dim hiddenSize .scalar) h0T
          let out0 ← Gondlin.const (m := m) (α := α) (s := .dim seqLen (.dim hiddenSize .scalar))
            out0T
          let (_, _, out) ← (List.finRange seqLen).foldlM (init := (h0, c0, out0)) (fun st t => do
            ...
            let concat ← Gondlin.concatVectors (m := m) (α := α)
              (nDim := inputSize) (mDim := hiddenSize) x_t hPrev
            let f_pre ← Gondlin.linear (m := m) (α := α)
              (inDim := inputSize + hiddenSize) (outDim := hiddenSize)
              wF bF concat
            let f ← Gondlin.sigmoid (m := m) (α := α) (s := .dim hiddenSize .scalar) f_pre
            let i_pre ← Gondlin.linear (m := m) (α := α)
              (inDim := inputSize + hiddenSize) (outDim := hiddenSize)
              wI bI concat
            let i ← Gondlin.sigmoid (m := m) (α := α) (s := .dim hiddenSize .scalar) i_pre
            let g_pre ← Gondlin.linear (m := m) (α := α)
              (inDim := inputSize + hiddenSize) (outDim := hiddenSize)
              wC bC concat
            let g ← Gondlin.tanh (m := m) (α := α) (s := .dim hiddenSize .scalar) g_pre
            let o_pre ← Gondlin.linear (m := m) (α := α)
              (inDim := inputSize + hiddenSize) (outDim := hiddenSize)
              wO bO concat
            let o ← Gondlin.sigmoid (m := m) (α := α) (s := .dim hiddenSize .scalar) o_pre
            let fc ← Gondlin.mul (m := m) (α := α) (s := .dim hiddenSize .scalar) f cPrev
            let ig ← Gondlin.mul (m := m) (α := α) (s := .dim hiddenSize .scalar) i g
            let c_t ← Gondlin.add (m := m) (α := α) (s := .dim hiddenSize .scalar) fc ig
            let tanhC ← Gondlin.tanh (m := m) (α := α) (s := .dim hiddenSize .scalar) c_t
            let h_t ← Gondlin.mul (m := m) (α := α) (s := .dim hiddenSize .scalar) o tanhC
            let outNext ← Gondlin.scatterAddRow (m := m) (α := α)
              (rows := seqLen) (cols := hiddenSize) outPrev h_t t
...
def relu {s : Shape} : LayerDef s s :=
  { paramShapes := []
    ...
    forward := fun _ {α} _ _ =>
      fun {m} _ _ =>
        fun x => Gondlin.relu (m := m) (α := α) (s := s) x
  }
...
def sigmoid {s : Shape} : LayerDef s s :=
  { paramShapes := []
    ...
    forward := fun _ {α} _ _ =>
      fun {m} _ _ =>
        fun x => Gondlin.sigmoid (m := m) (α := α) (s := s) x
  }
...
def tanh {s : Shape} : LayerDef s s :=
  { paramShapes := []
    ...
    forward := fun _ {α} _ _ =>
      fun {m} _ _ =>
        fun x => Gondlin.tanh (m := m) (α := α) (s := s) x
  }
...
def softmax {s : Shape} : LayerDef s s :=
  { paramShapes := []
    ...
    forward := fun _ {α} _ _ =>
      fun {m} _ _ =>
        fun x => Gondlin.softmax (m := m) (α := α) (s := s) x
  }
...
def square {s : Shape} : LayerDef s s :=
  { paramShapes := []
    ...
    forward := fun _ {α} _ _ =>
      fun {m} _ _ =>
        fun x => Gondlin.F.square (m := m) (α := α) (s := s) x
  }
...
def sum {s : Shape} : LayerDef s Shape.scalar :=
  { paramShapes := []
    ...
    forward := fun _ {α} _ _ =>
      fun {m} _ _ =>
        fun x => Gondlin.sum (m := m) (α := α) (s := s) x
  }
...
def flatten {s : Shape} : LayerDef s (.dim (Shape.size s) .scalar) :=
  { paramShapes := []
    ...
    forward := fun _ {α} _ _ =>
      fun {m} _ _ =>
        fun x => Gondlin.flatten (m := m) (α := α) (s := s) x
  }
...
def flattenKeep0 {batch : Nat} {s : Shape} :
  ...
  { paramShapes := []
    ...
    forward := fun _ {α} _ _ =>
      fun {m} _ _ =>
        fun x => Gondlin.flattenKeep0 (m := m) (α := α) (batch := batch) (s := s) x
  }
...
def dropout {s : Shape} (p : Float) (seed : Nat := 0) : LayerDef s s :=
  ...
  { paramShapes := [pShape]
    ...
    forward := fun mode {α} _ _ =>
      fun {m} _ _ =>
        fun pRef x =>
          _root_.Runtime.Autograd.Gondlin.F.dropoutRefSeeded (m := m) (α := α) (s := s) x pRef
            seed
            (training := mode == .train)
  }
...
def rmsNorm
  ...
  { paramShapes := [gammaShape]
    ...
    forward := fun _ {α} _ _ =>
      fun {m} _ _ =>
        fun gamma x =>
          Gondlin.Norm.rmsNormLastBatched (m := m) (α := α)
            (batch := batch) (seqLen := seqLen) (embedDim := embedDim)
            (h_seq_pos := h_seq_pos) (h_embed_pos := h_embed_pos)
            x gamma
  }
...
def batchnormChannelFirst
  ...
  { paramShapes := [gammaShape, betaShape]
    ...
    forward := fun _ {α} _ _ =>
      fun {m} _ _ =>
        fun gamma beta x =>
          Gondlin.batchnormChannelFirst (m := m) (α := α)
    ...
    forward := fun _ {α} _ _ =>
      fun {m} _ _ =>
        fun gamma beta mean var x =>
          Gondlin.Norm.batchNorm2dChwEval (m := m) (α := α)
...
def instanceNorm2dNchw
  ...
  { paramShapes := [gammaShape, betaShape]
    ...
    forward := fun _ {α} _ _ =>
      fun {m} _ _ =>
        fun gamma beta x =>
          Gondlin.Norm.instanceNorm2dNchw (m := m) (α := α)
            (n := n) (c := c) (h := h) (w := w)
...
def groupNorm2dNchw
  ...
  { paramShapes := [gammaShape, betaShape]
    ...
    forward := fun _ {α} _ _ =>
      fun {m} _ _ =>
        fun gamma beta x =>
          Gondlin.Norm.groupNorm2dNchw (m := m) (α := α)
...
def batchNorm2dNchw
  ...
  { paramShapes := [gammaShape, betaShape]
    ...
    forward := fun _ {α} _ _ =>
      fun {m} _ _ =>
        fun gamma beta x =>
          Gondlin.Norm.batchNorm2dNchwTrain (m := m) (α := α)
            (n := n) (c := c) (h := h) (w := w)
            h_n_pos h_c_pos h_h_pos h_w_pos
            x gamma beta
  }
        ...
        fun gamma beta mean var _momentum x =>
          ...
          | .train =>
              Gondlin.Norm.batchNorm2dNchwTrain (m := m) (α := α)
                ...
                x gamma beta
          | .eval =>
              Gondlin.Norm.batchNorm2dNchwEval (m := m) (α := α)
                (n := n) (c := c) (h := h) (w := w)
                h_n_pos h_c_pos h_h_pos h_w_pos
...
def convTranspose2d
  ...
  { paramShapes := [KShape, bShape]
    ...
    forward := fun _ {α} _ _ =>
      fun {m} _ _ =>
        fun k b x =>
          Gondlin.convTranspose2d (m := m) (α := α)
...
This layer packs the four projection matrices `(Wq, Wk, Wv, Wo)` and calls the Gondlin attention
...
PyTorch analogy: `torch.nn.MultiheadAttention(embed_dim=dModel, num_heads=numHeads)` in
  self-attention
mode (shape conventions differ; Gondlin uses explicit `n × dModel` tensors).
-/
def multiHeadAttention
  ...
  { paramShapes := [wProjShape, wProjShape, wProjShape, wOShape]
    ...
    forward := fun _ {α} _ _ =>
      fun {m} _ _ =>
        fun wq wk wv wo x =>
          Gondlin.multiHeadAttention (m := m) (α := α)
...
```lean
Gondlin.NN.seq1 (Gondlin.NN.linear inDim hidDim) >>>
Gondlin.NN.seq1 Gondlin.NN.tanh >>>
Gondlin.NN.seq1 (Gondlin.NN.linear hidDim outDim)
```
...
```lean
tlseq[
  Gondlin.NN.linear inDim hidDim,
  Gondlin.NN.tanh,
  Gondlin.NN.linear hidDim outDim
]
```
...
syntax (name := gondlinSeqLit) "tlseq" "[" term,+ "]" : term

macro_rules
  | `(tlseq[$l]) => `(Gondlin.NN.seq1 $l)
  | `(tlseq[$l, $ls,*]) => `(Gondlin.NN.seq1 $l >>> tlseq[$ls,*])
