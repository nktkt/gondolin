...
Deterministic RNG utilities for Gondlin (seed-threaded, *pure*).
...
whose results depend on hidden runtime state. For Gondlin, that is a poor fit:
...
Today this is used by *Session-level* stochastic layers (e.g. `Gondlin.Session.dropout`), where
...
and then using the seeded RNG from that point onward (`Gondlin.Session.initRngFromIO`).
-/
