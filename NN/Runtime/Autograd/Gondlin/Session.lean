...
Gondlin unified imperative session.

## Mental Model

A `Session α` is Gondlin's runtime analogue of a PyTorch "training loop environment". It:
...
Gondlin exposes a **single API** with two execution backends selected at construction time:
...
Gondlin does *not* store mutable `.grad` fields on each tensor ref; instead, gradients are
  returned
...
## Connection To Gondlin IR / Graph Execution
...
This is the Gondlin analogue of calling `backward()` and then reading `.grad` for every leaf,
...
def applyGradHook {α : Type}
  ...
  for i in List.finRange grads.size do
    ...
    if h : g'.s = g.s then
      ...
      throw <| IO.userError <|
        s!"gondlin: grad hook changed shape at id={i.1} (expected {Shape.pretty g.s}, got "
          ++ s!"{Shape.pretty g'.s})"
  pure out
...
This is the Gondlin analogue of reading `x.grad` (but without mutation).
-/
def grad {α : Type} {sh : Shape} [DecidableEq Shape]
  ...
  let gAny ← match grads[x.id]? with
    | some g => pure g
    | none => throw <| IO.userError "gondlin: gradient array out of bounds"
  if h : gAny.s = sh then
    pure (Tensor.castShape gAny.t h)
  else
    throw <| IO.userError
      s!"gondlin: grad shape mismatch (expected {Shape.pretty sh}, got {Shape.pretty gAny.s})"
...
def jvpLeaf {α : Type} (s : Session α) [Zero α] [DecidableEq Shape]
  ...
  match s.impl with
  | .eager _ =>
      throw <| IO.userError "gondlin: jvpLeaf is only supported for compiled sessions"
  | .compiled sess =>
...
def jvpScalarLeaf {α : Type} (s : Session α) [Zero α] [DecidableEq Shape]
  ...
  match s.impl with
  | .eager _ =>
      throw <| IO.userError "gondlin: jvpScalarLeaf is only supported for compiled sessions"
  | .compiled sess =>
...
def jvpDenseAll {α : Type} (s : Session α) [Zero α] [DecidableEq Shape]
  ...
  match s.impl with
  | .eager _ =>
      throw <| IO.userError "gondlin: jvpDenseAll is only supported for compiled sessions"
  | .compiled sess =>
...
workflows that are lower-level than `API.Gondlin.Trainer`, but should still avoid manual
