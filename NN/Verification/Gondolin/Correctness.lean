...
Gondolin→IR correctness helpers.

This file does **not** (yet) contain a full compiler-correctness theorem for arbitrary
`Gondolin.Program`s (the current embedding is higher-order). It provides the small, reusable
...
def evalCompiledForward1
  ...
  if h : out.shape = outShape then
    ...
    throw <|
      s!"GondolinCorrectness: output shape mismatch: " ++
