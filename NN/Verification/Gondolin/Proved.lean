...
`NN.Verification.Gondolin.Verified`, which re-exports the concise public names.
...
namespace NN.Verification.Gondolin.Proved
...
def getVal {α : Type} [Context α] [DecidableEq Shape]
  ...
  if h : v.shape = s then
    ...
    throw s!"GondolinVerified: expected shape {repr s}, got {repr v.shape}"
...
def evalNode
  ...
  | .log (s := s) x =>
      ...
      let y : Tensor α s :=
        if Tensor.allSpec (α := α) (s := s) (fun v => decide (0 < v)) tx then
          ...
          panic!
            "GondolinVerified: log: input contains values <= 0 (or NaN); use `safe_log` if you want epsilon protection"
  ...
  | .mseLoss (s := _s) yhat target =>
      ...
      if h : yV.shape = tV.shape then
        ...
        throw
          s!"GondolinVerified: mse_loss expects equal shapes, got {repr yV.shape} vs {repr tV.shape}"
...
def evalFGraph
  ...
  | .ret y =>
      ...
      if h : v.shape = out then
        ...
        throw s!"GondolinVerified: expected shape {repr out}, got {repr v.shape}"
...
def compileFGraph
    ...
    (g : FGraph α paramShapes inShape ss out)
    (params : Runtime.Autograd.Torch.TList α paramShapes)
    (c : NN.Verification.Gondolin.CompiledIR α) :
  ...
  | .let1 (ss := ss) (mid := mid) (out := out) node gNext =>
      ...
      let (n, ps') :=
        compileNode (α := α) (paramShapes := paramShapes) (inShape := inShape) (ss := ss) (out :=
          mid)
          id node params c.ps
      let c' : NN.Verification.Gondolin.CompiledIR α :=
...
def compileVerifiedForward1
  ...
  let c0 : NN.Verification.Gondolin.CompiledIR α :=
...
private def evalFGraphVals
  ...
  private theorem evalFGraph_eq_evalFGraphVals_outIdx
      ...
      (do
        ...
        if h : v.shape = out then
          ...
          throw s!"GondolinVerified: expected shape {repr out}, got {repr v.shape}") := by
    classical
  ...
  **Main compiler correctness theorem (verified forward fragment).**
  ...
  have hEvalForward :
      ...
      (do
        ...
        if h : v.shape = outShape then
          ...
          throw s!"GondolinVerified: expected shape {repr outShape}, got {repr v.shape}") := by
  ...
  rw [hEvalForward]
  rw [NN.Verification.Gondolin.evalCompiledForward1, NN.IR.Graph.denote, hOutId]
...
end NN.Verification.Gondolin.Proved
