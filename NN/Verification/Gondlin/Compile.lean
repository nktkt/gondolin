...
Gondlin → verifier bridge.

This module lets you take a Gondlin `Program` (written once over the `Gondlin.Ops` interface)
and compile it into the op-tagged IR (`NN.IR.Graph`) plus a CROWN/LiRPA-style `ParamStore`.

Why this exists:
- Gondlin programs can be executed (eager/compiled) for training, but the same computation graph
...
Scope:
- targets the forward-only Gondlin fragment used by bound-propagation checkers;
...
/--
Reference produced while compiling a Gondlin program.
...
/-- Mutable builder state for translating a Gondlin program into verifier IR. -/
...
/-- Builder monad used by the Gondlin→IR compiler. -/
...
def emitInput {α : Type} [Context α] {s : Shape} : BuildM α (Ref α s) := do
  ...
  if id ≠ 0 then
    ...
    fail (α := α) "Gondlin IR compile: internal error (input node must be id 0)"
...
def getConst {α : Type} [Context α] {s : Shape} (r : Ref α s) : BuildM α (Tensor α s) :=
  ...
  | .const t => pure t
  | .node _ => fail (α := α)
    "Gondlin IR compile: expected a compile-time constant tensor (got a graph node)"
...
instance {α : Type} [Context α] [DecidableEq Shape] [Inhabited α] :
    ...
    | depth', s' =>
        if h : s'.swapAdjacentAtDepth depth' = s' then
          ...
          if depth' + 1 < r then
            ...
            -- it defensive.
            fail (α := α)
              s!"Gondlin→IR: swapAdjacentAtDepth (depth={depth}) invalid for shape {repr s'}"
  ...
  gatherScalarNat := fun {_n} _x _i =>
    fail (α := α) "Gondlin→IR: gather is outside the verifier IR fragment"
  ...
  scatterAddVec := fun {_n} _x _val _i =>
    fail (α := α) "Gondlin→IR: scatter is outside the verifier IR fragment"
    ...
    | 2 =>
        ...
        if hs : sH = sW then
          if hp : pH = pW then
            ...
            fail (α := α) "Gondlin→IR: max_pool: verifier IR requires uniform padding"
        else
          fail (α := α) "Gondlin→IR: max_pool: verifier IR requires uniform stride"
    | _ =>
          fail (α := α) "Gondlin→IR: max_pool: verifier IR accepts d=2"
    ...
    | 2 =>
        ...
        if hs : sH = sW then
          if hp : pH = pW then
            ...
            fail (α := α) "Gondlin→IR: avg_pool: verifier IR requires uniform padding"
        else
          fail (α := α) "Gondlin→IR: avg_pool: verifier IR requires uniform stride"
    | _ =>
        fail (α := α) "Gondlin→IR: avg_pool: only d=2 is supported by the verifier IR right now"

  smoothMaxPool := fun {_d _C} {_inSpatial _kernel _stride _padding} {_hKernel} _x _temp =>
    fail (α := α) "Gondlin→IR: smooth_max_pool is outside the verifier IR fragment"
  ...
  smoothMaxPool2d := fun {_kH _kW _inH _inW _inC _stride} {_h1} {_h2} _x _temp =>
    fail (α := α) "Gondlin→IR: smooth_max_pool2d is outside the verifier IR fragment"
  ...
  batchnormChannelFirst := fun {_channels _height _width} _hC _hH _hW _x _gamma _beta => do
    fail (α := α)
      ("Gondlin→IR: batchnorm_channel_first (training-style BN: stats from " ++
        "x) is outside the verifier IR fragment. For inference-time BN, " ++
        "use Gondlin.Norm.batch_norm2d_chw_eval / batch_norm2d_nchw_eval (or " ++
    ...
    | 2 =>
        ...
        if hs : sH = sW then
          if hp : pH = pW then
            ...
            fail (α := α) "Gondlin→IR: conv: verifier IR requires uniform padding"
        else
          fail (α := α) "Gondlin→IR: conv: verifier IR requires uniform stride"
    | _ =>
          fail (α := α) "Gondlin→IR: conv: verifier IR accepts d=2"

  convTranspose := fun {_d _inC _outC} {_kernel _stride _padding} {_inSpatial} {_hInC} {_hKernel} _w
      _b _x =>
    fail (α := α) "Gondlin→IR: conv_transpose is outside the verifier IR fragment"
  ...
  convTranspose2d := fun {_inC _outC _kH _kW _stride _padding _inH _inW} {_h1} {_h2} {_h3} _kernel _bias
      _input =>
    fail (α := α) "Gondlin→IR: conv_transpose2d is outside the verifier IR fragment"
...
/--
Result of compiling a Gondlin forward model to verifier IR.
...
/-- Compile a Gondlin forward model with a single distinguished input (the last argument). -/
def compileForward1
  ...
  | Except.ok (outId, st) =>
      ...
      | Except.error e => Except.error s!"Gondlin→IR: produced an ill-formed graph: {e}"
      | Except.ok _ =>
