...
public import NN.Runtime.Autograd.Gondolin.ParamIO
public import NN.Runtime.Autograd.Gondolin.Module

/-!
# Gondolin ParamIO (Model Checkpoints)

Gondolin examples often want the same simple workflow:
...
`Runtime.Autograd.Gondolin.ParamIO`.
...
This is enough to checkpoint *any* Gondolin runtime model implemented as a
`Gondolin.Module.ScalarModule` over `Float`, independent of architecture.
...
def saveTListBits
  ...
  _root_.Runtime.Autograd.Gondolin.ParamIO.writeTListBits (ss := paramShapes) path ps pretty

/--
Save the current parameter values of a Gondolin runtime module to a JSON bits file.
...
def saveModuleParamsBits
    ...
    (m : _root_.Runtime.Autograd.Gondolin.Module.ScalarModule Float paramShapes inputShapes)
    (path : System.FilePath) : IO Unit := do
  let ps ← _root_.Runtime.Autograd.Torch.ParamList.valuesSynced (α := Float) (ss := paramShapes)
    m.trainer.params
  _root_.Runtime.Autograd.Gondolin.ParamIO.writeTListBits (ss := paramShapes) path ps
...
def loadModuleParamsBits
    ...
    (m : _root_.Runtime.Autograd.Gondolin.Module.ScalarModule Float paramShapes inputShapes)
    (path : System.FilePath) : IO Unit := do
  let psRes ← _root_.Runtime.Autograd.Gondolin.ParamIO.readTListBits (ss := paramShapes) path
  match psRes with
  | Except.error e =>
...
def readTListBits
  ...
  _root_.Runtime.Autograd.Gondolin.ParamIO.readTListBits (ss := paramShapes) path
