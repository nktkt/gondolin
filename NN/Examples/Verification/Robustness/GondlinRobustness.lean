...
End-to-end robustness certification for a Gondlin model.

We build a compact 2-class classifier in Gondlin, compile it to the verifier IR, and certify a
...
Run:
  ...
  `lake exe verify -- gondlin-robustness --float32-mode ieee754exec`
-/

@[expose] public section


namespace NN.Examples.Verification.Robustness.GondlinRobustness
...
/-- Input dimension for the Gondlin robustness example. -/
def inDim : Nat := 2
/-- Hidden width for the Gondlin robustness example. -/
def hidDim : Nat := 1
...
/-- Parameter shapes list used by the compiled Gondlin program (`[W1,b1,W2,b2]`). -/
...
/-- Gondlin program for a 2-layer ReLU MLP producing two logits. -/
def classifier {α : Type} [Context α] [DecidableEq Shape] :
    Gondlin.Program α (paramShapes ++ [xShape]) yShape :=
  fun {m} _ _ =>
    fun w1 b1 w2 b2 x =>
      (do
        let z1 ← Gondlin.linear (m := m) (α := α) (inDim := inDim) (outDim := hidDim) w1 b1 x
        let h ← Gondlin.relu (m := m) (α := α) (s := b1Shape) z1
        Gondlin.linear (m := m) (α := α) (inDim := hidDim) (outDim := outDim) w2 b2 h
        : m (Gondlin.RefTy (m := m) (α := α) yShape))

/--
Run the robustness check once under a chosen scalar backend `α`.

This compiles the Gondlin program to the verifier IR, then computes output bounds with IBP and an
...
def runOnce {α : Type} [Semantics.Scalar α] [DecidableEq Shape] [ToString α]
  ...
  -- These in-source constants are intentional: this workflow is a compact Gondlin-native
...
/--
CLI entry point for the Gondlin robustness workflow.

This is wired into `lake exe verify -- gondlin-robustness`.
-/
def main (args : List String) : IO Unit :=
  NN.API.Common.mainWithRuntimeDType "Gondlin → IR → IBP/CROWN robustness" args
    (fun {α} _ _ _ _ => runOnce (α := α))

end NN.Examples.Verification.Robustness.GondlinRobustness
