...
End-to-end robustness certification for a Gondolin model.

We build a compact 2-class classifier in Gondolin, compile it to the verifier IR, and certify a
...
Run:
  ...
  `lake exe verify -- gondolin-robustness --float32-mode ieee754exec`
-/

@[expose] public section


namespace NN.Examples.Verification.Robustness.GondolinRobustness
...
/-- Input dimension for the Gondolin robustness example. -/
def inDim : Nat := 2
/-- Hidden width for the Gondolin robustness example. -/
def hidDim : Nat := 1
...
/-- Parameter shapes list used by the compiled Gondolin program (`[W1,b1,W2,b2]`). -/
...
/-- Gondolin program for a 2-layer ReLU MLP producing two logits. -/
def classifier {α : Type} [Context α] [DecidableEq Shape] :
    Gondolin.Program α (paramShapes ++ [xShape]) yShape :=
  fun {m} _ _ =>
    fun w1 b1 w2 b2 x =>
      (do
        let z1 ← Gondolin.linear (m := m) (α := α) (inDim := inDim) (outDim := hidDim) w1 b1 x
        let h ← Gondolin.relu (m := m) (α := α) (s := b1Shape) z1
        Gondolin.linear (m := m) (α := α) (inDim := hidDim) (outDim := outDim) w2 b2 h
        : m (Gondolin.RefTy (m := m) (α := α) yShape))

/--
Run the robustness check once under a chosen scalar backend `α`.

This compiles the Gondolin program to the verifier IR, then computes output bounds with IBP and an
...
def runOnce {α : Type} [Semantics.Scalar α] [DecidableEq Shape] [ToString α]
  ...
  -- These in-source constants are intentional: this workflow is a compact Gondolin-native
...
/--
CLI entry point for the Gondolin robustness workflow.

This is wired into `lake exe verify -- gondolin-robustness`.
-/
def main (args : List String) : IO Unit :=
  NN.API.Common.mainWithRuntimeDType "Gondolin → IR → IBP/CROWN robustness" args
    (fun {α} _ _ _ _ => runOnce (α := α))

end NN.Examples.Verification.Robustness.GondolinRobustness
