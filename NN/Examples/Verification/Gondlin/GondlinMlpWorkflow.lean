...
/-!
# Gondlin MLP training and verification workflow

This is a native Gondlin workflow: the model is built, trained, compiled to verifier IR, and
checked without importing an external certificate.

Training:
- build a 2-layer ReLU MLP with a scalar MSE loss
- train it for a few SGD steps under the compiled Gondlin backend
...
Run:
  ...
  `lake exe verify -- gondlin-mlp-workflow --dtype float32`
-/

@[expose] public section


namespace NN.Examples.Verification.Gondlin.GondlinMlpWorkflow
...
/-- Gondlin model used for training and verification. -/
...
def runOnce {α : Type} [Semantics.Scalar α] [DecidableEq Shape] [ToString α]
    [Runtime.Scalar α] (opts : API.Gondlin.Options) : IO Unit := do
  let cast : Float → α := Runtime.ofFloat
  let dataset := _root_.NN.API.Data.supervisedDim0F (α := α) XFloat YFloat

  IO.println s!"== Gondlin MLP workflow ({inDim} → {hiddenDim} → {outDim}) =="
  ...
  let forward : Gondlin.Program α (paramShapes ++ [xShape]) yShape :=
...
/--
CLI entry point for the native Gondlin MLP workflow.

This is wired into `lake exe verify -- gondlin-mlp-workflow`.
-/
def main (args : List String) : IO Unit := do
  ...
  NN.API.Gondlin.Module.withRuntime args (fun {α} _ _ _ _ _ opts rest => do
    NN.API.Common.orThrow "gondlin-mlp-workflow" <| NN.API.CLI.requireNoArgs rest
    if opts.useGpu then
      throw <| IO.userError
        "gondlin-mlp-workflow: CUDA eager training is not used here; this workflow keeps trained parameters as Lean tensors so the verifier can compile and check them. Use the model-training examples for CUDA runtime training, or run this verifier workflow without --cuda."
    runOnce (α := α) opts)

end NN.Examples.Verification.Gondlin.GondlinMlpWorkflow
