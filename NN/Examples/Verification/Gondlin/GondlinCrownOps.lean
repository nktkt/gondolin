...
Running CROWN end-to-end on small Gondlin graphs.

We compile Gondlin programs to the verifier IR (`NN.IR.Graph`), then run:
- IBP (`runIBP`)
...
For attention + `layer_norm`, see
  `NN/Examples/Verification/Gondlin/GondlinTransformerIBP.lean`.

Run:
  `lake exe verify -- gondlin-crown-ops`
  `lake exe verify -- gondlin-crown-ops --dtype ieee754exec`
-/

@[expose] public section


namespace NN.Examples.Verification.Gondlin.GondlinCrownOps
...
This compiles the Gondlin model to the verifier IR and prints IBP/CROWN bounds.
-/
...
/-- Gondlin program: `yhat = linear(x); mse_loss(yhat, target)` returning a scalar. -/
def mseLossModel {α : Type} [Context α] [DecidableEq Shape] :
    Gondlin.Program α (mseParamShapes ++ [mseXShape]) Shape.scalar :=
  fun {m} _ _ =>
    fun w b target x =>
      (do
        let yhat ← Gondlin.linear (m := m) (α := α) (inDim := mseInDim) (outDim := mseOutDim) w
          b x
        Gondlin.mseLoss (m := m) (α := α) (s := mseYShape) yhat target
        : m (Gondlin.RefTy (m := m) (α := α) Shape.scalar))

/--
Run the MSE-loss workflow under a chosen scalar backend `α`.

This compiles the Gondlin program to the verifier IR and prints IBP/CROWN bounds for the scalar
  loss.
-/
...
This is wired into `lake exe verify -- gondlin-crown-ops`.
-/
def main (args : List String) : IO Unit :=
  NN.API.Common.mainWithRuntimeDType "Gondlin → IR → IBP + CROWN (ops: softmax/mse_loss)" args
    (fun {α} _ _ _ _ => runOnce (α := α))

end NN.Examples.Verification.Gondlin.GondlinCrownOps
