...
Gondlin (MHA + LayerNorm + MSE) → compile to `NN.IR.Graph` → run:
- IBP (`runIBP`)
...
Run:
  ...
  `lake exe verify -- gondlin-transformer-ibp --with-crown`
  `lake exe verify -- gondlin-transformer-ibp --dtype ieee754exec`
-/

@[expose] public section


namespace NN.Examples.Verification.Gondlin.GondlinTransformerIBP
...
/-- Gondlin program: `mha -> layer_norm -> mse_loss`, returning a scalar loss. -/
def modelLoss {α : Type} [Context α] [DecidableEq Shape] :
    Gondlin.Program α (paramShapes ++ [xShape]) Shape.scalar :=
  fun {m} _ _ =>
    fun wq wk wv wo gamma beta target x =>
      (do
        let y ← Gondlin.multiHeadAttention (m := m) (α := α)
          (batch := batch) (n := n) (numHeads := numHeads) (dModel := dModel) (headDim := headDim)
          (h1 := by decide) wq wk wv wo x (mask := none)
        let yLn ← Gondlin.layerNorm (m := m) (α := α)
          (batch := batch) (seqLen := n) (embedDim := dModel) (h_seq_pos := by decide)
          (h_embed_pos := by decide)
          y gamma beta
        Gondlin.mseLoss (m := m) (α := α) (s := xShape) yLn target
        : m (Gondlin.RefTy (m := m) (α := α) Shape.scalar))

/--
CLI entry point for the transformer-IBP workflow.

This is wired into `lake exe verify -- gondlin-transformer-ibp`.

By default this command is a fast validation check: compile the Gondlin transformer fragment to the
verification IR and run IBP on the scalar loss. Pass `--with-crown` to also run the experimental
transformer-scale CROWN passes. The separate `gondlin-crown-ops` command keeps CROWN itself in the
...
def main (args : List String) : IO Unit := do
  ...
  NN.API.Common.runWithRuntimeDType "Gondlin (MHA+LayerNorm+MSE) → IR → IBP" args
    (fun {α} _ _ _ _ => do
...
end NN.Examples.Verification.Gondlin.GondlinTransformerIBP
