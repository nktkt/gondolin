...
Gondolin forward model → compile to `NN.IR.Graph` → run Lean IBP (`runIBP`).

Run:
  `lake exe verify -- gondolin-ibp`
  `lake exe verify -- gondolin-ibp --dtype ieee754exec`
  `lake exe verify -- gondolin-ibp --dtype float32`
-/

@[expose] public section


namespace NN.Examples.Verification.Gondolin.GondolinIBP
...
/-- Gondolin model used in the workflow (a 2-layer ReLU MLP). -/
...
/--
CLI entry point for the Gondolin → IR → IBP workflow.

This is wired into `lake exe verify -- gondolin-ibp`.
-/
def main (args : List String) : IO Unit := do
  NN.API.Common.runWithRuntimeDType "Gondolin → IR → IBP (small MLP)" args
    (fun {α} _ _ _ _ => do
...
end NN.Examples.Verification.Gondolin.GondolinIBP
