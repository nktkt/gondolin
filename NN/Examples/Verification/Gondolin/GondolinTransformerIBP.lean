...
Gondolin (MHA + LayerNorm + MSE) → compile to `NN.IR.Graph` → run:
- IBP (`runIBP`)
...
Run:
  ...
  `lake exe verify -- gondolin-transformer-ibp --with-crown`
  `lake exe verify -- gondolin-transformer-ibp --dtype ieee754exec`
-/

@[expose] public section


namespace NN.Examples.Verification.Gondolin.GondolinTransformerIBP
...
/-- Gondolin program: `mha -> layer_norm -> mse_loss`, returning a scalar loss. -/
def modelLoss {α : Type} [Context α] [DecidableEq Shape] :
    Gondolin.Program α (paramShapes ++ [xShape]) Shape.scalar :=
  fun {m} _ _ =>
    fun wq wk wv wo gamma beta target x =>
      (do
        let y ← Gondolin.multiHeadAttention (m := m) (α := α)
          (batch := batch) (n := n) (numHeads := numHeads) (dModel := dModel) (headDim := headDim)
          (h1 := by decide) wq wk wv wo x (mask := none)
        let yLn ← Gondolin.layerNorm (m := m) (α := α)
          (batch := batch) (seqLen := n) (embedDim := dModel) (h_seq_pos := by decide)
          (h_embed_pos := by decide)
          y gamma beta
        Gondolin.mseLoss (m := m) (α := α) (s := xShape) yLn target
        : m (Gondolin.RefTy (m := m) (α := α) Shape.scalar))

/--
CLI entry point for the transformer-IBP workflow.

This is wired into `lake exe verify -- gondolin-transformer-ibp`.

By default this command is a fast validation check: compile the Gondolin transformer fragment to the
verification IR and run IBP on the scalar loss. Pass `--with-crown` to also run the experimental
transformer-scale CROWN passes. The separate `gondolin-crown-ops` command keeps CROWN itself in the
...
def main (args : List String) : IO Unit := do
  ...
  NN.API.Common.runWithRuntimeDType "Gondolin (MHA+LayerNorm+MSE) → IR → IBP" args
    (fun {α} _ _ _ _ => do
...
end NN.Examples.Verification.Gondolin.GondolinTransformerIBP
