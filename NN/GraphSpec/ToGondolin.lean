...
/-!
# GraphSpec → Gondolin.NN.Seq (sequential lowering)

This provides a *structural lowering* from GraphSpec graphs into Gondolin sequential models.
...
- a compiler to an **executable** Gondolin program (`Compile.torchProgram`) for running/training.

So why do we also lower to `Gondolin.NN.Seq`?

`Gondolin.NN.Seq` is a small “training ergonomics” wrapper around a sequence of `LayerDef`s:
...
- `NN.GraphSpec/README.md` for the high-level “GraphSpec vs Gondolin” motivation.

### Example (informal)

If `g : Graph ps σ τ` is built from primitives that provide `toLayerDefM?`, then:

- `ToGondolin.toSeq g : Except String (Gondolin.NN.Seq σ τ)` gives a sequential model with
  deterministic initialization, and
- `Compile.torchProgram g : Gondolin.Program α (ps ++ [σ]) τ` gives the general executable program
...
/--
Public alias for Gondolin's sequential model type used as the lowering target.

This keeps GraphSpec examples from exposing the lower-level
`Runtime.Autograd.Gondolin.NN.Seq` path directly. It is definitionally the same type as
`NN.API.nn.Sequential`, but avoids importing the full API facade back into GraphSpec.
-/
abbrev Sequential (σ τ : Shape) : Type 2 :=
  _root_.Runtime.Autograd.Gondolin.NN.Seq σ τ

/--
Lower a graph to a Gondolin `Seq`, threading a “layer occurrence index”.
...
def toSeqAux
  ...
  | .id s =>
      ...
      return (_root_.Runtime.Autograd.Gondolin.NN.Seq.id s, i)
  | .seq g₁ g₂ =>
      -- Sequential composition becomes sequential composition.
      let (s₁, i') ← toSeqAux (ps := _) (σ := _) (τ := _) g₁ i
      let (s₂, i'') ← toSeqAux (ps := _) (σ := _) (τ := _) g₂ i'
      return (_root_.Runtime.Autograd.Gondolin.NN.Seq.comp s₁ s₂, i'')
  | .prim p =>
      ...
      | some mk =>
          ...
          let ⟨l, _hps⟩ := mk i
          return (_root_.Runtime.Autograd.Gondolin.NN.seq1 l, i')

/--
Try to lower a sequential GraphSpec graph into a `Gondolin.NN.Seq`.
