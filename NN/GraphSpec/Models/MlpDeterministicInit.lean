...
than “all zeros”. GraphSpec supports that by letting primitives optionally provide a Gondolin
...
parameter list you would get by initializing the two Gondolin `Linear` layers directly with the
...
runtime layer order used by Gondolin.
-/
...
/--
Deterministic init for `Models.mlp` is exactly the concatenation of the two Gondolin `Linear`
initializers.
...
-/
theorem mlp_detInitParams_eq_gondolin_linear_inits
    ...
    .ok
      (Runtime.Autograd.Torch.Proofs.Autograd.Algebra.TList.append (α := Float)
        ...
        (Runtime.Autograd.Gondolin.NN.linear inDim hidDim (seedW := 0) (seedB := 1)).initParams
        (Runtime.Autograd.Gondolin.NN.linear hidDim outDim (seedW := 2) (seedB := 3)).initParams)
          := by
  ...
  simp [Runtime.Autograd.Torch.Proofs.Autograd.Algebra.TList.append,
    Runtime.Autograd.Gondolin.NN.relu]
