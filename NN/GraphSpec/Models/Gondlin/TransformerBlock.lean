...
/-!
# Gondlin-executable model: compact Transformer block
...
def transformerBlock
    ...
    _root_.Runtime.Autograd.Gondlin.NN.Seq (.dim batch (.dim n (.dim dModel .scalar)))
      (.dim batch (.dim n (.dim dModel .scalar))) :=
  tlseq[
    _root_.Runtime.Autograd.Gondlin.NN.multiHeadAttention
      (batch := batch) (n := n) (dModel := dModel) (numHeads := numHeads) (headDim := headDim)
      (h1 := h_n) (seedW := seedW),
    _root_.Runtime.Autograd.Gondlin.NN.layerNorm
