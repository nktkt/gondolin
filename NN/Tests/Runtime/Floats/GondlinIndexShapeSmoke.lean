...
/-!
# GondlinIndexShapeSmoke

 Smoke tests for Gondlin indexing and shape helpers over floats. -/
...
namespace Tests
namespace Floats
namespace GondlinIndexShapeSmoke

instance : Fact (3 > 0) where
  out := by decide

def gradGatherVec (backend : Gondlin.Backend) : IO (Tensor Float (.dim 3 .scalar)) := do
  let sess ← Gondlin.Session.new (α := Float) (opts := { backend := backend })
  let xVal : Tensor Float (.dim 3 .scalar) := NN.Tensor.tensor1d (α := Float) [1.0, 2.0, 3.0]
  let x ← Gondlin.Session.input sess xVal (name := some "x") (requiresGrad := true)
  let y ← Gondlin.Session.gatherScalar sess (n := 3) x ⟨1, by decide⟩
  let grads ← Gondlin.Session.backwardScalarDenseAll sess y
  Gondlin.Session.grad grads x

def gradGatherScalarNat (backend : Gondlin.Backend) : IO (Tensor Float (.dim 3 .scalar)) := do
  let sess ← Gondlin.Session.new (α := Float) (opts := { backend := backend })
  let xVal : Tensor Float (.dim 3 .scalar) := NN.Tensor.tensor1d (α := Float) [1.0, 2.0, 3.0]
  let x ← Gondlin.Session.input sess xVal (name := some "x") (requiresGrad := true)
  let y ← Gondlin.Session.gatherScalarNat sess (n := 3) x 1
  let grads ← Gondlin.Session.backwardScalarDenseAll sess y
  Gondlin.Session.grad grads x

def gradGatherVecNat (backend : Gondlin.Backend) : IO (Tensor Float (.dim 3 .scalar)) := do
  let sess ← Gondlin.Session.new (α := Float) (opts := { backend := backend })
  let xVal : Tensor Float (.dim 3 .scalar) := NN.Tensor.tensor1d (α := Float) [1.0, 2.0, 3.0]
  let idx : Tensor Nat (.dim 4 .scalar) := NN.Tensor.tensor1d (α := Nat) [2, 0, 2, 10]
  let x ← Gondlin.Session.input sess xVal (name := some "x") (requiresGrad := true)
  let y ← Gondlin.Session.gatherVecNat sess (n := 3) (k := 4) x idx
  let total ← Gondlin.Session.sum sess (sh := .dim 4 .scalar) y
  let grads ← Gondlin.Session.backwardScalarDenseAll sess total
  Gondlin.Session.grad grads x

def gradGatherRowsNat (backend : Gondlin.Backend) : IO (Tensor Float (.dim 3 (.dim 2 .scalar))) :=
  do
  let sess ← Gondlin.Session.new (α := Float) (opts := { backend := backend })
  ...
  let x ← Gondlin.Session.input sess xVal (name := some "x") (requiresGrad := true)
  let y ← Gondlin.Session.gatherRowsNat sess (rows := 3) (cols := 2) (k := 3) x idx
  let total ← Gondlin.Session.sum sess (sh := .dim 3 (.dim 2 .scalar)) y
  let grads ← Gondlin.Session.backwardScalarDenseAll sess total
  Gondlin.Session.grad grads x

def gradBroadcastScalar (backend : Gondlin.Backend) : IO Float := do
  let sess ← Gondlin.Session.new (α := Float) (opts := { backend := backend })
  let sVal : Tensor Float Shape.scalar := Tensor.scalar 2.0
  let sRef ← Gondlin.Session.input sess sVal (name := some "s") (requiresGrad := true)
  let cb : Shape.CanBroadcastTo Shape.scalar (.dim 4 .scalar) :=
    Shape.CanBroadcastTo.scalar_to_any (.dim 4 .scalar)
  let v ← Gondlin.Session.broadcastTo sess (sh1 := Shape.scalar) (sh2 := .dim 4 .scalar) cb sRef
  let total ← Gondlin.Session.sum sess (sh := .dim 4 .scalar) v
  let grads ← Gondlin.Session.backwardScalarDenseAll sess total
  let dsT ← Gondlin.Session.grad grads sRef
  pure (scalarVal dsT)

def gradReshapeMat (backend : Gondlin.Backend) : IO (Tensor Float (.dim 2 (.dim 3 .scalar))) := do
  let sess ← Gondlin.Session.new (α := Float) (opts := { backend := backend })
  ...
  let y ← Gondlin.Session.reshape sess (sh1 := .dim 2 (.dim 3 .scalar)) (sh2 := .dim 6 .scalar) x
    h
  let total ← Gondlin.Session.sum sess (sh := .dim 6 .scalar) y
  let grads ← Gondlin.Session.backwardScalarDenseAll sess total
  Gondlin.Session.grad grads x

def gradTransposeMat (backend : Gondlin.Backend) : IO (Tensor Float (.dim 2 (.dim 3 .scalar))) :=
  do
  let sess ← Gondlin.Session.new (α := Float) (opts := { backend := backend })
  let xVal : Tensor Float (.dim 2 (.dim 3 .scalar)) :=
    Tensor.dim (fun i => Tensor.dim (fun j => Tensor.scalar (Float.ofNat (i.val + j.val + 1))))
  let x ← Gondlin.Session.input sess xVal (name := some "x") (requiresGrad := true)
  let xt ← Gondlin.Session.transpose2d sess (m := 2) (n := 3) x
  let total ← Gondlin.Session.sum sess (sh := .dim 3 (.dim 2 .scalar)) xt
  let grads ← Gondlin.Session.backwardScalarDenseAll sess total
  Gondlin.Session.grad grads x

def gradReduceMeanVec (backend : Gondlin.Backend) : IO (Tensor Float (.dim 3 .scalar)) := do
  let sess ← Gondlin.Session.new (α := Float) (opts := { backend := backend })
  let xVal : Tensor Float (.dim 3 .scalar) := NN.Tensor.tensor1d (α := Float) [1.0, 2.0, 3.0]
  let x ← Gondlin.Session.input sess xVal (name := some "x") (requiresGrad := true)
  let m ← Gondlin.Session.reduceMean sess (sh := .dim 3 .scalar) 0 x
  let grads ← Gondlin.Session.backwardScalarDenseAll sess m
  Gondlin.Session.grad grads x

def gradScatterAddVec (backend : Gondlin.Backend) : IO (Tensor Float (.dim 3 .scalar) × Float) :=
  do
  let sess ← Gondlin.Session.new (α := Float) (opts := { backend := backend })
  let xVal : Tensor Float (.dim 3 .scalar) := NN.Tensor.tensor1d (α := Float) [1.0, 2.0, 3.0]
  let vVal : Tensor Float Shape.scalar := Tensor.scalar 5.0
  let x ← Gondlin.Session.input sess xVal (name := some "x") (requiresGrad := true)
  let v ← Gondlin.Session.input sess vVal (name := some "v") (requiresGrad := true)
  let y ← Gondlin.Session.scatterAddVec sess (n := 3) x v ⟨2, by decide⟩
  let total ← Gondlin.Session.sum sess (sh := .dim 3 .scalar) y
  let grads ← Gondlin.Session.backwardScalarDenseAll sess total
  let dx ← Gondlin.Session.grad grads x
  let dvT ← Gondlin.Session.grad grads v
  pure (dx, scalarVal dvT)

def run : IO Unit := do
  IO.println "gondlin_index_shape_smoke: begin"
  ...
  IO.println "gondlin_index_shape_smoke: ok"

end GondlinIndexShapeSmoke
end Floats
end Tests
/-!
