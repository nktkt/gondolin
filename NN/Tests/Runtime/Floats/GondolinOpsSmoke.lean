...
# GondolinOpsSmoke
 ...
 Smoke tests for Gondolin operator wrappers over the float runtime. -/
...
namespace Tests
namespace Floats
namespace GondolinOpsSmoke

def evalMatmul (backend : Gondolin.Backend) : IO (Tensor Float (.dim 2 (.dim 2 .scalar))) := do
  ...
  let aR ← Gondolin.Session.const sess (sh := .dim 2 (.dim 3 .scalar)) a
  let bR ← Gondolin.Session.const sess (sh := .dim 3 (.dim 2 .scalar)) b
  let cR ← Gondolin.Session.matmul sess (m := 2) (n := 3) (p := 2) aR bR
  Gondolin.Session.getValue sess (sh := .dim 2 (.dim 2 .scalar)) cR

def evalConcat (backend : Gondolin.Backend) : IO (Tensor Float (.dim 5 .scalar)) := do
  let sess ← Gondolin.Session.new (α := Float) (opts := { backend := backend })
  ...
  let b : Tensor Float (.dim 3 .scalar) := Tensor.dim (fun i => Tensor.scalar (10.0 + Float.ofNat
    i.val))
  let aR ← Gondolin.Session.const sess (sh := .dim 2 .scalar) a
  let bR ← Gondolin.Session.const sess (sh := .dim 3 .scalar) b
  let cR ← Gondolin.Session.concatVectors sess (n := 2) (m := 3) aR bR
  Gondolin.Session.getValue sess (sh := .dim 5 .scalar) cR

def evalMaxPool (backend : Gondolin.Backend) : IO (Tensor Float (.dim 1 (.dim 2 (.dim 2 .scalar))))
  := do
  let sess ← Gondolin.Session.new (α := Float) (opts := { backend := backend })
  ...
  let xR ← Gondolin.Session.const sess (sh := .dim 1 (.dim 4 (.dim 4 .scalar))) x
  let yR ← Gondolin.Session.maxPool2d sess (kH := 2) (kW := 2) (inH := 4) (inW := 4) (inC := 1)
    (stride := 2)
    (h1 := by decide) (h2 := by decide) xR
  Gondolin.Session.getValue sess (sh := .dim 1 (.dim 2 (.dim 2 .scalar))) yR

def evalAvgPool (backend : Gondolin.Backend) : IO (Tensor Float (.dim 1 (.dim 2 (.dim 2 .scalar))))
  := do
  let sess ← Gondolin.Session.new (α := Float) (opts := { backend := backend })
  let x : Tensor Float (.dim 1 (.dim 4 (.dim 4 .scalar))) :=
    Tensor.dim (fun _c =>
      Tensor.dim (fun i =>
        Tensor.dim (fun j =>
          Tensor.scalar (Float.ofNat (i.val * 10 + j.val)))))
  let xR ← Gondolin.Session.const sess (sh := .dim 1 (.dim 4 (.dim 4 .scalar))) x
  let yR ← Gondolin.Session.avgPool2d sess (kH := 2) (kW := 2) (inH := 4) (inW := 4) (inC := 1)
    (stride := 2)
    (by decide) (by decide) xR
  Gondolin.Session.getValue sess (sh := .dim 1 (.dim 2 (.dim 2 .scalar))) yR

def run : IO Unit := do
  IO.println "gondolin_ops_smoke: begin"
  ...
  IO.println "gondolin_ops_smoke: ok"

end GondolinOpsSmoke
end Floats
end Tests
/-!
Gondolin op-surface smoke tests (floats).
