...
# GondlinOpsSmoke
 ...
 Smoke tests for Gondlin operator wrappers over the float runtime. -/
...
namespace Tests
namespace Floats
namespace GondlinOpsSmoke

def evalMatmul (backend : Gondlin.Backend) : IO (Tensor Float (.dim 2 (.dim 2 .scalar))) := do
  ...
  let aR ← Gondlin.Session.const sess (sh := .dim 2 (.dim 3 .scalar)) a
  let bR ← Gondlin.Session.const sess (sh := .dim 3 (.dim 2 .scalar)) b
  let cR ← Gondlin.Session.matmul sess (m := 2) (n := 3) (p := 2) aR bR
  Gondlin.Session.getValue sess (sh := .dim 2 (.dim 2 .scalar)) cR

def evalConcat (backend : Gondlin.Backend) : IO (Tensor Float (.dim 5 .scalar)) := do
  let sess ← Gondlin.Session.new (α := Float) (opts := { backend := backend })
  ...
  let b : Tensor Float (.dim 3 .scalar) := Tensor.dim (fun i => Tensor.scalar (10.0 + Float.ofNat
    i.val))
  let aR ← Gondlin.Session.const sess (sh := .dim 2 .scalar) a
  let bR ← Gondlin.Session.const sess (sh := .dim 3 .scalar) b
  let cR ← Gondlin.Session.concatVectors sess (n := 2) (m := 3) aR bR
  Gondlin.Session.getValue sess (sh := .dim 5 .scalar) cR

def evalMaxPool (backend : Gondlin.Backend) : IO (Tensor Float (.dim 1 (.dim 2 (.dim 2 .scalar))))
  := do
  let sess ← Gondlin.Session.new (α := Float) (opts := { backend := backend })
  ...
  let xR ← Gondlin.Session.const sess (sh := .dim 1 (.dim 4 (.dim 4 .scalar))) x
  let yR ← Gondlin.Session.maxPool2d sess (kH := 2) (kW := 2) (inH := 4) (inW := 4) (inC := 1)
    (stride := 2)
    (h1 := by decide) (h2 := by decide) xR
  Gondlin.Session.getValue sess (sh := .dim 1 (.dim 2 (.dim 2 .scalar))) yR

def evalAvgPool (backend : Gondlin.Backend) : IO (Tensor Float (.dim 1 (.dim 2 (.dim 2 .scalar))))
  := do
  let sess ← Gondlin.Session.new (α := Float) (opts := { backend := backend })
  let x : Tensor Float (.dim 1 (.dim 4 (.dim 4 .scalar))) :=
    Tensor.dim (fun _c =>
      Tensor.dim (fun i =>
        Tensor.dim (fun j =>
          Tensor.scalar (Float.ofNat (i.val * 10 + j.val)))))
  let xR ← Gondlin.Session.const sess (sh := .dim 1 (.dim 4 (.dim 4 .scalar))) x
  let yR ← Gondlin.Session.avgPool2d sess (kH := 2) (kW := 2) (inH := 4) (inW := 4) (inC := 1)
    (stride := 2)
    (by decide) (by decide) xR
  Gondlin.Session.getValue sess (sh := .dim 1 (.dim 2 (.dim 2 .scalar))) yR

def run : IO Unit := do
  IO.println "gondlin_ops_smoke: begin"
  ...
  IO.println "gondlin_ops_smoke: ok"

end GondlinOpsSmoke
end Floats
end Tests
/-!
Gondlin op-surface smoke tests (floats).
