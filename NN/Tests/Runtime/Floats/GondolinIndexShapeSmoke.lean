...
/-!
# GondolinIndexShapeSmoke

 Smoke tests for Gondolin indexing and shape helpers over floats. -/
...
namespace Tests
namespace Floats
namespace GondolinIndexShapeSmoke

instance : Fact (3 > 0) where
  out := by decide

def gradGatherVec (backend : Gondolin.Backend) : IO (Tensor Float (.dim 3 .scalar)) := do
  let sess ← Gondolin.Session.new (α := Float) (opts := { backend := backend })
  let xVal : Tensor Float (.dim 3 .scalar) := NN.Tensor.tensor1d (α := Float) [1.0, 2.0, 3.0]
  let x ← Gondolin.Session.input sess xVal (name := some "x") (requiresGrad := true)
  let y ← Gondolin.Session.gatherScalar sess (n := 3) x ⟨1, by decide⟩
  let grads ← Gondolin.Session.backwardScalarDenseAll sess y
  Gondolin.Session.grad grads x

def gradGatherScalarNat (backend : Gondolin.Backend) : IO (Tensor Float (.dim 3 .scalar)) := do
  let sess ← Gondolin.Session.new (α := Float) (opts := { backend := backend })
  let xVal : Tensor Float (.dim 3 .scalar) := NN.Tensor.tensor1d (α := Float) [1.0, 2.0, 3.0]
  let x ← Gondolin.Session.input sess xVal (name := some "x") (requiresGrad := true)
  let y ← Gondolin.Session.gatherScalarNat sess (n := 3) x 1
  let grads ← Gondolin.Session.backwardScalarDenseAll sess y
  Gondolin.Session.grad grads x

def gradGatherVecNat (backend : Gondolin.Backend) : IO (Tensor Float (.dim 3 .scalar)) := do
  let sess ← Gondolin.Session.new (α := Float) (opts := { backend := backend })
  let xVal : Tensor Float (.dim 3 .scalar) := NN.Tensor.tensor1d (α := Float) [1.0, 2.0, 3.0]
  let idx : Tensor Nat (.dim 4 .scalar) := NN.Tensor.tensor1d (α := Nat) [2, 0, 2, 10]
  let x ← Gondolin.Session.input sess xVal (name := some "x") (requiresGrad := true)
  let y ← Gondolin.Session.gatherVecNat sess (n := 3) (k := 4) x idx
  let total ← Gondolin.Session.sum sess (sh := .dim 4 .scalar) y
  let grads ← Gondolin.Session.backwardScalarDenseAll sess total
  Gondolin.Session.grad grads x

def gradGatherRowsNat (backend : Gondolin.Backend) : IO (Tensor Float (.dim 3 (.dim 2 .scalar))) :=
  do
  let sess ← Gondolin.Session.new (α := Float) (opts := { backend := backend })
  ...
  let x ← Gondolin.Session.input sess xVal (name := some "x") (requiresGrad := true)
  let y ← Gondolin.Session.gatherRowsNat sess (rows := 3) (cols := 2) (k := 3) x idx
  let total ← Gondolin.Session.sum sess (sh := .dim 3 (.dim 2 .scalar)) y
  let grads ← Gondolin.Session.backwardScalarDenseAll sess total
  Gondolin.Session.grad grads x

def gradBroadcastScalar (backend : Gondolin.Backend) : IO Float := do
  let sess ← Gondolin.Session.new (α := Float) (opts := { backend := backend })
  let sVal : Tensor Float Shape.scalar := Tensor.scalar 2.0
  let sRef ← Gondolin.Session.input sess sVal (name := some "s") (requiresGrad := true)
  let cb : Shape.CanBroadcastTo Shape.scalar (.dim 4 .scalar) :=
    Shape.CanBroadcastTo.scalar_to_any (.dim 4 .scalar)
  let v ← Gondolin.Session.broadcastTo sess (sh1 := Shape.scalar) (sh2 := .dim 4 .scalar) cb sRef
  let total ← Gondolin.Session.sum sess (sh := .dim 4 .scalar) v
  let grads ← Gondolin.Session.backwardScalarDenseAll sess total
  let dsT ← Gondolin.Session.grad grads sRef
  pure (scalarVal dsT)

def gradReshapeMat (backend : Gondolin.Backend) : IO (Tensor Float (.dim 2 (.dim 3 .scalar))) := do
  let sess ← Gondolin.Session.new (α := Float) (opts := { backend := backend })
  ...
  let y ← Gondolin.Session.reshape sess (sh1 := .dim 2 (.dim 3 .scalar)) (sh2 := .dim 6 .scalar) x
    h
  let total ← Gondolin.Session.sum sess (sh := .dim 6 .scalar) y
  let grads ← Gondolin.Session.backwardScalarDenseAll sess total
  Gondolin.Session.grad grads x

def gradTransposeMat (backend : Gondolin.Backend) : IO (Tensor Float (.dim 2 (.dim 3 .scalar))) :=
  do
  let sess ← Gondolin.Session.new (α := Float) (opts := { backend := backend })
  let xVal : Tensor Float (.dim 2 (.dim 3 .scalar)) :=
    Tensor.dim (fun i => Tensor.dim (fun j => Tensor.scalar (Float.ofNat (i.val + j.val + 1))))
  let x ← Gondolin.Session.input sess xVal (name := some "x") (requiresGrad := true)
  let xt ← Gondolin.Session.transpose2d sess (m := 2) (n := 3) x
  let total ← Gondolin.Session.sum sess (sh := .dim 3 (.dim 2 .scalar)) xt
  let grads ← Gondolin.Session.backwardScalarDenseAll sess total
  Gondolin.Session.grad grads x

def gradReduceMeanVec (backend : Gondolin.Backend) : IO (Tensor Float (.dim 3 .scalar)) := do
  let sess ← Gondolin.Session.new (α := Float) (opts := { backend := backend })
  let xVal : Tensor Float (.dim 3 .scalar) := NN.Tensor.tensor1d (α := Float) [1.0, 2.0, 3.0]
  let x ← Gondolin.Session.input sess xVal (name := some "x") (requiresGrad := true)
  let m ← Gondolin.Session.reduceMean sess (sh := .dim 3 .scalar) 0 x
  let grads ← Gondolin.Session.backwardScalarDenseAll sess m
  Gondolin.Session.grad grads x

def gradScatterAddVec (backend : Gondolin.Backend) : IO (Tensor Float (.dim 3 .scalar) × Float) :=
  do
  let sess ← Gondolin.Session.new (α := Float) (opts := { backend := backend })
  let xVal : Tensor Float (.dim 3 .scalar) := NN.Tensor.tensor1d (α := Float) [1.0, 2.0, 3.0]
  let vVal : Tensor Float Shape.scalar := Tensor.scalar 5.0
  let x ← Gondolin.Session.input sess xVal (name := some "x") (requiresGrad := true)
  let v ← Gondolin.Session.input sess vVal (name := some "v") (requiresGrad := true)
  let y ← Gondolin.Session.scatterAddVec sess (n := 3) x v ⟨2, by decide⟩
  let total ← Gondolin.Session.sum sess (sh := .dim 3 .scalar) y
  let grads ← Gondolin.Session.backwardScalarDenseAll sess total
  let dx ← Gondolin.Session.grad grads x
  let dvT ← Gondolin.Session.grad grads v
  pure (dx, scalarVal dvT)

def run : IO Unit := do
  IO.println "gondolin_index_shape_smoke: begin"
  ...
  IO.println "gondolin_index_shape_smoke: ok"

end GondolinIndexShapeSmoke
end Floats
end Tests
/-!
