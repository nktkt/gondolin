...
# RnnGruLstmBpttSmoke
 ...
 Smoke tests for RNN, GRU, and LSTM BPTT specifications over floats. -/
...
def run : IO Unit := do
  -- RNN BPTT smoke: gradients are nonzero for a nontrivial sequence.
  let seqLen := 2
  let inputSize := 2
  let hiddenSize := 2
  ...
  IO.println "bptt_smoke: OK"

end BPTT
end Floats
end Tests
/-!
RNN/GRU/LSTM BPTT smoke tests (floats).
