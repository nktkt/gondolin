...
def run : IO Unit := do
  ...
  -- Sanity: a huge binary64 value should be rejected by the checked Float→float32 cast.
  ...
  -- Boundary-contract sanity: validate a small discrete-action transition.
