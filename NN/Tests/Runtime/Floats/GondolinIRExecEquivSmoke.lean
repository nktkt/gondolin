...
/-!
# GondolinIRExecEquivSmoke

Smoke test: IR denotation agrees with the executable `IRExec` bridge.

We compile a small Gondolin model to `NN.IR.Graph` (plus payload), then compile that IR to an
...
namespace Tests
namespace Floats
namespace GondolinIRExecEquivSmoke

def run : IO Unit := do
  IO.println "gondolin_ir_exec_equiv_smoke: begin"
  ...
  -- A small deterministic Gondolin MLP (weights initialized by explicit seeds).
  let model :=
    NN.GraphSpec.Models.Gondolin.mlp
      (inDim := inDim) (hidDim := hidDim) (outDim := outDim)
      (seedW1 := 0) (seedB1 := 1) (seedW2 := 2) (seedB2 := 3)

  let paramShapes := Runtime.Autograd.Gondolin.NN.Seq.paramShapes model
  let params : Runtime.Autograd.Torch.TList Float paramShapes :=
    Runtime.Autograd.Gondolin.NN.Seq.initParams (m := model)

  -- One input vector.
  let x : Tensor Float xShape :=
    Tensor.dim (fun i => Tensor.scalar ([0.5, 0.8][i.val]!))

  -- Gondolin program for the model.
  let prog :
      Runtime.Autograd.Gondolin.Program Float (paramShapes ++ [xShape]) yShape :=
    Runtime.Autograd.Gondolin.NN.Seq.program (model := model) (α := Float)

  -- Compile to IR and executable `ExecGraphData`.
  let (c, exec) ←
    match NN.Verification.Gondolin.compileForward1Exec
        (α := Float) (paramShapes := paramShapes) (inShape := xShape) (outShape := yShape) prog
          params with
    | .error e => throw <| IO.userError s!"gondolin_ir_exec_equiv_smoke: compile failed: {e}"
    | .ok r => pure r

  let payload : NN.IR.Payload Float :=
    NN.Verification.Gondolin.payloadOfParamStore (α := Float) c.ps
  ...
  let xExec : Tensor Float exec.inShape ←
    if hIn : exec.inShape = xShape then
      ...
      let msg :=
        s!"gondolin_ir_exec_equiv_smoke: exec input shape mismatch: got {repr exec.inShape}" ++
  ...
  let yIR : Tensor Float yShape ←
    ...
    | .error e => throw <| IO.userError s!"gondolin_ir_exec_equiv_smoke: IR denote failed: {e}"
    | .ok out =>
        match NN.IR.Graph.expectShape (α := Float) (expected := yShape) out with
        | .ok t => pure t
        | .error e =>
            throw <| IO.userError s!"gondolin_ir_exec_equiv_smoke: IR output shape mismatch: {e}"
  ...
  let yExec : Tensor Float yShape ←
    ...
    | none =>
        let msg :=
          "gondolin_ir_exec_equiv_smoke: exec outputId out of bounds: " ++
            s!"{c.outputId}"
        throw <| IO.userError msg
    | some out =>
        match NN.IR.Graph.expectShape (α := Float) (expected := yShape) out with
        | .ok t => pure t
        | .error e =>
            throw <| IO.userError s!"gondolin_ir_exec_equiv_smoke: exec output shape mismatch: {e}"

  for i in List.finRange outDim do
    assertApprox s!"ir/exec forward[{i.val}]" (vecVal yIR i) (vecVal yExec i) 1e-6

  IO.println "gondolin_ir_exec_equiv_smoke: ok"

end GondolinIRExecEquivSmoke
end Floats
end Tests
