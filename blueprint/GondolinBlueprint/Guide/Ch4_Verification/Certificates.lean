...
Gondolin does not vendor the Two-Stage / α,β-CROWN repository. The core Lean build does not
...
certificate* for each terminal subdomain. Gondolin can parse that JSON and validate several
...
file to Gondolin's checker.

# How To Check In Lean

Use the unified `verify` CLI tool `abcrown-leaf` to check a JSON certificate against Gondolin's
semantics.
...
End-to-end Python plus Lean post-check: *Two-Stage Workflows*. What Gondolin proves vs imports:
*Verification*.
