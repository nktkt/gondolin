...
A Gondolin training run may call SGD or Adam, but a convergence theorem cannot come from the name
...
Gondolin can also state the mathematical contract that would justify calling the step contractive
or convergent.
...
The [FirstOrder optimization API](https://github.com/nktkt/gondolin/blob/main/NN/MLTheory/Optimization/FirstOrder.lean) contains
...
[low rank optimization API](https://github.com/nktkt/gondolin/blob/main/NN/MLTheory/Optimization/LowRank.lean) records equations such as
...
The [linear convergence API](https://github.com/nktkt/gondolin/blob/main/NN/MLTheory/Optimization/GDLinearConvergence.lean) names the
...
[strongly convex gradient descent API](https://github.com/nktkt/gondolin/blob/main/NN/MLTheory/Optimization/StronglyConvexGD.lean) then
...
strong monotonicity of an abstract gradient map. Gondolin keeps both vocabularies and proves the
bridge between them.
...
The [smooth strong convex bridge API](https://github.com/nktkt/gondolin/blob/main/NN/MLTheory/Optimization/SmoothStrongConvexBridge.lean)
...
Gondolin formalizes selected first order optimization facts and their assumptions, so future
