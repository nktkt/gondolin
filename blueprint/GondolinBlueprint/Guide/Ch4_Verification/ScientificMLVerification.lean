...
Gondolin also has verification bridges for scientific ML artifacts: ODE enclosures, PINN residual
...
enclosure. Gondolin makes all of these look like the same proof pattern: a producer proposes a
...
[ODE checker API](https://github.com/nktkt/gondolin/blob/main/NN/Verification/ODE/Verify.lean). The core pieces are the expression AST,
...
[ODE enclosure API](https://github.com/nktkt/gondolin/blob/main/NN/Proofs/Verification/ODE/Enclosure.lean), a corridor theorem says, in
plain language:
...
[ODE enclosure backends](https://github.com/nktkt/gondolin/blob/main/NN/Proofs/Verification/ODE/EnclosureBackends.lean) explains how
...
Gondolin gives each piece a small object. The
[PINN architecture API](https://github.com/nktkt/gondolin/blob/main/NN/Verification/PINN/Architecture.lean) names sequential network
records and graph construction. The
[PDE expression API](https://github.com/nktkt/gondolin/blob/main/NN/Verification/PINN/PdeAst.lean) names the PDE language. The
[PyTorch parameter store API](https://github.com/nktkt/gondolin/blob/main/NN/Verification/PINN/PyTorch/ParamStore.lean) names imported
parameters instead of letting a raw tensor dictionary float around unchecked. The
[residual affine API](https://github.com/nktkt/gondolin/blob/main/NN/Verification/PINN/ResidualAffine.lean) contains the bound helpers,
...
The [PINN certificate API](https://github.com/nktkt/gondolin/blob/main/NN/Verification/PINN/Certificate.lean), the
[dataset checker API](https://github.com/nktkt/gondolin/blob/main/NN/Verification/PINN/DatasetCheck.lean), and the
[PINN command API](https://github.com/nktkt/gondolin/blob/main/NN/Verification/PINN/CLI.lean) are the user surfaces for that path.

PINNs are a good stress test because the model is only part of the claim. The PDE residual, the
domain, the boundary data, and the imported parameters all matter. Gondolin's design makes those
pieces explicit in the certificate object.

# Piecewise Polynomial and Spline Certificates

The spline path is concentrated in
[NN.Verification.Splines.PiecewisePolyCert API](https://github.com/nktkt/gondolin/blob/main/NN/Verification/Splines/PiecewisePolyCert.lean).
...
Scientific ML often lives at the boundary between theorem proving and numerical tooling. Gondolin's
