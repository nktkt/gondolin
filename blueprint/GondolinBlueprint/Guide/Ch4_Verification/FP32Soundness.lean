...
- `FP32`: Gondolin's proof model for binary32 style rounding on the reals.
...
Gondolin deliberately separates two questions:
...
In Gondolin, `FP32` serves as the model for binary32-style rounding on the finite path.

More concretely:

- `Gondolin.Floats.FP32` in [NN.Floats.FP32 API](https://github.com/nktkt/gondolin/blob/main/NN/Floats/FP32.lean) is the canonical
...
[IEEE32Exec semantics API](https://github.com/nktkt/gondolin/blob/main/NN/Floats/IEEEExec/Exec32.lean). Bridge and soundness theorems
connect that executable view to the standardized [FP32 proof semantics API](https://github.com/nktkt/gondolin/blob/main/NN/Floats/FP32.lean),
...
The kind of theorem Gondolin aims at looks like this:
...
Gondolin splits that argument into reusable pieces instead of reproving it from scratch every time:
...
specifications used elsewhere in Gondolin:
...
- `Tensor R s` is still a Gondolin tensor; only the scalar semantics changed.
...
- Higham, *Accuracy and Stability of Numerical Algorithms* (2nd ed., SIAM, 2002), the standard
  numerical analysis reference for forward error and stability arguments of the kind Gondolin
