...
Gondolin is still a research system, but the direction is clear. Modern ML systems increasingly
...
This is why Gondolin is organized as a framework rather than a collection of isolated examples.
...
- *Introduction* explains the problem Gondolin is built to address.
...
# How To State A Gondolin Result

When writing about Gondolin, the strongest statements have a simple form:
...
That rule keeps Gondolin extensible without making the system vague. Every addition should make at
least one object easier to run, inspect, check, or prove.

# Closing

Gondolin is useful because it keeps two activities connected: building neural-network systems and
...
and say what has been run, checked, proved, or assumed. If Gondolin makes that habit easier for
