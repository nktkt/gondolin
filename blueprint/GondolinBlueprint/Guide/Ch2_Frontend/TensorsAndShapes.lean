...
The first thing to understand in Gondolin is that a tensor's shape is part of the object, not only
runtime metadata. This is a small change with a large effect. If a model returns predictions of
shape `[batch, 1]` and the target has shape `[batch]`, Gondolin does not silently guess the
...
In Gondolin, the same idea is written with shape information in the type:
...
That is the basic pattern throughout Gondolin: the syntax feels familiar, but the shape is
...
For common ML shapes, Gondolin also provides readable aliases:
...
Gondolin uses the scalar element type as the “dtype”. For example:
...
- `Tensor Gondolin.Floats.IEEE32Exec s` is an executable IEEE-754 binary32 tensor.
...
Here is the shape mismatch from the opening, written as a Gondolin object:
...
Gondolin does not decide whether this should be a squeeze, a reshape, a one-hot encoding, or a
...
backend scalar in the model code. The [runtime module API](https://github.com/nktkt/gondolin/blob/main/NN/Runtime/Autograd/Gondolin/Module.lean)
...
Gondolin's tensor layer makes a few choices that are worth understanding early.
...
2. *Dynamic inputs are allowed at the boundary.* File loaders, JSON artifacts, and CLI data often
   arrive with runtime dimensions. Gondolin checks those dimensions as data and then packages them
...
In Gondolin, the intended shapes are visible at the definition site:
...
The relevant design note is that Gondolin does not force every input format to be perfectly typed at
...
## How Gondolin Differs from Plain PyTorch

The PyTorch version checks shapes when you run the code. Gondolin checks many of them when the file
...
wrong axis order or layout. In PyTorch you often discover that after a runtime error; in Gondolin the type
...
- [NN.Examples.Quickstart.TensorBasics API](https://github.com/nktkt/gondolin/blob/main/NN/Examples/Quickstart/TensorBasics.lean)
- [NN.Examples.Advanced.Tensors.Basic API](https://github.com/nktkt/gondolin/blob/main/NN/Examples/Advanced/Tensors/Basic.lean)
...
Gondolin’s tensor layer is not just a convenience wrapper. It is strong because the shape index is
...
[tensor proof API](https://github.com/nktkt/gondolin/blob/main/NN/Proofs/Tensor/Basic.lean).
