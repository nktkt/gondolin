...
For a first pass through Gondolin, read the API this way:
...
- `NN.API.Gondolin` is the namespace that the runtime surface re-exports for ordinary code.
...
Closest Gondolin analogue to `torch.utils.data` plus a small, predictable preprocessing surface:
...
The most important design decision is the boundary format: Gondolin expects numeric tensors in
...
This is worth saying explicitly because Gondolin examples now cover both small tutorial fixtures and
data backed training runs.

# The Runtime Surface

`NN.API.Runtime` and `NN.API.Gondolin` expose the lower layer machinery that underlies the public
...
- `API.Gondolin.Schedulers.constant`
- `API.Gondolin.Schedulers.step`
- `API.Gondolin.Schedulers.exponential`
...
Gondolin uses the same public model code with two execution styles:
...
`gondolin` runner; the `NN.API` types do not change. Only the runtime buffer implementation does. See
...
2. `NN.API.Runtime` or `NN.API.Gondolin` only when the lower layer API is the topic.
...
then *Gondolin vs PyTorch* for interop.
