...
Most Gondolin examples make three runtime choices: the scalar type, the execution backend, and the
...
```
lake exe gondolin mlp --cuda --steps 100
```
...
- [NN.API.Runtime API](https://github.com/nktkt/gondolin/blob/main/NN/API/Runtime.lean)
- [NN.API.Public API](https://github.com/nktkt/gondolin/blob/main/NN/API/Public.lean)
...
In Gondolin, the corresponding data remains explicit:
...
turn its computation into a reusable graph. Gondolin's compiled path is more explicit because the
...
normalization are the common examples. Gondolin keeps this state in the runner, rather than
...
Those live in [NN.API.Runtime API](https://github.com/nktkt/gondolin/blob/main/NN/API/Runtime.lean), with public re-exports in
[NN.API.Public API](https://github.com/nktkt/gondolin/blob/main/NN/API/Public.lean).
...
That separation is the reason Gondolin can be used as a tutorial framework, an executable
experiment harness, and a verification codebase at the same time.

# What To Read Next

For a small runnable example, open [SimpleMlpTrain](https://github.com/nktkt/gondolin/blob/main/NN/Examples/Quickstart/SimpleMlpTrain.lean).
For minibatches and epochs, open
[MinibatchMlpTrain](https://github.com/nktkt/gondolin/blob/main/NN/Examples/Quickstart/MinibatchMlpTrain.lean). For the declarations behind
the runner, open [NN.API.Runtime](https://github.com/nktkt/gondolin/blob/main/NN/API/Runtime.lean).
