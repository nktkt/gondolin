...
#doc (Manual) "Gondolin vs. PyTorch" =>
%%%
tag := "gondolin_vs_pytorch"
%%%

PyTorch and Gondolin should not be compared as if they were trying to solve the same problem.
PyTorch is the right default for broad, high-performance ML engineering. Gondolin asks a narrower
...
- Gondolin optimizes for semantic access: typed shapes, explicit parameters, graph denotations,
  numerical meanings, and checker-friendly artifacts.
- A realistic workflow can use both systems.

For the main design dimensions:

- Main goal: PyTorch is a production ML engineering system. Gondolin is proof-aware ML
  infrastructure.
- Tensor shapes: PyTorch treats shapes dynamically. Gondolin often carries shapes in Lean types.
- Parameters: PyTorch modules own mutable state. Gondolin keeps architecture and payload as
  explicit values.
- Autograd: PyTorch provides a mature, broad engine. Gondolin exposes supported gradients through
  definitions that can be related to Lean semantics.
- Graphs: PyTorch graph tools support capture, compilation, and deployment. Gondolin uses a shared
  IR with a Lean denotation.
- Floating point: PyTorch follows backend behavior. Gondolin names real, float32, executable
...
Gondolin keeps the authoring style familiar, but separates the model description from the parameter
...
In Gondolin, the parameter bundle is already part of the ordinary data path. This makes it easier
...
Gondolin uses Lean's dependent types for the core tensor APIs. A tensor carries both a scalar type
and a shape:
...
That tradeoff is intentional. Gondolin gives up some of Python's dynamism in exchange for a
...
Gondolin keeps the same conceptual rhythm, but the training state is explicit. Gradients are
...
PyTorch's autograd is mature, broad, and deeply optimized. Gondolin's advantage is semantic access:
...
For the public training surface, see the [public API facade](https://github.com/nktkt/gondolin/blob/main/NN/API/Public.lean) and the
[training runtime](https://github.com/nktkt/gondolin/blob/main/NN/Runtime/Autograd/Train.lean).
...
Gondolin's graph path is designed for a different use case. A supported model can be lowered to
[NN.IR.Graph](https://github.com/nktkt/gondolin/blob/main/NN/IR/Graph.lean), a graph whose nodes carry operation tags and a Lean denotation. Verifier
...
This is where Gondolin most clearly differs from a PyTorch clone. The goal is not simply to run the
...
checkpoints, distributed training, debugging tools, and deployment practices. Gondolin therefore
...
The relevant APIs are [PyTorch export](https://github.com/nktkt/gondolin/blob/main/NN/Runtime/PyTorch/Export.lean),
[PyTorch import](https://github.com/nktkt/gondolin/blob/main/NN/Runtime/PyTorch/Import.lean), and the
[round-trip examples](https://github.com/nktkt/gondolin/blob/main/NN/Examples/Interop/PyTorch.lean). A narrow contract is easier to audit
...
Gondolin names the relevant layers separately:
...
Use Gondolin when the supported model family needs to be a Lean object: when shapes should be part
...
Export a known architecture and payload. Import it into Gondolin. Inspect the graph. Check the
certificate. State the remaining assumptions. Gondolin's contribution is the semantic layer around
