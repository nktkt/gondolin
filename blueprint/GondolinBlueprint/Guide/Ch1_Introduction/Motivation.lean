...
fragile if no one can say precisely how these representations agree. Gondolin is built around that
...
Gondolin is built around making those distinctions visible. The graph representation, the
...
correctness, and floating-point approximation are different properties, and Gondolin keeps them as
different theorems.

# Our Response

Gondolin keeps model code, graph semantics, executable float models, certificate checkers, and
...
follows, and which runtime or external assumptions remain. That is why Gondolin keeps runnable
