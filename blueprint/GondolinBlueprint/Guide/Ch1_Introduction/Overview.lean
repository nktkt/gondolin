...
#doc (Manual) "What Gondolin Is" =>
%%%
tag := "overview"
%%%

Gondolin is a Lean 4 framework for neural networks whose computations can be run, inspected, and
...
certificates, and scientific claims. Gondolin gives these pieces a common place to meet: a typed
Lean development with runnable examples, graph semantics, explicit numerical models, and checkable
artifacts.

PyTorch remains the everyday training ecosystem for many projects. Gondolin focuses on the parts
...
The broad user import is [NN root API](https://github.com/nktkt/gondolin/blob/main/NN.lean). Narrower entry points include
[NN.Entrypoint.API](https://github.com/nktkt/gondolin/blob/main/NN/Entrypoint/API.lean),
[NN.Entrypoint.GraphSpec](https://github.com/nktkt/gondolin/blob/main/NN/Entrypoint/GraphSpec.lean),
[NN.Entrypoint.IR](https://github.com/nktkt/gondolin/blob/main/NN/Entrypoint/IR.lean), and
[NN.Entrypoint.Verification](https://github.com/nktkt/gondolin/blob/main/NN/Entrypoint/Verification.lean).

# What Gondolin Covers

At a high level, Gondolin provides three layers.
...
training mode, random state, scalar semantics, and the exact graph a verifier checked. Gondolin
...
hidden by broadcasting. In Gondolin's typed tensor APIs, the shape is part of the tensor type, so
...
At the top level, Gondolin provides a facade with PyTorch vocabulary over a shared internal model.
...
Gondolin separates fast execution from proof, but it does not treat them as unrelated worlds. For
...
is not the deployed computation. Gondolin's design pushes parameter payloads, graph lowering, and
