...
The reason Gondolin uses a functional style is not aesthetic. It is about making state visible.
...
In a pure functional language such as Lean, ordinary functions have no side effects. A Gondolin
...
The same idea scales to tensors. In Gondolin, a layer is still read as
...
certificate producer contributes a value, Gondolin can name that imported value and state what is
...
For Gondolin, this matters because tensor code needs both a clean semantics and a realistic path to
...
and proof obligations close enough that they stay aligned. Gondolin applies that principle to
...
passed, saved, inspected, and mentioned in theorem statements. Gondolin does not remove state from
