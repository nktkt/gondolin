...
When the model is built, Gondolin separates architecture from parameters. That is the first
...
PyTorch also has a parameter bundle, usually exposed through `state_dict`. Gondolin makes that idea
...
The graph chapters explain how Gondolin lowers model code into `NN.IR.Graph`, a DAG whose nodes
...
A theorem over real numbers and a float32 execution are not the same statement. Gondolin keeps the
...
That tiny example is the design in miniature. Gondolin does not try to guess which broadcast,
...
A certificate imported from outside Lean? Most Gondolin mistakes become easier to diagnose once
