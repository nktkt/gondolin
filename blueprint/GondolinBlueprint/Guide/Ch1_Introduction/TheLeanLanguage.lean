...
Lean matters for Gondolin because it can play two roles at once. It is a theorem prover, so it can
...
That combination is the reason Gondolin can be more than a paper formalization. The same
...
The relevance for Gondolin is not that neural networks are pure mathematics in the usual textbook
...
Lean 4 made a few engineering choices that matter for Gondolin:
...
- Lean’s *dependent type system*, which Gondolin uses to make shapes and other invariants part of the type.

Gondolin uses those properties to bring Lean's strengths into scientific computing. The same
...
The Gondolin pattern is to import a result through a small typed interface, check whatever can be
...
theorem can state a quantified property of the object being checked. Gondolin needs both kinds of
...
This looks ordinary because it is ordinary executable code. Its usefulness for Gondolin comes from
...
checked at all. In Gondolin, the tensor type can carry the scalar type and the exact dimensional
...
from silent shape conventions, so Gondolin tries to make those conventions inspectable.
...
For Gondolin, `#check` is often the fastest way to learn whether you are holding a tensor, a model
...
Gondolin relies on this distinction. A model denotation should be a pure object that can appear in a
