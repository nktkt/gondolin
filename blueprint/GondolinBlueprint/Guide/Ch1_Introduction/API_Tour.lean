...
Gondolin's public API is easiest to understand by following one model through the system. At
first, it looks like a small PyTorch-style library embedded in Lean: we write tensors, layers,
losses, optimizers, and training loops. The difference is what Gondolin keeps around as the model
...
The [public API facade](https://github.com/nktkt/gondolin/blob/main/NN/API/Public.lean) is the default entry point for this path. The
...
- When the user writes a tensor literal, Gondolin keeps the scalar type and shape.
- When the user builds a model, Gondolin keeps the architecture and parameter shapes.
- When the user runs training, Gondolin keeps parameter state, optimizer state, random state, and logs.
- When the user lowers a graph, Gondolin keeps operation tags, node ids, parent links, and payloads.
- When the user checks a certificate, Gondolin keeps the checker predicate and scalar semantics.
...
with predictions shaped `[batch, 1]` and targets shaped `[batch]` is often a modeling bug. Gondolin
...
For tensor constructors and literals, see the [tensor API](https://github.com/nktkt/gondolin/blob/main/NN/Tensor/API.lean). For the model
builder surface used above, start with the [public API facade](https://github.com/nktkt/gondolin/blob/main/NN/API/Public.lean).

# Building And Training

In Gondolin, model structure and parameters are separate values. Building a model chooses an
...
[training runtime](https://github.com/nktkt/gondolin/blob/main/NN/Runtime/Autograd/Train.lean) and the
[runtime optimizer definitions](https://github.com/nktkt/gondolin/blob/main/NN/Runtime/Autograd/Gondolin/Optim.lean).

# Inspecting Runs

A runnable model should not become opaque after it trains. Gondolin keeps several inspection
surfaces close to the training path:

- structured training logs, persisted as ordinary JSON, are defined by the
  [training log API](https://github.com/nktkt/gondolin/blob/main/NN/Runtime/Training/Log.lean);
- per-step logger hooks for training loops are defined by the
  [training logger API](https://github.com/nktkt/gondolin/blob/main/NN/Runtime/Autograd/Train/Logging.lean);
- widgets can display tensors, logs, execution traces, and IR graphs through the
  [widget entrypoint](https://github.com/nktkt/gondolin/blob/main/NN/Entrypoint/Widgets.lean).
...
[NN.IR.Graph](https://github.com/nktkt/gondolin/blob/main/NN/IR/Graph.lean). Its denotation is the semantic reference used by graph
checkers, widgets, compiler bridges, and verifier passes.

The PyTorch boundary follows the same discipline. Gondolin does not claim to import arbitrary
...
\text{shape checks}
\quad\leadsto\quad
\text{Gondolin parameters}`

That contract is implemented by the [PyTorch export](https://github.com/nktkt/gondolin/blob/main/NN/Runtime/PyTorch/Export.lean) and
[PyTorch import](https://github.com/nktkt/gondolin/blob/main/NN/Runtime/PyTorch/Import.lean) APIs, with runnable examples in the
[PyTorch interop tutorial](https://github.com/nktkt/gondolin/blob/main/NN/Examples/Interop/PyTorch.lean). Python can remain the right tool
...
A robustness certificate in Gondolin is a concrete claim about a graph, a parameter payload, an
input region, a scalar semantics, and a checker result. The [verification entrypoint](https://github.com/nktkt/gondolin/blob/main/NN/Entrypoint/Verification.lean)
collects the public verification surface, while the [CROWN graph API](https://github.com/nktkt/gondolin/blob/main/NN/MLTheory/CROWN/Graph.lean)
...
tokenizer tables, and cache layouts affect the computation. Gondolin's functional style keeps
...
[floating-point entrypoint](https://github.com/nktkt/gondolin/blob/main/NN/Entrypoint/Floats.lean) exists so a theorem, a verifier claim,
