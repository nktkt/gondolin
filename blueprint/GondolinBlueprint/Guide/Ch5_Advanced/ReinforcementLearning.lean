...
Reinforcement learning is a good test for Gondolin because the model is only one part of the
...
as mutable Python arrays. Gondolin gives them Lean-side names.

We built the Gondolin stack differently. We still want the familiar pieces: Gymnasium
...
[runtime RL API](https://github.com/nktkt/gondolin/blob/main/NN/Runtime/RL.lean), [specification RL API](https://github.com/nktkt/gondolin/blob/main/NN/Spec/RL.lean), and
[RL proofs API](https://github.com/nktkt/gondolin/blob/main/NN/Proofs/RL/Core.lean) are deliberately separate: the split lets us say
...
\;\to\; \text{Gondolin rollout tensors}`
...
boundaries because that makes it easy to move fast. Gondolin keeps them visible because that makes
...
The runtime umbrella [NN.Runtime.RL API](https://github.com/nktkt/gondolin/blob/main/NN/Runtime/RL.lean) collects the executable pieces:

- [Runtime.RL.Core](https://github.com/nktkt/gondolin/blob/main/NN/Runtime/RL/Core.lean) contains typed transitions, returns, TD errors,
  action encodings, and rollout helpers with tensor shapes.
- [Runtime.RL.Replay](https://github.com/nktkt/gondolin/blob/main/NN/Runtime/RL/Replay.lean) contains bounded FIFO replay buffers for
  off policy algorithms.
- [Runtime.RL.Boundary](https://github.com/nktkt/gondolin/blob/main/NN/Runtime/RL/Boundary.lean) checks externally supplied observations,
  rewards, actions, and done flags before they enter training.
- [Runtime.RL.Gymnasium](https://github.com/nktkt/gondolin/blob/main/NN/Runtime/RL/Gymnasium.lean) is the subprocess bridge to Python
  Gymnasium.
- [Runtime.RL.PPO](https://github.com/nktkt/gondolin/blob/main/NN/Runtime/RL/PPO.lean) contains PPO rollout and sample collection with a fixed horizon.
- [Runtime.RL.Numerics](https://github.com/nktkt/gondolin/blob/main/NN/Runtime/RL/Numerics.lean) contains checked float32 diagnostics for
  RL recurrences.

# MDP Semantics First

The spec side is intentionally not "whatever the Python environment did." In
[NN.Spec.RL API](https://github.com/nktkt/gondolin/blob/main/NN/Spec/RL.lean), Gondolin collects several MDP vocabularies:

- [Spec.RL.Core](https://github.com/nktkt/gondolin/blob/main/NN/Spec/RL/Core.lean) gives discounted backups, TD residuals, returns, and
  GAE style algebra over rollout data.
- [Spec.RL.Environment](https://github.com/nktkt/gondolin/blob/main/NN/Spec/RL/Environment.lean) gives a pure Gymnasium style environment
  contract with explicit latent state.
- [Spec.RL.MDP](https://github.com/nktkt/gondolin/blob/main/NN/Spec/RL/MDP.lean) gives deterministic finite MDPs over finite states and
  actions.
- [Spec.RL.FiniteStochasticMDP](https://github.com/nktkt/gondolin/blob/main/NN/Spec/RL/FiniteStochasticMDP.lean) gives finite stochastic
  transition kernels.
- [Spec.RL.MarkovMDP](https://github.com/nktkt/gondolin/blob/main/NN/Spec/RL/MarkovMDP.lean) gives the heavier measurable-space development.
- [Spec.RL.Envs.GridWorld](https://github.com/nktkt/gondolin/blob/main/NN/Spec/RL/Envs/GridWorld.lean) gives a concrete finite GridWorld
...
In Gondolin, that line is also a named semantic object. The proof layer can state and prove facts
about the Bellman operator rather than reconstructing a tensor expression after the fact.
[NN.Proofs.RL.MDP API](https://github.com/nktkt/gondolin/blob/main/NN/Proofs/RL/MDP.lean), for example, proves monotonicity and contraction
...
The [Gymnasium client API](https://github.com/nktkt/gondolin/blob/main/NN/Runtime/RL/Gymnasium/Client.lean) talks to
...
The contract itself is in [NN.Runtime.RL.Boundary.Core API](https://github.com/nktkt/gondolin/blob/main/NN/Runtime/RL/Boundary/Core.lean).
...
[NN.Proofs.RL.Gymnasium API](https://github.com/nktkt/gondolin/blob/main/NN/Proofs/RL/Gymnasium.lean) returns the transition bundled with
...
objects whose shape invariants are enforced by convention. Gondolin's
[NN.Runtime.RL.Replay API](https://github.com/nktkt/gondolin/blob/main/NN/Runtime/RL/Replay.lean) defines a bounded FIFO buffer over
typed transitions:
...
The proof layer in [NN.Proofs.RL.Replay API](https://github.com/nktkt/gondolin/blob/main/NN/Proofs/RL/Replay.lean) certifies structural
facts such as:
...
[NN.Examples.Models.RL.DQNReplay API](https://github.com/nktkt/gondolin/blob/main/NN/Examples/Models/RL/DQNReplay.lean) as a small
...
import is [NN.Runtime.RL.PPO API](https://github.com/nktkt/gondolin/blob/main/NN/Runtime/RL/PPO.lean), with the data layout for a fixed horizon
in [NN.Runtime.RL.PPO.Rollout API](https://github.com/nktkt/gondolin/blob/main/NN/Runtime/RL/PPO/Rollout.lean).
...
Gondolin keeps this recurrence as a Lean definition, feeds its tensorized result to the PPO
...
Gondolin's checked numerics for ratios, clipping, returns, and advantages are local pieces of that
larger PPO story.
...
Gondolin's checked float32 helpers are collected in the
[RL float32 numerics API](https://github.com/nktkt/gondolin/tree/main/NN/Runtime/RL/Numerics/Float32). They use the executable
`IEEE32Exec` model and return `Except String ...` so finite path failures are visible. The core
pieces are:

- [Returns](https://github.com/nktkt/gondolin/blob/main/NN/Runtime/RL/Numerics/Float32/Returns.lean): checked discounted backups and
  returns over a fixed horizon.
- [Advantage](https://github.com/nktkt/gondolin/blob/main/NN/Runtime/RL/Numerics/Float32/Advantage.lean): checked TD residuals, GAE, and
  advantage normalization.
- [PPO](https://github.com/nktkt/gondolin/blob/main/NN/Runtime/RL/Numerics/Float32/PPO.lean): checked importance ratios and clipped PPO
  surrogate pieces.
- [Intervals](https://github.com/nktkt/gondolin/blob/main/NN/Runtime/RL/Numerics/Float32/Intervals.lean): interval diagnostics for
  RL scalar recurrences.

The proof bridge is in [NN.Proofs.RL.Floats.IEEE32Exec API](https://github.com/nktkt/gondolin/blob/main/NN/Proofs/RL/Floats/IEEE32Exec.lean)
and [NN.Proofs.RL.Floats.CheckedRuntime API](https://github.com/nktkt/gondolin/blob/main/NN/Proofs/RL/Floats/CheckedRuntime.lean). The
...
[NN.Examples.Models.RL.PPOGridWorld API](https://github.com/nktkt/gondolin/blob/main/NN/Examples/Models/RL/PPOGridWorld.lean) trains an
actor critic on a GridWorld defined in Lean. It imports the spec environment and proof hooks:
[NN.Spec.RL.Envs.GridWorld API](https://github.com/nktkt/gondolin/blob/main/NN/Spec/RL/Envs/GridWorld.lean) and
[NN.Proofs.RL.Envs.GridWorld API](https://github.com/nktkt/gondolin/blob/main/NN/Proofs/RL/Envs/GridWorld.lean). The example proves that
...
[NN.Examples.RL.PPOGridWorldView API](https://github.com/nktkt/gondolin/blob/main/NN/Examples/RL/PPOGridWorldView.lean) reads the
...
[NN.Examples.Models.RL.PPOCartPole API](https://github.com/nktkt/gondolin/blob/main/NN/Examples/Models/RL/PPOCartPole.lean) uses
...
The Gondolin difference is the contract. CartPole observations have shape `4`, actions live in
`Fin 2`, and every transition must pass the boundary checker before becoming training data. The
viewer [NN.Examples.RL.PPOCartPoleView API](https://github.com/nktkt/gondolin/blob/main/NN/Examples/RL/PPOCartPoleView.lean) displays the
...
[NN.Examples.Models.RL.PPOPongRam API](https://github.com/nktkt/gondolin/blob/main/NN/Examples/Models/RL/PPOPongRam.lean) targets
...
The viewer [NN.Examples.RL.PPOPongRamView API](https://github.com/nktkt/gondolin/blob/main/NN/Examples/RL/PPOPongRamView.lean) displays
...
# Interpreting A Gondolin RL Claim
