...
Gondolin contract. We built it as a verification catalog, not as a collection of artificial
failures. Each card points at a bug family that has appeared in frameworks, compilers, deployment
tools, or LLM serving stacks, then asks a narrow question: what object would have made the intended
behavior explicit?

The [BugZoo catalog API](https://github.com/nktkt/gondolin/tree/main/NN/Examples/BugZoo/) and
[BugZoo overview](https://github.com/nktkt/gondolin/blob/main/NN/Examples/BugZoo/README.md) focus on the Gondolin fragment itself:
once a computation enters a typed Gondolin spec, shape changes, masks, token bounds, finite domain
choices, stateful normalization parameters, and backend semantics become named objects that can be
checked.

BugZoo should be read as the smallest public proof-of-concept for Gondolin's philosophy. Each card
...
- the Gondolin object that names the intended behavior;
...
That is why the cards are short. The goal is not to recreate an entire framework bug; the goal is
...
[NN.Examples.BugZoo.ShapeAndBroadcast API](https://github.com/nktkt/gondolin/blob/main/NN/Examples/BugZoo/ShapeAndBroadcast.lean)
records one of Gondolin's core design choices: shapes belong in the object, not as an
...
matrix and still produce a plausible loss. In Gondolin, ordinary elementwise operations require the
...
[NN.Examples.BugZoo.StableLoss API](https://github.com/nktkt/gondolin/blob/main/NN/Examples/BugZoo/StableLoss.lean) is about losses that
...
Gondolin keeps two APIs separate. Logits should use `crossEntropyLogitsSpec`, which unfolds through
...
This card is motivated by [TensorFuzz](https://proceedings.mlr.press/v97/odena19a.html), which
...
[NN.Examples.BugZoo.IgnoredLabelLoss API](https://github.com/nktkt/gondolin/blob/main/NN/Examples/BugZoo/IgnoredLabelLoss.lean) turns a
...
The Gondolin lesson is not "we copied every branch of a framework kernel." Gondolin exposes the
...
[NN.Examples.BugZoo.AutogradDomain API](https://github.com/nktkt/gondolin/blob/main/NN/Examples/BugZoo/AutogradDomain.lean) follows
...
Gondolin's card names the difference between "divide first, mask later" and "safe divide, then
...
[NN.Examples.BugZoo.AttentionMask API](https://github.com/nktkt/gondolin/blob/main/NN/Examples/BugZoo/AttentionMask.lean) is the catalog
...
`exp(-infinity) = 0`. Gondolin's ordinary attention spec then uses the equivalent hard masked
...
This card also points back to the original transformer paper,
[*Attention Is All You Need*](https://arxiv.org/abs/1706.03762).

## Compiler Boundary

[NN.Examples.BugZoo.CompilerBoundary API](https://github.com/nktkt/gondolin/blob/main/NN/Examples/BugZoo/CompilerBoundary.lean) is the
...
[NN.Examples.BugZoo.FloatBoundary API](https://github.com/nktkt/gondolin/blob/main/NN/Examples/BugZoo/FloatBoundary.lean) is where we
...
reduction order. The card cites
[Jia and Rinard's warning paper](https://doi.org/10.1109/SPW50608.2020.00058).

Gondolin's answer is explicit modeling. `IEEE32Exec` is the executable bit level float32 model.
...
[NN.Examples.BugZoo.NormalizationState API](https://github.com/nktkt/gondolin/blob/main/NN/Examples/BugZoo/NormalizationState.lean)
...
that Gondolin's scalar normalization puts epsilon inside the variance term before square root.
...
[NN.Examples.BugZoo.BatchInvariance API](https://github.com/nktkt/gondolin/blob/main/NN/Examples/BugZoo/BatchInvariance.lean) is about
...
[NN.Examples.BugZoo.KVCache API](https://github.com/nktkt/gondolin/blob/main/NN/Examples/BugZoo/KVCache.lean) models cache accounting in
...
[NN.Examples.BugZoo.RoPEPosition API](https://github.com/nktkt/gondolin/blob/main/NN/Examples/BugZoo/RoPEPosition.lean) pairs naturally
with the KV cache card. Rotary position embeddings make position accounting part of the model's
...
[NN.Examples.BugZoo.TokenizerBoundary API](https://github.com/nktkt/gondolin/blob/main/NN/Examples/BugZoo/TokenizerBoundary.lean) marks
...
Gondolin's current contract is small but valuable: token IDs inside the verified fragment can be
...
BugZoo makes proof engineering more concrete. Instead of saying "Gondolin helps with ML bugs," we
can point at a card and ask what changed:
...
Future BugZoo entries should follow the same rhythm: cite the real bug family, name the Gondolin
