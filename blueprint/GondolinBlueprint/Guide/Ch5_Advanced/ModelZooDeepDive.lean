...
The model zoo exercises Gondolin as a whole system. A tensor library can look good on
...
We built the [model examples API](https://github.com/nktkt/gondolin/tree/main/NN/Examples/Models/) to answer that question
...
Most demos are dispatched by [NN.Examples.Models.Runner API](https://github.com/nktkt/gondolin/blob/main/NN/Examples/Models/Runner.lean):
...
part of the gated recurrent design space: Gondolin's present runnable recurrent coverage is RNN,
...
The architecture equations are familiar, but the Gondolin reading is specific:
...
- [NN.Examples.Models.Sequence.Gpt2 API](https://github.com/nktkt/gondolin/blob/main/NN/Examples/Models/Sequence/Gpt2.lean), a small
  byte level GPT-2 style causal language model.
- [NN.Examples.Models.Sequence.TextGpt2 API](https://github.com/nktkt/gondolin/blob/main/NN/Examples/Models/Sequence/TextGpt2.lean), a
  trainer over a text corpus that can use GPT-2 style BPE assets.
- [NN.Examples.Models.Sequence.CharGpt API](https://github.com/nktkt/gondolin/blob/main/NN/Examples/Models/Sequence/CharGpt.lean), a
  minGPT style character model with deterministic random windows and generation probes.
- [NN.Examples.Models.Sequence.Gpt2Saved API](https://github.com/nktkt/gondolin/blob/main/NN/Examples/Models/Sequence/Gpt2Saved.lean),
  which exercises saved parameter loading and prompting.
- [NN.Examples.Models.Sequence.GptAdder API](https://github.com/nktkt/gondolin/blob/main/NN/Examples/Models/Sequence/GptAdder.lean), a
...
lake exe -K cuda=true gondolin gpt2 --cuda --fast-kernels \
...
```
lake exe -K cuda=true gondolin chargpt --cuda --tiny-shakespeare \
...
They also stress the boundary between "GPT style" and "GPT-2". Gondolin's `gpt2` command is not
...
[NN.Examples.Models.Sequence.Mamba API](https://github.com/nktkt/gondolin/blob/main/NN/Examples/Models/Sequence/Mamba.lean) keeps the
...
```
python3 scripts/datasets/download_example_data.py --tiny-shakespeare
lake exe -K cuda=true gondolin mamba --cuda --fast-kernels \
...
[NN.Spec.Models.Mamba API](https://github.com/nktkt/gondolin/blob/main/NN/Spec/Models/Mamba.lean), including run-list length facts for
compact Mamba style blocks. The API side in [NN.API.Models.Mamba API](https://github.com/nktkt/gondolin/blob/main/NN/API/Models/Mamba.lean)
...
- [NN.Examples.Models.Sequence.Rnn API](https://github.com/nktkt/gondolin/blob/main/NN/Examples/Models/Sequence/Rnn.lean), a vanilla RNN
  text-window runtime check.
- [NN.Examples.Models.Sequence.Lstm API](https://github.com/nktkt/gondolin/blob/main/NN/Examples/Models/Sequence/Lstm.lean), an LSTM
  text-window runtime check.
- [NN.Examples.Models.Supervised.LstmRegression API](https://github.com/nktkt/gondolin/blob/main/NN/Examples/Models/Supervised/LstmRegression.lean),
  a more natural LSTM forecasting tutorial over household power windows.

The small text commands are intentionally short:

```
lake exe -K cuda=true gondolin rnn --cuda --tiny-shakespeare --steps 1
lake exe -K cuda=true gondolin lstm --cuda --tiny-shakespeare --steps 1
```
...
used as one category in ML writing. Gondolin covers vanilla RNN and LSTM executables;
...
[NN.Examples.Models.Vision.Resnet API](https://github.com/nktkt/gondolin/blob/main/NN/Examples/Models/Vision/Resnet.lean) trains a
compact ResNet style classifier over CIFAR arrays:

```
python3 scripts/datasets/download_example_data.py --cifar10
lake exe -K cuda=true gondolin resnet --cuda --n-total 20 --steps 1
```

The spec side in [NN.Spec.Models.Resnet API](https://github.com/nktkt/gondolin/blob/main/NN/Spec/Models/Resnet.lean) is richer than a
...
[NN.Examples.Models.Vision.Vit API](https://github.com/nktkt/gondolin/blob/main/NN/Examples/Models/Vision/Vit.lean) trains a compact
ViT style CIFAR classifier:

```
python3 scripts/datasets/download_example_data.py --cifar10
lake exe -K cuda=true gondolin vit --cuda --n-total 20 --steps 1
```

The corresponding spec material in [NN.Spec.Models.Vit API](https://github.com/nktkt/gondolin/blob/main/NN/Spec/Models/Vit.lean) names
...
ViT is a useful bridge between the vision and sequence parts of Gondolin. It asks the image loader
...
[NN.Examples.Models.Operators.Fno1dBurgers API](https://github.com/nktkt/gondolin/blob/main/NN/Examples/Models/Operators/Fno1dBurgers.lean)
...
lake exe -K cuda=true gondolin fno1d_burgers --cuda --fast-kernels \
...
```
lake exe gondolin autoencoder --steps 10
lake exe gondolin mae --steps 10
lake exe gondolin vae --steps 10
lake exe gondolin vqvae --steps 10
lake exe gondolin gan --steps 10
lake exe gondolin diffusion --steps 5
```
...
[generative theory API](https://github.com/nktkt/gondolin/tree/main/NN/MLTheory/Generative/). The companion generative guide explains
...
- [NN.Examples.Models.RL.PPOCartPole API](https://github.com/nktkt/gondolin/blob/main/NN/Examples/Models/RL/PPOCartPole.lean) trains an
  actor critic on Gymnasium `CartPole-v1` through a Python subprocess bridge.
- [NN.Examples.Models.RL.PPOGridWorld API](https://github.com/nktkt/gondolin/blob/main/NN/Examples/Models/RL/PPOGridWorld.lean) gives a
  smaller controlled PPO environment.
- [NN.Examples.Models.RL.PPOPongRam API](https://github.com/nktkt/gondolin/blob/main/NN/Examples/Models/RL/PPOPongRam.lean) exercises a
  larger observation/action setting.
- [NN.Examples.Models.RL.DQNReplay API](https://github.com/nktkt/gondolin/blob/main/NN/Examples/Models/RL/DQNReplay.lean) focuses on the
  off policy replay and minibatch update pieces.

For CartPole:

```
lake exe -K cuda=true gondolin ppo_cartpole --cuda --updates 50
```
...
RL stresses producer/checker boundaries harder than ordinary supervised learning. Gondolin can
...
boundary contract before becoming training data. Gondolin verifies what enters the Lean side, while
...
Gondolin can type and check the policy, rollout buffers, returns, and PPO losses. If the
...
That layered view is a feature of the zoo. Readers can see where Gondolin is already a proof
