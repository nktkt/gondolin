...
The model zoo is the point where Gondolin stops being a clean semantic story and starts being an
...
The subcommands are registered in the [model runner API](https://github.com/nktkt/gondolin/blob/main/NN/Examples/Models/Runner.lean), and
the model implementations are collected by the [model examples API](https://github.com/nktkt/gondolin/tree/main/NN/Examples/Models/).
...
- the same training loop works for vectors, images, residual blocks, and patch/attention-style
  vision models;
- model parameters are ordinary Gondolin parameters, not hidden Python objects;
...
lake exe -K cuda=true gondolin transformer --cuda --tiny-stories --steps 1
```
...
through Gondolin's runtime.

# GPT-Style Language Models

Gondolin has two GPT-facing demos:
...
```
lake exe -K cuda=true gondolin gpt2 --cuda --steps 1
lake exe -K cuda=true gondolin text_gpt2 --cuda \
  --data-file data/real/text/tinystories_valid.txt --allow-small-data --steps 100 --log-every 10
```

The `text_gpt2` example is explicit about scale. It is a Gondolin-native miniature
...
does exercise the same kind of boundary Gondolin needs for serious sequence-model work.
...
```
lake exe -K cuda=true gondolin mamba --cuda --tiny-shakespeare --steps 25
```

Under the CUDA backend, Gondolin includes native selective-scan kernels for the float32 path.  The
model is small, but the architectural lesson is real: not every modern sequence model is attention,
and Gondolin's runtime has begun to cover that broader operator family.
...
The FNO example is the best current example of Gondolin training on a real scientific-ML dataset.
The Python helper only downloads/converts data and plots the result; the model and training loop are
native Gondolin.

Prepare data with the [Burgers data helper](https://github.com/nktkt/gondolin/blob/main/NN/Examples/Data/prepare_fno1d_burgers.py):
...
Plot one held-out prediction with the [plot helper](https://github.com/nktkt/gondolin/blob/main/NN/Examples/Data/plot_fno1d_burgers.py):
...
The PPO examples show that Gondolin is not limited to supervised losses:

```
lake exe gondolin ppo_gridworld --steps 100
lake exe gondolin ppo_cartpole --cuda --steps 100
```
...
For RL, the useful Gondolin idea is not "PPO exists." It is that an episode can be treated as data:
...
The [example-data helper](https://github.com/nktkt/gondolin/blob/main/scripts/datasets/download_example_data.py) prepares common tiny datasets:

```
python3 scripts/datasets/download_example_data.py --tiny-shakespeare --tinystories-valid --cifar10
```

Gondolin examples should make the data source visible. If a file is generated, downloaded, or
