...
| first training example | `lake exe gondolin mlp --cpu --steps 10` |
| first autograd example | `NN/Examples/Quickstart/AutogradBasics.lean` |
| first graph example | `lake exe verify -- gondolin-ibp` |
| first BugZoo example | `NN.Examples.BugZoo.All` |
| first CUDA smoke test | `lake exe -K cuda=true gondolin mlp --cuda --steps 20` |
| first scientific ML example | `lake exe verify -- ode` or the PINN/FNO examples |

- *Can I build and inspect small tensors?* Read [Quickstart](https://github.com/nktkt/gondolin/tree/main/NN/Examples/Quickstart/).
- *Can I train a model in Lean?* Read [Models](https://github.com/nktkt/gondolin/tree/main/NN/Examples/Models/) and
  [Data](https://github.com/nktkt/gondolin/tree/main/NN/Examples/Data/).
- *Can I cross the PyTorch boundary without losing shapes?* Read
  [PyTorch interop](https://github.com/nktkt/gondolin/tree/main/NN/Examples/Interop/PyTorch/).
- *Can I see the graph that a verifier sees?* Read [Advanced GraphSpec and IR demos](https://github.com/nktkt/gondolin/tree/main/NN/Examples/Advanced/).
- *Can I check a property or certificate?* Read the verification examples and fixtures.
- *Can I see common ML bugs as checked Lean case studies?* Read [Bug Zoo](https://github.com/nktkt/gondolin/tree/main/NN/Examples/BugZoo/).
...
Gondolin turns a bug pattern into a checked statement.
...
- Verification fixture: `gondolin-transformer-ibp` pushes a tiny transformer style graph through
  the same verifier surface.

This is the preferred Gondolin pattern: faster execution is welcome, but it is paired with a named
...
- [Quickstart examples](https://github.com/nktkt/gondolin/tree/main/NN/Examples/Quickstart/)
  Typed-tensor warmups, autograd basics, and the smallest training tutorials.
- [Model examples](https://github.com/nktkt/gondolin/tree/main/NN/Examples/Models/)
  The main model zoo used by `lake exe gondolin`: MLP, CNN, ResNet, ViT, RNN/LSTM,
  Transformer, GPT text demos, Mamba demos, diffusion, and RL examples.
- [Advanced examples](https://github.com/nktkt/gondolin/tree/main/NN/Examples/Advanced/)
  GraphSpec, float-mode, tensor-bridge, and Gondolin-to-PyTorch IR demos.
- [PyTorch interop examples](https://github.com/nktkt/gondolin/tree/main/NN/Examples/Interop/PyTorch/)
  PyTorch round-trip scripts and generated-code interop examples.
- [Verification fixtures](https://github.com/nktkt/gondolin/tree/main/NN/Examples/Verification/)
  Small bundled certificate/checker demos used by `lake exe verify -- ...` (with external producers
  under `scripts/verification/`).
- [Widget gallery](https://github.com/nktkt/gondolin/blob/main/NN/Examples/Advanced/Widgets.lean)
  Infoview widgets used in the editor rather than through a long training run.
- [RL widget viewers](https://github.com/nktkt/gondolin/tree/main/NN/Examples/RL/)
  File-backed PPO/GridWorld dashboards colocated with the RL examples they visualize.
- [Bug Zoo](https://github.com/nktkt/gondolin/tree/main/NN/Examples/BugZoo/)
...
- [real data paths](https://github.com/nktkt/gondolin/blob/main/NN/Examples/Data/RealPaths.lean) and
  [model data helpers](https://github.com/nktkt/gondolin/blob/main/NN/Examples/Models/Common/RealData.lean)
...
5. Widgets demo, opened in the editor:
   open the [widgets example API](https://github.com/nktkt/gondolin/blob/main/NN/Examples/Advanced/Widgets.lean)
6. Autograd demo:
...
- `lake exe gondolin mlp --cpu --steps 10`: prints a short loss trace trending downward.
- `lake exe gondolin gpt_adder --steps 50 --cuda` (after `lake build -R -K cuda=true`): runs the minGPT
  addition demo on GPU when the CUDA backend has been built.
- `gondolin-ibp`: prints interval bounds for the output node, not raw graph internals.
...
- `--dtype ieee32exec` or `--dtype ieee754exec`
  Gondolin's executable IEEE-754 binary32 model defined in Lean (`IEEE32Exec`).
...
`lake exe gondolin <demo> [flags...]`

The [model examples API](https://github.com/nktkt/gondolin/tree/main/NN/Examples/Models/) contains the implementations, and the
[example runner API](https://github.com/nktkt/gondolin/blob/main/NN/Examples/Models/Runner.lean) defines the public subcommand names.

First run:

`lake exe gondolin --help`

## `gondolin` subcommands (curated zoo)
...
Runnable via `gondolin`: `lake exe gondolin gpt_adder` (Karpathy-style single-CUDA-kernel addition learning;
not a `gondolin` subcommand).
...
lake exe gondolin mlp --cpu --steps 10
lake exe gondolin cnn --cpu --n-total 20 --steps 1
lake exe gondolin resnet --cuda --n-total 20 --steps 1
lake exe gondolin vit --cuda --n-total 20 --steps 1
lake exe gondolin rnn --cuda --tiny-shakespeare --steps 1
lake exe gondolin transformer --cuda --tiny-stories --steps 1
lake exe gondolin mamba --cuda --tiny-shakespeare --steps 25
lake exe gondolin diffusion --cpu --steps 5
lake exe gondolin gpt2 --cuda --steps 1
lake build -R -K cuda=true && lake exe gondolin gpt_adder --steps 50 --cuda
```
...
data remains in the [example data API](https://github.com/nktkt/gondolin/tree/main/NN/Examples/Data/) so the repo can still run smoke
tests without network access.

These model examples matter because they show that Gondolin's public API is not limited to one
teaching example:
...
> Is Gondolin only comfortable with in-memory tutorial tensors?
...
- Lean demo: [GraphSpec tutorial API](https://github.com/nktkt/gondolin/blob/main/NN/Examples/Advanced/GraphSpec/Tutorial.lean)
- Command:
   `lake exe gondolin graphspec_mlp --backend compiled`

This demo defines an MLP once in GraphSpec, lowers it to the Gondolin training surface, and then
...
## Gondolin, IR, and Emitted PyTorch Code

- Lean demo: [Torch IR PyTorch export API](https://github.com/nktkt/gondolin/blob/main/NN/Examples/Advanced/TorchIRPyTorch.lean)
- Commands:
...
- Gondolin model, compiled IR, and generated PyTorch code.
...
BugZoo deserves its own reading pass because it explains what kinds of mistakes Gondolin is trying
to make visible. The [BugZoo API](https://github.com/nktkt/gondolin/tree/main/NN/Examples/BugZoo/) contains the checked cards, with the
full motivation in the [BugZoo overview](https://github.com/nktkt/gondolin/blob/main/NN/Examples/BugZoo/README.md).

The case studies are compact, but they cover a broad range:

- *Shape and broadcast*: missing axes and wrong broadcasts; Gondolin uses tensors indexed by shape
  and explicit broadcast evidence.
- *Stable loss*: unstable loss formulas and invalid domains; Gondolin names stable logit losses and
  safe-domain ops.
- *Ignored labels*: all-ignored label reductions; Gondolin uses an explicit contribution mask and
  empty-reduction policy.
- *Autograd domain*: masking after undefined division; Gondolin uses epsilon-protected division
  before masking.
- *Attention mask*: mask polarity and fully masked attention; Gondolin states exact causal-mask
  zero-weight properties.
- *Compiler boundary*: optimized graph wrong-code bugs; Gondolin asks for a source/target semantic
  preservation contract.
- *Float boundary*: real-proof versus float-deployment gaps; Gondolin names the bridge from runtime
  float32 to `IEEE32Exec`.
- *Normalization state*: BatchNorm formula/state mismatch; Gondolin makes epsilon placement and
  running statistics explicit.
- *Batch invariance*: serving output depends on batch composition; Gondolin states per-example
  batching behavior.
- *KV cache*: shifted or malformed decode cache; Gondolin names append-last key/value invariants.
- *RoPE position*: position mismatch during decoding; Gondolin names the position schedule.
- *Tokenizer boundary*: vocabulary/config mismatch; Gondolin represents token ids as
  `Fin vocabSize`.
...
bug class, the exact object Gondolin uses as the contract, and the theorem or boundary that
follows.
...
Start with the [widgets example API](https://github.com/nktkt/gondolin/blob/main/NN/Examples/Advanced/Widgets.lean), then return here for
...
- If it belongs in the public runner, register it in the
  [model runner API](https://github.com/nktkt/gondolin/blob/main/NN/Examples/Models/Runner.lean) and make sure `NN.Examples.Zoo`
  imports it.
...
There is a central model runner for model demos (`lake exe gondolin ...`), while
...
- George et al., *Gondolin* (2026): project overview. https://arxiv.org/abs/2602.22631
