...
This page is a citation map for numerical claims in Gondolin. Many papers use similar words:
...
Gondolin theorem should name whether the claim is over reals, `FP32`, `IEEE32Exec`, or a
...
Gondolin keeps these cases separate in code:
...
Flocq is the closest proof-engineering precedent for Gondolin's `FP32` layer: it formalizes
...
Gondolin's verification and bound propagation passes.
...
Among the references here, this paper is the closest match to how Gondolin separates concerns: it
...
# What Gondolin Can Cite Today

Gondolin already has a chain checked by Lean that is more concrete than a paper sketch:
...
- `relu_universal_approximation_Icc_fp32` layers Gondolin's `FP32` rounding model on
...
- FP32 rounded networks approximate a real target with an error budget: cite
  `relu_universal_approximation_Icc_fp32`, Higham-style forward error analysis, and the Gondolin
  `FP32` semantics.
...
- [IEEE32Exec universal approximation theorem](https://github.com/nktkt/gondolin/blob/main/NN/MLTheory/Proofs/Approximation/Universal/UniversalApproximationIEEE32Exec.lean)
...
- [approximation proofs](https://github.com/nktkt/gondolin/tree/main/NN/MLTheory/Proofs/Approximation/) for the universal-approximation
  theorems themselves.
