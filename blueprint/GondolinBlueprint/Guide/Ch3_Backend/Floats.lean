...
In Gondolin, "Float32" is not one object. It can mean an idealized rounded-real model used in
...
Gondolin separates four numerical layers:
...
about it require an agreement statement with the Lean-side model. That is how Gondolin can run real
...
#check Gondolin.Floats.Float32Mode
#check Gondolin.Floats.F32

-- Proof model: finite rounded-real binary32 arithmetic.
#check Gondolin.Floats.FP32
#check Gondolin.Floats.FP32.toReal
...
Gondolin states reduction facts around a finite evaluation tree rather than pretending every
parallel schedule is the same:

The [reductions API](https://github.com/nktkt/gondolin/blob/main/NN/Floats/IEEEExec/Reductions.lean) contains the long-form theorem
...
- [BridgeFP32 API](https://github.com/nktkt/gondolin/blob/main/NN/Floats/IEEEExec/BridgeFP32.lean) proves finite refinements such as
...
- [BridgeFP32Total API](https://github.com/nktkt/gondolin/blob/main/NN/Floats/IEEEExec/BridgeFP32Total.lean) packages total statements
...
- [SpecialRules API](https://github.com/nktkt/gondolin/blob/main/NN/Floats/IEEEExec/SpecialRules.lean) records NaN, infinity,
...
may provide an approximation with documented accuracy. Gondolin therefore keeps transcendental
claims explicit:
...
Gondolin names that assumption surface:
...
[Widgets API](https://github.com/nktkt/gondolin/blob/main/NN/Examples/Advanced/Widgets.lean), especially `#float32_view` and
`#float32_round_view`.
...
That split is the claim. Native execution is part of the workflow, and Gondolin makes the agreement
