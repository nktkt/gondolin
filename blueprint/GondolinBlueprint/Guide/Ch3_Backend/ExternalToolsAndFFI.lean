...
Gondolin is not trying to make Lean do every job. Python can train models and load checkpoints.
...
That distinction matters because Gondolin is designed to work with numerical ecosystems. Lean
serves as the accountability layer; the other tools remain excellent at what they do, while the
boundary stays explicit.

# Two Kinds Of Boundary

Gondolin uses two main external boundaries.
...
Python enters Gondolin in several different roles, which have different trust meanings.
...
operators fail closed. They do not get silently approximated as verified Gondolin operations.
...
We resolve `GONDOLIN_JULIA` when it is set, otherwise we use `julia` from `PATH`. Importing the
...
[trusted CUDA bridge](https://github.com/nktkt/gondolin/blob/main/NN/Runtime/Autograd/Engine/Cuda/Trusted.lean),
[buffer surface](https://github.com/nktkt/gondolin/blob/main/NN/Runtime/Autograd/Engine/Cuda/Buffer.lean),
[native source map](https://github.com/nktkt/gondolin/blob/main/NN/Runtime/Autograd/Engine/Cuda/NativeSources.lean),
[float32 contract](https://github.com/nktkt/gondolin/blob/main/NN/Runtime/Autograd/Engine/Cuda/Float32Contract.lean), and
[kernel specs](https://github.com/nktkt/gondolin/blob/main/NN/Runtime/Autograd/Engine/Cuda/KernelSpec.lean).
