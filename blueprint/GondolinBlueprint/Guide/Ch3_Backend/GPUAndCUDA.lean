...
CUDA belongs in Gondolin for a practical reason: serious ML systems use GPUs. If Gondolin only
...
4. *Runtime agreement is stated explicitly.* Gondolin documents which facts are proved in Lean,
...
lake exe -K cuda=true gondolin mlp --cuda --steps 20
lake exe -K cuda=true gondolin fno1d_burgers --cuda --fast-kernels --steps 700 --lr 0.003
```
...
[CUDA source tree](https://github.com/nktkt/gondolin/tree/main/csrc/cuda/) and links CUDA libraries such as cuBLAS and cuFFT. The
command line flag `--cuda` is a *runtime* flag: it asks a demo to use the CUDA eager backend.

If either piece is missing, Gondolin should fail loudly rather than silently claiming that GPU
execution happened.
...
- [`tensor/`](https://github.com/nktkt/gondolin/tree/main/csrc/cuda/tensor/)
  owns raw float32 buffers and elementwise tensor kernels;
- [`blas/`](https://github.com/nktkt/gondolin/tree/main/csrc/cuda/blas/)
  provides cuBLAS-backed matrix multiplication;
- [`conv_pool/`](https://github.com/nktkt/gondolin/tree/main/csrc/cuda/conv_pool/)
  provides convolution and pooling kernels;
- [`kernels/`](https://github.com/nktkt/gondolin/tree/main/csrc/cuda/kernels/)
...
- [`common/`](https://github.com/nktkt/gondolin/tree/main/csrc/cuda/common/)
...
The CUDA work in Gondolin is more than a single "move tensors to GPU" hook.  It is a layered
...
library choices, and compute capability still affect a numerical claim. Gondolin keeps those cases
...
- *BMM/GEMM*: row-major Gondolin buffers must be interpreted consistently around cuBLAS, and the
...
Second, Gondolin still needs to run real models.  Proving every GPU instruction before using CUDA
...
[CUDA runtime modules](https://github.com/nktkt/gondolin/tree/main/NN/Runtime/Autograd/Engine/Cuda/):
...
still looks like an ordinary Gondolin run.  Under the hood, tensors are stored in CUDA buffers and
...
In practice this gives Gondolin three related but distinct CUDA layers:
...
produce order-dependent roundoff.  Gondolin therefore separates fast atomic reductions from
...
Gondolin uses vendor libraries where that is the right engineering choice:
...
Attention is the clearest example of why Gondolin separates a mathematical contract from a fast
...
The proof contract is in the [FlashAttention API](https://github.com/nktkt/gondolin/blob/main/NN/Spec/Layers/FlashAttention.lean):
...
The native runtime path is separate. The [CUDA kernel API](https://github.com/nktkt/gondolin/blob/main/NN/Runtime/Autograd/Engine/Cuda/Kernels.lean)
declares the FFI symbols, the
[native CUDA source](https://github.com/nktkt/gondolin/blob/main/csrc/cuda/kernels/gondolin_cuda_kernels.cu) implements them, and the CPU
stub sits next to it in
[the CPU stub](https://github.com/nktkt/gondolin/blob/main/csrc/cuda/kernels/gondolin_cuda_kernels_stub.c).

Gondolin's fused attention kernel is correctness-first: it implements the same masked,
...
- *Verified CUDA FlashAttention* would require a proof about the native kernel, which Gondolin does
  not claim.

The regression test for this boundary is
[NN.Tests.Runtime.Cuda.Attention API](https://github.com/nktkt/gondolin/blob/main/NN/Tests/Runtime/Cuda/Attention.lean), which compares
...
[Burgers preparation helper](https://github.com/nktkt/gondolin/blob/main/NN/Examples/Data/prepare_fno1d_burgers.py) downloads and converts
the data, and the [plot helper](https://github.com/nktkt/gondolin/blob/main/NN/Examples/Data/plot_fno1d_burgers.py) renders a held-out
prediction:
