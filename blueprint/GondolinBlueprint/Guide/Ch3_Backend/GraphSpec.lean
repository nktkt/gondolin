...
The sequential DSL is defined in [NN.GraphSpec.Core API](https://github.com/nktkt/gondolin/blob/main/NN/GraphSpec/Core.lean).
...
The [GraphSpec DAG API](https://github.com/nktkt/gondolin/blob/main/NN/GraphSpec/DAG.lean) and
[GraphSpec DAG core](https://github.com/nktkt/gondolin/blob/main/NN/GraphSpec/DAG/Core.lean) define the shared-branch authoring surface.
...
- the Gondolin compiler supplies an executable program in the runtime layer.
...
#check NN.GraphSpec.ToGondolin.toSeq
```
...
GraphSpec model becomes a runnable Gondolin program or a lower-level DAG.

## Sequential Graph To DAG Model

The relevant definitions are [GraphSpec core](https://github.com/nktkt/gondolin/blob/main/NN/GraphSpec/Core.lean) and
[GraphSpec DAG core](https://github.com/nktkt/gondolin/blob/main/NN/GraphSpec/DAG/Core.lean).
...
## GraphSpec To Gondolin `nn.Sequential`

The lowering API is [GraphSpec to Gondolin](https://github.com/nktkt/gondolin/blob/main/NN/GraphSpec/ToGondolin.lean).

Important name:

- `NN.GraphSpec.ToGondolin.toSeq`

This path is used by the runnable
[GraphSpec tutorial](https://github.com/nktkt/gondolin/blob/main/NN/Examples/Advanced/GraphSpec/Tutorial.lean): author once in GraphSpec,
then plug the lowered model into the same `train.*` API used elsewhere in the guide.

## DAG Model To Runtime Model Zoo Wrappers

The runtime examples to open are [GraphSpec ResNet18](https://github.com/nktkt/gondolin/blob/main/NN/GraphSpec/Models/Gondolin/Resnet18.lean)
and [runtime FNO1D](https://github.com/nktkt/gondolin/blob/main/NN/Runtime/Autograd/Gondolin/Fno1d.lean).
...
#check NN.GraphSpec.Models.mlp_detInitParams_eq_gondolin_linear_inits
...
1. Read the [GraphSpec README](https://github.com/nktkt/gondolin/blob/main/NN/GraphSpec/README.md).
2. Open the [GraphSpec tutorial API](https://github.com/nktkt/gondolin/blob/main/NN/Examples/Advanced/GraphSpec/Tutorial.lean).
3. Run:
   `lake exe gondolin graphspec --backend compiled`
4. Read the [residual linear model API](https://github.com/nktkt/gondolin/blob/main/NN/GraphSpec/Models/ResidualLinear.lean).
5. Read the [ResNet-18 GraphSpec API](https://github.com/nktkt/gondolin/blob/main/NN/GraphSpec/Models/Resnet18.lean).
...
GraphSpec refines Gondolin's semantic model at the architecture boundary. In a Python codebase,
...
To inspect the implementation, start with the [GraphSpec README](https://github.com/nktkt/gondolin/blob/main/NN/GraphSpec/README.md), then
move to the sequential and DAG cores, then to the model zoo. The
[GraphSpec to Gondolin API](https://github.com/nktkt/gondolin/blob/main/NN/GraphSpec/ToGondolin.lean) gives the lowering path into
Gondolin `Seq`, and the
[GraphSpec Gondolin wrappers](https://github.com/nktkt/gondolin/tree/main/NN/GraphSpec/Models/Gondolin/) expose runtime model wrappers.
