...
Gondlin keeps that public surface intentionally small. The idea is not to expose every internal
...
- `lake exe gondlin <demo> [args...]` for the main model zoo,
- `lake env lean --run NN/Examples/.../Foo.lean -- [args...]` for files in the
  [examples tree](https://github.com/nktkt/gondlin/tree/main/NN/Examples/),
...
| `lake exe gondlin --help` | list model demos |
| `lake exe gondlin <demo>` | run a model-zoo demo |
...
[CUDA source tree](https://github.com/nktkt/gondlin/tree/main/csrc/cuda/) link against the toolkit:
...
Model examples use the `gondlin` runner:
