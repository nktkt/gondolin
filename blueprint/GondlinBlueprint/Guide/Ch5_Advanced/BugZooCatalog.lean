...
Gondlin contract. We built it as a verification catalog, not as a collection of artificial
...
The [BugZoo catalog API](https://github.com/nktkt/gondlin/tree/main/NN/Examples/BugZoo/) and
[BugZoo overview](https://github.com/nktkt/gondlin/blob/main/NN/Examples/BugZoo/README.md) focus on the Gondlin fragment itself:
once a computation enters a typed Gondlin spec, shape changes, masks, token bounds, finite domain
choices, stateful normalization parameters, and backend semantics become named objects that can be
checked.

BugZoo should be read as the smallest public proof-of-concept for Gondlin's philosophy. Each card
...
- the Gondlin object that names the intended behavior;
...
[NN.Examples.BugZoo.ShapeAndBroadcast API](https://github.com/nktkt/gondlin/blob/main/NN/Examples/BugZoo/ShapeAndBroadcast.lean)
records one of Gondlin's core design choices: shapes belong in the object, not as an
...
matrix and still produce a plausible loss. In Gondlin, ordinary elementwise operations require the
...
[NN.Examples.BugZoo.StableLoss API](https://github.com/nktkt/gondlin/blob/main/NN/Examples/BugZoo/StableLoss.lean) is about losses that
...
Gondlin keeps two APIs separate. Logits should use `crossEntropyLogitsSpec`, which unfolds through
...
[NN.Examples.BugZoo.IgnoredLabelLoss API](https://github.com/nktkt/gondlin/blob/main/NN/Examples/BugZoo/IgnoredLabelLoss.lean) turns a
...
The Gondlin lesson is not "we copied every branch of a framework kernel." Gondlin exposes the
...
[NN.Examples.BugZoo.AutogradDomain API](https://github.com/nktkt/gondlin/blob/main/NN/Examples/BugZoo/AutogradDomain.lean) follows
...
Gondlin's card names the difference between "divide first, mask later" and "safe divide, then
...
[NN.Examples.BugZoo.AttentionMask API](https://github.com/nktkt/gondlin/blob/main/NN/Examples/BugZoo/AttentionMask.lean) is the catalog
...
`exp(-infinity) = 0`. Gondlin's ordinary attention spec then uses the equivalent hard masked
...
[NN.Examples.BugZoo.CompilerBoundary API](https://github.com/nktkt/gondlin/blob/main/NN/Examples/BugZoo/CompilerBoundary.lean) is the
...
[NN.Examples.BugZoo.FloatBoundary API](https://github.com/nktkt/gondlin/blob/main/NN/Examples/BugZoo/FloatBoundary.lean) is where we
...
Gondlin's answer is explicit modeling. `IEEE32Exec` is the executable bit level float32 model.
...
[NN.Examples.BugZoo.NormalizationState API](https://github.com/nktkt/gondlin/blob/main/NN/Examples/BugZoo/NormalizationState.lean)
...
that Gondlin's scalar normalization puts epsilon inside the variance term before square root.
...
[NN.Examples.BugZoo.BatchInvariance API](https://github.com/nktkt/gondlin/blob/main/NN/Examples/BugZoo/BatchInvariance.lean) is about
...
[NN.Examples.BugZoo.KVCache API](https://github.com/nktkt/gondlin/blob/main/NN/Examples/BugZoo/KVCache.lean) models cache accounting in
...
[NN.Examples.BugZoo.RoPEPosition API](https://github.com/nktkt/gondlin/blob/main/NN/Examples/BugZoo/RoPEPosition.lean) pairs naturally
...
[NN.Examples.BugZoo.TokenizerBoundary API](https://github.com/nktkt/gondlin/blob/main/NN/Examples/BugZoo/TokenizerBoundary.lean) marks
...
Gondlin's current contract is small but valuable: token IDs inside the verified fragment can be
...
BugZoo makes proof engineering more concrete. Instead of saying "Gondlin helps with ML bugs," we
...
Future BugZoo entries should follow the same rhythm: cite the real bug family, name the Gondlin
