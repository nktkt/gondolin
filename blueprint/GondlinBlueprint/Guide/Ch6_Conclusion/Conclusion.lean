...
Gondlin is still a research system, but the direction is clear. Modern ML systems increasingly
...
This is why Gondlin is organized as a framework rather than a collection of isolated examples.
...
- *Introduction* explains the problem Gondlin is built to address.
...
# How To State A Gondlin Result

When writing about Gondlin, the strongest statements have a simple form:
...
That rule keeps Gondlin extensible without making the system vague. Every addition should make at
least one object easier to run, inspect, check, or prove.

# Closing

Gondlin is useful because it keeps two activities connected: building neural-network systems and
...
and say what has been run, checked, proved, or assumed. If Gondlin makes that habit easier for
