...
#doc (Manual) "Gondlin vs. PyTorch" =>
%%%
tag := "gondlin_vs_pytorch"
%%%

PyTorch and Gondlin should not be compared as if they were trying to solve the same problem.
PyTorch is the right default for broad, high-performance ML engineering. Gondlin asks a narrower
...
- Gondlin optimizes for semantic access: typed shapes, explicit parameters, graph denotations,
  numerical meanings, and checker-friendly artifacts.
- A realistic workflow can use both systems.

For the main design dimensions:

- Main goal: PyTorch is a production ML engineering system. Gondlin is proof-aware ML
  infrastructure.
- Tensor shapes: PyTorch treats shapes dynamically. Gondlin often carries shapes in Lean types.
- Parameters: PyTorch modules own mutable state. Gondlin keeps architecture and payload as
  explicit values.
- Autograd: PyTorch provides a mature, broad engine. Gondlin exposes supported gradients through
  definitions that can be related to Lean semantics.
- Graphs: PyTorch graph tools support capture, compilation, and deployment. Gondlin uses a shared
  IR with a Lean denotation.
- Floating point: PyTorch follows backend behavior. Gondlin names real, float32, executable
...
Gondlin keeps the authoring style familiar, but separates the model description from the parameter
...
In Gondlin, the parameter bundle is already part of the ordinary data path. This makes it easier
...
Gondlin uses Lean's dependent types for the core tensor APIs. A tensor carries both a scalar type
and a shape:
...
That tradeoff is intentional. Gondlin gives up some of Python's dynamism in exchange for a
...
Gondlin keeps the same conceptual rhythm, but the training state is explicit. Gradients are
...
PyTorch's autograd is mature, broad, and deeply optimized. Gondlin's advantage is semantic access:
...
For the public training surface, see the [public API facade](https://github.com/nktkt/gondlin/blob/main/NN/API/Public.lean) and the
[training runtime](https://github.com/nktkt/gondlin/blob/main/NN/Runtime/Autograd/Train.lean).
...
Gondlin's graph path is designed for a different use case. A supported model can be lowered to
[NN.IR.Graph](https://github.com/nktkt/gondlin/blob/main/NN/IR/Graph.lean), a graph whose nodes carry operation tags and a Lean denotation. Verifier
...
This is where Gondlin most clearly differs from a PyTorch clone. The goal is not simply to run the
...
checkpoints, distributed training, debugging tools, and deployment practices. Gondlin therefore
...
The relevant APIs are [PyTorch export](https://github.com/nktkt/gondlin/blob/main/NN/Runtime/PyTorch/Export.lean),
[PyTorch import](https://github.com/nktkt/gondlin/blob/main/NN/Runtime/PyTorch/Import.lean), and the
[round-trip examples](https://github.com/nktkt/gondlin/blob/main/NN/Examples/Interop/PyTorch.lean). A narrow contract is easier to audit
...
Gondlin names the relevant layers separately:
...
Use Gondlin when the supported model family needs to be a Lean object: when shapes should be part
...
Export a known architecture and payload. Import it into Gondlin. Inspect the graph. Check the
certificate. State the remaining assumptions. Gondlin's contribution is the semantic layer around
