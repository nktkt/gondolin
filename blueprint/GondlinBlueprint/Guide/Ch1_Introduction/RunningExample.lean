...
When the model is built, Gondlin separates architecture from parameters. That is the first
...
PyTorch also has a parameter bundle, usually exposed through `state_dict`. Gondlin makes that idea
...
The graph chapters explain how Gondlin lowers model code into `NN.IR.Graph`, a DAG whose nodes
...
A theorem over real numbers and a float32 execution are not the same statement. Gondlin keeps the
...
That tiny example is the design in miniature. Gondlin does not try to guess which broadcast,
...
A certificate imported from outside Lean? Most Gondlin mistakes become easier to diagnose once
