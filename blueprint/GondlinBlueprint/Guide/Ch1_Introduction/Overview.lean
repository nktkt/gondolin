...
#doc (Manual) "What Gondlin Is" =>
%%%
tag := "overview"
%%%

Gondlin is a Lean 4 framework for neural networks whose computations can be run, inspected, and
...
certificates, and scientific claims. Gondlin gives these pieces a common place to meet: a typed
Lean development with runnable examples, graph semantics, explicit numerical models, and checkable
artifacts.

PyTorch remains the everyday training ecosystem for many projects. Gondlin focuses on the parts
...
The broad user import is [NN root API](https://github.com/nktkt/gondlin/blob/main/NN.lean). Narrower entry points include
[NN.Entrypoint.API](https://github.com/nktkt/gondlin/blob/main/NN/Entrypoint/API.lean),
[NN.Entrypoint.GraphSpec](https://github.com/nktkt/gondlin/blob/main/NN/Entrypoint/GraphSpec.lean),
[NN.Entrypoint.IR](https://github.com/nktkt/gondlin/blob/main/NN/Entrypoint/IR.lean), and
[NN.Entrypoint.Verification](https://github.com/nktkt/gondlin/blob/main/NN/Entrypoint/Verification.lean).

# What Gondlin Covers

At a high level, Gondlin provides three layers.
...
training mode, random state, scalar semantics, and the exact graph a verifier checked. Gondlin
...
hidden by broadcasting. In Gondlin's typed tensor APIs, the shape is part of the tensor type, so
...
At the top level, Gondlin provides a facade with PyTorch vocabulary over a shared internal model.
...
Gondlin separates fast execution from proof, but it does not treat them as unrelated worlds. For
...
is not the deployed computation. Gondlin's design pushes parameter payloads, graph lowering, and
