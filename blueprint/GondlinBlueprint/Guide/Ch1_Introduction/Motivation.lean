...
fragile if no one can say precisely how these representations agree. Gondlin is built around that
...
Gondlin is built around making those distinctions visible. The graph representation, the
...
correctness, and floating-point approximation are different properties, and Gondlin keeps them as
different theorems.

# Our Response

Gondlin keeps model code, graph semantics, executable float models, certificate checkers, and
...
follows, and which runtime or external assumptions remain. That is why Gondlin keeps runnable
