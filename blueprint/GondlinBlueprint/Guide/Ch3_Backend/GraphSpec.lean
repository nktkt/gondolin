...
The sequential DSL is defined in [NN.GraphSpec.Core API](https://github.com/nktkt/gondlin/blob/main/NN/GraphSpec/Core.lean).
...
The [GraphSpec DAG API](https://github.com/nktkt/gondlin/blob/main/NN/GraphSpec/DAG.lean) and
[GraphSpec DAG core](https://github.com/nktkt/gondlin/blob/main/NN/GraphSpec/DAG/Core.lean) define the shared-branch authoring surface.
...
- the Gondlin compiler supplies an executable program in the runtime layer.
...
#check NN.GraphSpec.ToGondlin.toSeq
```
...
GraphSpec model becomes a runnable Gondlin program or a lower-level DAG.

## Sequential Graph To DAG Model

The relevant definitions are [GraphSpec core](https://github.com/nktkt/gondlin/blob/main/NN/GraphSpec/Core.lean) and
[GraphSpec DAG core](https://github.com/nktkt/gondlin/blob/main/NN/GraphSpec/DAG/Core.lean).
...
## GraphSpec To Gondlin `nn.Sequential`

The lowering API is [GraphSpec to Gondlin](https://github.com/nktkt/gondlin/blob/main/NN/GraphSpec/ToGondlin.lean).

Important name:

- `NN.GraphSpec.ToGondlin.toSeq`

This path is used by the runnable
[GraphSpec tutorial](https://github.com/nktkt/gondlin/blob/main/NN/Examples/Advanced/GraphSpec/Tutorial.lean): author once in GraphSpec,
then plug the lowered model into the same `train.*` API used elsewhere in the guide.

## DAG Model To Runtime Model Zoo Wrappers

The runtime examples to open are [GraphSpec ResNet18](https://github.com/nktkt/gondlin/blob/main/NN/GraphSpec/Models/Gondlin/Resnet18.lean)
and [runtime FNO1D](https://github.com/nktkt/gondlin/blob/main/NN/Runtime/Autograd/Gondlin/Fno1d.lean).
...
#check NN.GraphSpec.Models.mlp_detInitParams_eq_gondlin_linear_inits
...
1. Read the [GraphSpec README](https://github.com/nktkt/gondlin/blob/main/NN/GraphSpec/README.md).
2. Open the [GraphSpec tutorial API](https://github.com/nktkt/gondlin/blob/main/NN/Examples/Advanced/GraphSpec/Tutorial.lean).
3. Run:
   `lake exe gondlin graphspec --backend compiled`
4. Read the [residual linear model API](https://github.com/nktkt/gondlin/blob/main/NN/GraphSpec/Models/ResidualLinear.lean).
5. Read the [ResNet-18 GraphSpec API](https://github.com/nktkt/gondlin/blob/main/NN/GraphSpec/Models/Resnet18.lean).
...
GraphSpec refines Gondlin's semantic model at the architecture boundary. In a Python codebase,
...
To inspect the implementation, start with the [GraphSpec README](https://github.com/nktkt/gondlin/blob/main/NN/GraphSpec/README.md), then
move to the sequential and DAG cores, then to the model zoo. The
[GraphSpec to Gondlin API](https://github.com/nktkt/gondlin/blob/main/NN/GraphSpec/ToGondlin.lean) gives the lowering path into
Gondlin `Seq`, and the
[GraphSpec Gondlin wrappers](https://github.com/nktkt/gondlin/tree/main/NN/GraphSpec/Models/Gondlin/) expose runtime model wrappers.
