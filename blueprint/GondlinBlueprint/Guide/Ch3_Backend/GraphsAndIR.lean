...
The word "graph" appears in several places in Gondlin. The distinction is worth making early.
...
The graph that matters most in Gondlin is the symbolic DAG carrying explicit operation tags.

The [IR graph API](https://github.com/nktkt/gondlin/blob/main/NN/IR/Graph.lean) introduces `NN.IR.OpKind`, `NN.IR.Node`, and
...
- [NN.IR.Graph API](https://github.com/nktkt/gondlin/blob/main/NN/IR/Graph.lean)
...
- [NN.IR.Infer API](https://github.com/nktkt/gondlin/blob/main/NN/IR/Infer.lean) / [NN.IR.Check API](https://github.com/nktkt/gondlin/blob/main/NN/IR/Check.lean)
...
- [NN.IR.Semantics API](https://github.com/nktkt/gondlin/blob/main/NN/IR/Semantics.lean)
...
Gondlin uses a small, explicit set of *executable* invariants for IR graphs. These checks are not
...
defined in [NN.IR.Semantics API](https://github.com/nktkt/gondlin/blob/main/NN/IR/Semantics.lean):
...
The [IR semantics API](https://github.com/nktkt/gondlin/blob/main/NN/IR/Semantics.lean) centers on two functions:
...
Start from the [IR graph API](https://github.com/nktkt/gondlin/blob/main/NN/IR/Graph.lean), the
[IR semantics API](https://github.com/nktkt/gondlin/blob/main/NN/IR/Semantics.lean), and the
[IR checking API](https://github.com/nktkt/gondlin/blob/main/NN/IR/Check.lean). That is enough context to make the rest of the
...
Gondlin tries to make these failures local. Instead of discovering a bad certificate at the end of
...
The [CROWN graph API](https://github.com/nktkt/gondlin/blob/main/NN/MLTheory/CROWN/Graph.lean) does not introduce a second graph
...
The [eager engine API](https://github.com/nktkt/gondlin/blob/main/NN/Runtime/Autograd/Engine/Core.lean) contains `Tape`, where each node
has runtime values and local VJP rules. The [compiled graph builder](https://github.com/nktkt/gondlin/blob/main/NN/Runtime/Autograd/Compiled/GraphM.lean)
...
On the proof side, the [tape soundness API](https://github.com/nktkt/gondlin/blob/main/NN/Proofs/Autograd/Tape/Algebra/Soundness.lean)
...
# Compiling Gondlin Programs To IR
...
The [Gondlin to IR compiler](https://github.com/nktkt/gondlin/blob/main/NN/Verification/Gondlin/Compile.lean) turns a forward model
...
[Gondlin correctness API](https://github.com/nktkt/gondlin/blob/main/NN/Verification/Gondlin/Correctness.lean) contains the helpers
...
> evaluating the compiled IR graph equals evaluating the Gondlin forward model.

For the full public Gondlin embedding (`Runtime.Autograd.Gondlin.Program`), this is a big
theorem because that embedding is higher order and tagless final.

Rather than forcing the public API to become an AST, Gondlin takes an additive path:
...
The [proved Gondlin compiler fragment](https://github.com/nktkt/gondlin/blob/main/NN/Verification/Gondlin/Proved.lean) defines a
...
Next: *Verification* (Gondlin to IR to bounds), *Floating-Point Semantics* (which scalar backend
the verifier uses), *Widgets* (infoview inspection of graphs and bounds).

# References

- Gondlin paper: https://arxiv.org/abs/2602.22631
