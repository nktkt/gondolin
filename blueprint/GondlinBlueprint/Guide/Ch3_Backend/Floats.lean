...
In Gondlin, "Float32" is not one object. It can mean an idealized rounded-real model used in
...
Gondlin separates four numerical layers:
...
about it require an agreement statement with the Lean-side model. That is how Gondlin can run real
...
#check Gondlin.Floats.Float32Mode
#check Gondlin.Floats.F32

-- Proof model: finite rounded-real binary32 arithmetic.
#check Gondlin.Floats.FP32
#check Gondlin.Floats.FP32.toReal
...
Gondlin states reduction facts around a finite evaluation tree rather than pretending every
parallel schedule is the same:

The [reductions API](https://github.com/nktkt/gondlin/blob/main/NN/Floats/IEEEExec/Reductions.lean) contains the long-form theorem
...
- [BridgeFP32 API](https://github.com/nktkt/gondlin/blob/main/NN/Floats/IEEEExec/BridgeFP32.lean) proves finite refinements such as
...
- [BridgeFP32Total API](https://github.com/nktkt/gondlin/blob/main/NN/Floats/IEEEExec/BridgeFP32Total.lean) packages total statements
...
- [SpecialRules API](https://github.com/nktkt/gondlin/blob/main/NN/Floats/IEEEExec/SpecialRules.lean) records NaN, infinity,
...
may provide an approximation with documented accuracy. Gondlin therefore keeps transcendental
claims explicit:
...
Gondlin names that assumption surface:
...
[Widgets API](https://github.com/nktkt/gondlin/blob/main/NN/Examples/Advanced/Widgets.lean), especially `#float32_view` and
`#float32_round_view`.
...
That split is the claim. Native execution is part of the workflow, and Gondlin makes the agreement
