...
The main thing to remember is that Gondlin has more than one runtime artifact. Eager execution
...
Gondlin keeps the rhythm but changes where the objects live:
...
VJP closures. It is the closest Gondlin analogue of PyTorch eager autograd.
...
Gondlin exposes one front end with two execution backends:
...
Gondlin therefore uses an existential wrapper, `Runtime.AnyTensor α`, which pairs a `Shape` with
the corresponding tensor. That preserves the strongly typed spec layer while still supporting
runtime-style tooling.

See the [runtime context API](https://github.com/nktkt/gondlin/blob/main/NN/Runtime/Context.lean)
...
In PyTorch terms, this is the part of the system behind `loss.backward()`. Gondlin makes the tape
...
The [widgets demo](https://github.com/nktkt/gondlin/blob/main/NN/Examples/Advanced/Widgets.lean)
...
- [compiled graph builder](https://github.com/nktkt/gondlin/blob/main/NN/Runtime/Autograd/Compiled/GraphM.lean)
- [compiled runtime core](https://github.com/nktkt/gondlin/blob/main/NN/Runtime/Autograd/Torch/Core.lean)
- [runtime overview](https://github.com/nktkt/gondlin/blob/main/NN/Runtime/Autograd/Overview.lean)

# IR Execution Bridge

For verification, Gondlin standardizes on `NN.IR.Graph`, the operation-tagged DAG described in
*Graphs and IR*. Runtime closures are good for execution, but a verifier needs explicit operation
tags, shapes, and parent ids.

The [IR execution compiler](https://github.com/nktkt/gondlin/blob/main/NN/Runtime/Autograd/Compiled/IRExec.lean)
...
- [autograd proof overview](https://github.com/nktkt/gondlin/blob/main/NN/Proofs/Autograd/Overview.lean)
- [tape algebra soundness API](https://github.com/nktkt/gondlin/blob/main/NN/Proofs/Autograd/Tape/Algebra/Soundness.lean)
- [runtime autograd link API](https://github.com/nktkt/gondlin/blob/main/NN/Proofs/Autograd/Runtime/Link.lean)
...
- [AutogradBasics](https://github.com/nktkt/gondlin/blob/main/NN/Examples/Quickstart/AutogradBasics.lean)
- [SimpleMlpTrain](https://github.com/nktkt/gondlin/blob/main/NN/Examples/Quickstart/SimpleMlpTrain.lean)
- [MinibatchMlpTrain](https://github.com/nktkt/gondlin/blob/main/NN/Examples/Quickstart/MinibatchMlpTrain.lean)
