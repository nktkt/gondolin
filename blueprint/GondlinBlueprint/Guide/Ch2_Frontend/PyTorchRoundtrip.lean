...
Many useful Gondlin workflows will still involve Python. A model may be trained in PyTorch
because the dataset, optimizer, or engineering environment belongs there. The question for
Gondlin is what happens after that training run. Can the result come back into Lean as a checked
artifact, with names, shapes, and parameter order made explicit?

The round trip is deliberately narrow. Gondlin does not import arbitrary `nn.Module` objects. It
...
- [NN.Examples.Interop.PyTorch.Roundtrip API](https://github.com/nktkt/gondlin/blob/main/NN/Examples/Interop/PyTorch/Roundtrip.lean)
- [NN.Examples.Interop.PyTorch.TorchExportSmoke API](https://github.com/nktkt/gondlin/blob/main/NN/Examples/Interop/PyTorch/TorchExportSmoke.lean)
- [Torch IR and PyTorch example](https://github.com/nktkt/gondlin/blob/main/NN/Examples/Advanced/TorchIRPyTorch.lean)

The main declarations for this boundary are
[PyTorch export](https://github.com/nktkt/gondlin/blob/main/NN/Runtime/PyTorch/Export.lean),
[PyTorch import](https://github.com/nktkt/gondlin/blob/main/NN/Runtime/PyTorch/Import.lean),
[interop examples](https://github.com/nktkt/gondlin/blob/main/NN/Examples/Interop/PyTorch.lean), and the spec pages for
[modules](https://github.com/nktkt/gondlin/blob/main/NN/Spec/Module.lean) and [models](https://github.com/nktkt/gondlin/blob/main/NN/Spec/Models.lean).
...
If any of those checks fail, the artifact stops at the boundary. It does not become a Gondlin
model by accident.
...
bundle used by the rest of Gondlin.
...
Lean then reconstructs the typed parameter bundle that the rest of Gondlin expects. The JSON file
...
- Lean entrypoint: [PyTorch roundtrip API](https://github.com/nktkt/gondlin/blob/main/NN/Examples/Interop/PyTorch/Roundtrip.lean)
- Python: [MLP training script](https://github.com/nktkt/gondlin/blob/main/NN/Examples/Interop/PyTorch/MLP/train_mlp.py)
...
- Lean entrypoint: [PyTorch roundtrip API](https://github.com/nktkt/gondlin/blob/main/NN/Examples/Interop/PyTorch/Roundtrip.lean)
- Python: [CNN training script](https://github.com/nktkt/gondlin/blob/main/NN/Examples/Interop/PyTorch/CNN/train_cnn.py)
...
- Lean entrypoint: [PyTorch roundtrip API](https://github.com/nktkt/gondlin/blob/main/NN/Examples/Interop/PyTorch/Roundtrip.lean)
- Python: [Transformer training script](https://github.com/nktkt/gondlin/blob/main/NN/Examples/Interop/PyTorch/Transformer/train_transformer.py)
...
# Gondlin, IR, and Generated PyTorch Code

A second interop path complements the JSON round-trip workflow:

- file: [Torch IR and PyTorch example](https://github.com/nktkt/gondlin/blob/main/NN/Examples/Advanced/TorchIRPyTorch.lean)

This example compiles a Gondlin model to the shared IR and then emits runnable PyTorch code for a
...
save_gondlin_json(model, "mlp.json")
```
...
enter the Gondlin world with typed parameters and auditable metadata. Use the runtime chapters
...
Binary32 claims still require the relevant Gondlin float backend and the theorems in the
floating-point chapters.

For a PyTorch-side-by-side comparison, read *Gondlin vs PyTorch*.
...
- Gondlin paper (George et al., 2026): project overview, shared IR architecture, and verification-driven
