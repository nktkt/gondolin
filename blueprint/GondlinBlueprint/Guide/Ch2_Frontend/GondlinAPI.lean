...
For a first pass through Gondlin, read the API this way:
...
- `NN.API.Gondlin` is the namespace that the runtime surface re-exports for ordinary code.
...
Closest Gondlin analogue to `torch.utils.data` plus a small, predictable preprocessing surface:
...
The most important design decision is the boundary format: Gondlin expects numeric tensors in
...
This is worth saying explicitly because Gondlin examples now cover both small tutorial fixtures and
data backed training runs.

# The Runtime Surface

`NN.API.Runtime` and `NN.API.Gondlin` expose the lower layer machinery that underlies the public
...
- `API.Gondlin.Schedulers.constant`
- `API.Gondlin.Schedulers.step`
- `API.Gondlin.Schedulers.exponential`
...
Gondlin uses the same public model code with two execution styles:
...
`gondlin` runner; the `NN.API` types do not change. Only the runtime buffer implementation does. See
...
2. `NN.API.Runtime` or `NN.API.Gondlin` only when the lower layer API is the topic.
...
then *Gondlin vs PyTorch* for interop.
