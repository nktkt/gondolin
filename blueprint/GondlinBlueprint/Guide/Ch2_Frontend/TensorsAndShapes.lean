...
The first thing to understand in Gondlin is that a tensor's shape is part of the object, not only
runtime metadata. This is a small change with a large effect. If a model returns predictions of
shape `[batch, 1]` and the target has shape `[batch]`, Gondlin does not silently guess the
...
In Gondlin, the same idea is written with shape information in the type:
...
That is the basic pattern throughout Gondlin: the syntax feels familiar, but the shape is
...
For common ML shapes, Gondlin also provides readable aliases:
...
Gondlin uses the scalar element type as the “dtype”. For example:
...
- `Tensor Gondlin.Floats.IEEE32Exec s` is an executable IEEE-754 binary32 tensor.
...
Here is the shape mismatch from the opening, written as a Gondlin object:
...
Gondlin does not decide whether this should be a squeeze, a reshape, a one-hot encoding, or a
...
backend scalar in the model code. The [runtime module API](https://github.com/nktkt/gondlin/blob/main/NN/Runtime/Autograd/Gondlin/Module.lean)
...
Gondlin's tensor layer makes a few choices that are worth understanding early.
...
2. *Dynamic inputs are allowed at the boundary.* File loaders, JSON artifacts, and CLI data often
   arrive with runtime dimensions. Gondlin checks those dimensions as data and then packages them
...
In Gondlin, the intended shapes are visible at the definition site:
...
The relevant design note is that Gondlin does not force every input format to be perfectly typed at
...
## How Gondlin Differs from Plain PyTorch

The PyTorch version checks shapes when you run the code. Gondlin checks many of them when the file
...
wrong axis order or layout. In PyTorch you often discover that after a runtime error; in Gondlin the type
...
- [NN.Examples.Quickstart.TensorBasics API](https://github.com/nktkt/gondlin/blob/main/NN/Examples/Quickstart/TensorBasics.lean)
- [NN.Examples.Advanced.Tensors.Basic API](https://github.com/nktkt/gondlin/blob/main/NN/Examples/Advanced/Tensors/Basic.lean)
...
Gondlin’s tensor layer is not just a convenience wrapper. It is strong because the shape index is
...
[tensor proof API](https://github.com/nktkt/gondlin/blob/main/NN/Proofs/Tensor/Basic.lean).
