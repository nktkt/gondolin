...
Verification in Gondlin is not a single button. It is a family of claims about explicit objects.
...
The usual object is an [NN.IR.Graph](https://github.com/nktkt/gondlin/blob/main/NN/IR/Graph.lean)
...
- start from a Gondlin API program, GraphSpec model, or imported artifact;
...
- `LiRPA`: linear relaxation-based perturbation analysis, the broader family that includes IBP and
  CROWN methods.
- `IEEE32Exec`: Gondlin's executable IEEE-754 binary32 kernel in Lean.
- `FP32`: Gondlin's float32 model used in proofs, instantiated as rounded real semantics.

# A Map Of Claim Types

Gondlin's verification support is easiest to read as a table of objects and support mechanisms:
...
A good way to read Gondlin verification output is as a ladder:
...
Gondlin is not just a verifier with a convenient front end. Runtime compilation and verification
...
and a CUDA kernel may produce runtime values. Gondlin treats those systems as producers: they do
...
- *Process wrappers*: [external process helpers](https://github.com/nktkt/gondlin/blob/main/NN/Runtime/External/Process.lean) and
  [Julia helpers](https://github.com/nktkt/gondlin/blob/main/NN/Runtime/External/Julia.lean) resolve commands, run tools, capture JSON,
...
That last item is exactly how to read FlashAttention in Gondlin. The spec theorem
...
Gondlin makes that pattern visible in the module structure. The shared object is `NN.IR.Graph`,
...
Gondlin's core bound propagation passes follow the LiRPA family of methods, in which the verifier
...
Gondlin stores these objects in flattened form for efficiency and reuse across operators. In
...
[AlphaCROWN certificate API](https://github.com/nktkt/gondlin/blob/main/NN/MLTheory/CROWN/Cert/AlphaCROWN.lean) contains this transfer
rule, especially the
...
# Worked Example: Gondlin, IR, and IBP (Tiny MLP)

The simplest verification pipeline in Gondlin is easy to state: define a model with the
Gondlin API, compile its forward pass to the canonical IR with operation tags (`NN.IR.Graph`), seed an
input uncertainty set, run a verifier pass such as IBP, and then read off the output bounds at the
output node id.

The runnable demo is
[Gondlin IBP](https://github.com/nktkt/gondlin/blob/main/NN/Examples/Verification/Gondlin/GondlinIBP.lean), and the
matching CLI entrypoint is `lake exe verify -- gondlin-ibp`.

Below is the essential core of that file, trimmed to the minimum needed to follow the pipeline:

```
import NN
import NN.Verification.Gondlin.Compile
...
Two Gondlin invariants are visible immediately:
...
ops and with a backward pass that tightens the bound for a particular output margin. Gondlin has
the same general shape, but the implementation is split between a Lean LiRPA engine
for a curated op set ([NN/MLTheory/CROWN](https://github.com/nktkt/gondlin/tree/main/NN/MLTheory/CROWN/)) and checkers that compare Lean
recomputation to JSON artifacts exported from Python tooling
([NN/Verification/Cert](https://github.com/nktkt/gondlin/tree/main/NN/Verification/Cert/)).
...
for good α values. Gondlin's Lean transfer rule does not pretend to have run that optimizer;
...
Gondlin's current α,β checker uses exactly that narrow interpretation. In the
[α,β-CROWN certificate API](https://github.com/nktkt/gondlin/blob/main/NN/MLTheory/CROWN/Cert/AlphaBetaCROWN.lean), β is represented as a
...
- The [CROWN node certificate API](https://github.com/nktkt/gondlin/blob/main/NN/Verification/Cert/CROWNNodeCert.lean) checks per node
  α-CROWN affine bounds by recomputing the Lean transfer step.
- The [α,β CROWN node certificate API](https://github.com/nktkt/gondlin/blob/main/NN/Verification/Cert/CROWNNodeCertAlphaBeta.lean)
...
Gondlin's current checker does not accept the split just because a JSON file says so. It checks
...
This is the natural place to connect Gondlin to the adversarial-robustness literature and to the
...
The implementation is organized around the [PINN CLI](https://github.com/nktkt/gondlin/blob/main/NN/Verification/PINN/CLI.lean), the
[PINN certificate checker](https://github.com/nktkt/gondlin/blob/main/NN/Verification/PINN/Certificate.lean), the
[PINN dataset checker](https://github.com/nktkt/gondlin/blob/main/NN/Verification/PINN/DatasetCheck.lean), and the commands
`pinn-cli`, `pinn-cert`, and `pinn-dataset-check`.

This is one of the clearest places where Gondlin's derivative-aware graph propagation is doing
...
- [ODE API](https://github.com/nktkt/gondlin/blob/main/NN/Verification/ODE.lean)
- [ODE verifier API](https://github.com/nktkt/gondlin/blob/main/NN/Verification/ODE/Verify.lean)
- CLI tool: `ode`
...
Not every useful workflow in Gondlin starts from a model written natively in Lean.

Relevant tools and examples:

- `abcrown-leaf`
- `vnncomp-mnistfc`
- [NN/Examples/Verification/VNNComp](https://github.com/nktkt/gondlin/tree/main/NN/Examples/Verification/VNNComp/)

These workflows matter because they show how Gondlin interfaces with existing verifier ecosystems
...
- [Lyapunov oracle boundary](https://github.com/nktkt/gondlin/blob/main/NN/MLTheory/CROWN/Lyapunov/Oracle.lean)
- [Lyapunov verification theorems](https://github.com/nktkt/gondlin/blob/main/NN/MLTheory/CROWN/Lyapunov/Verification.lean)
- [two stage Lyapunov runner](https://github.com/nktkt/gondlin/blob/main/NN/MLTheory/CROWN/Lyapunov/TwoStage/Run.lean)
...
- Gondlin intentionally keeps *values/parameters out of the graph*, because different backends want
...
#check NN.Verification.Gondlin.Proved.compileForward1_wellFormed
#check NN.Verification.Gondlin.Proved.evalCompiledForward1_eq_evalForward1
```

# Supported Proved Fragment (Today)

The authoritative statement of the current proved IBP fragment appears at the top of
[NN.MLTheory.CROWN.Proofs.GraphCertSoundness API](https://github.com/nktkt/gondlin/blob/main/NN/MLTheory/CROWN/Proofs/GraphCertSoundness.lean).

At present, the soundness development is centered on the core graph dialect used by the bundled
Gondlin verification demos:
...
In Gondlin documentation, the word "certificate" is used in two related but importantly different
senses.
...
- [IBP certificate checks](https://github.com/nktkt/gondlin/blob/main/NN/Verification/Cert/IBPCert.lean)
- [per node IBP checks](https://github.com/nktkt/gondlin/blob/main/NN/Verification/Cert/IBPNodeCert.lean)
- [per node α-CROWN checks](https://github.com/nktkt/gondlin/blob/main/NN/Verification/Cert/CROWNNodeCert.lean)
- [per node α,β-CROWN checks](https://github.com/nktkt/gondlin/blob/main/NN/Verification/Cert/CROWNNodeCertAlphaBeta.lean)
...
The [α,β-CROWN leaf checker](https://github.com/nktkt/gondlin/blob/main/NN/Verification/Cert/AbCrownLeafCert.lean) handles that boundary
...
First run: `gondlin-ibp`.

What these runs do, roughly:

1. Build or import a Gondlin model.
...
the [verification CLI API](https://github.com/nktkt/gondlin/blob/main/NN/Verification/CLI.lean) for the authoritative list and defaults:

- Gondlin, IR, and IBP demo: `gondlin-ibp`
- Gondlin, IR, and IBP plus CROWN demo (ops like softmax and `mse_loss`): `gondlin-crown-ops`
- Gondlin "tiny transformer" demo: `gondlin-transformer-ibp`
- Margin / robustness demos: `gondlin-robustness`, `digits`, `margin-cert`
...
- End-to-end Gondlin MLP workflow: `gondlin-mlp-workflow`
...
3. the [certificate checker API](https://github.com/nktkt/gondlin/blob/main/NN/Verification/Cert.lean)
...
- Canonical graph with operation tags: [IR graph API](https://github.com/nktkt/gondlin/blob/main/NN/IR/Graph.lean)
- Gondlin forward compiler to IR: [compiler API](https://github.com/nktkt/gondlin/blob/main/NN/Verification/Gondlin/Compile.lean)
- IR evaluation helpers: [Gondlin correctness API](https://github.com/nktkt/gondlin/blob/main/NN/Verification/Gondlin/Correctness.lean)
- Proved forward fragment: [proved compiler fragment](https://github.com/nktkt/gondlin/blob/main/NN/Verification/Gondlin/Proved.lean)
...
- Graph payloads and passes: [CROWN graph API](https://github.com/nktkt/gondlin/blob/main/NN/MLTheory/CROWN/Graph.lean)
- Operators and transfer rules: [CROWN operators source tree](https://github.com/nktkt/gondlin/tree/main/NN/MLTheory/CROWN/Operators/)
- Model-specific helpers: [CROWN models source tree](https://github.com/nktkt/gondlin/tree/main/NN/MLTheory/CROWN/Models/)
- Certificate-side definitions: [CROWN cert source tree](https://github.com/nktkt/gondlin/tree/main/NN/MLTheory/CROWN/Cert/)
- Soundness proofs: [CROWN proof source tree](https://github.com/nktkt/gondlin/tree/main/NN/MLTheory/CROWN/Proofs/)

## CLI registry

- Unified entrypoint: [verification CLI API](https://github.com/nktkt/gondlin/blob/main/NN/Verification/CLI.lean)
  - This is what `lake exe verify` runs.

## Certificate checkers (JSON compare)

- [certificate checker API](https://github.com/nktkt/gondlin/blob/main/NN/Verification/Cert.lean): checks externally produced JSON artifacts by
  Lean recomputation with explicit tolerance comparisons.

## Curated examples

- [NN/Examples/Verification](https://github.com/nktkt/gondlin/tree/main/NN/Examples/Verification/)
  - Small bundled fixtures and tutorial checkers registered into the `verify` CLI.

## PINN and ODE application code

- PINN helpers and CLIs: [PINN API](https://github.com/nktkt/gondlin/blob/main/NN/Verification/PINN.lean)
- ODE support: [ODE API](https://github.com/nktkt/gondlin/blob/main/NN/Verification/ODE.lean) and
  [ODE proof source tree](https://github.com/nktkt/gondlin/tree/main/NN/Proofs/Verification/ODE/)

## Lyapunov / controller workflows

- [Lyapunov oracle boundary](https://github.com/nktkt/gondlin/blob/main/NN/MLTheory/CROWN/Lyapunov/Oracle.lean) and
  [two stage Lyapunov runner](https://github.com/nktkt/gondlin/blob/main/NN/MLTheory/CROWN/Lyapunov/TwoStage/Run.lean): oracle style
  and two stage controller / Lyapunov developments.

# Trust Boundaries (What Is Executable vs Proved vs Assumed)

Gondlin marks three different kinds of claim explicitly:

## 1) Executable in Lean (Fast Path)

- IR evaluation ([NN.IR.Semantics API](https://github.com/nktkt/gondlin/blob/main/NN/IR/Semantics.lean)) is executable.
...
Gondlin has a soundness proof development for a supported subset of the graph dialect:

- Proof entrypoint: [graph certificate soundness API](https://github.com/nktkt/gondlin/blob/main/NN/MLTheory/CROWN/Proofs/GraphCertSoundness.lean)
...
Gondlin is explicit about what is proved in Lean and what is checked as an external artifact:
...
Lean source: [Gondlin compile proof fragment](https://github.com/nktkt/gondlin/blob/main/NN/Verification/Gondlin/Proved.lean)
...
Lean source: [graph certificate soundness](https://github.com/nktkt/gondlin/blob/main/NN/MLTheory/CROWN/Proofs/GraphCertSoundness.lean)
...
to cite is [graph certificate soundness](https://github.com/nktkt/gondlin/blob/main/NN/MLTheory/CROWN/Proofs/GraphCertSoundness.lean).
...
- [certificate checkers](https://github.com/nktkt/gondlin/blob/main/NN/Verification/Cert.lean)
- [verification CLI](https://github.com/nktkt/gondlin/blob/main/NN/Verification/CLI.lean)
...
Ecosystem tool Gondlin interoperates with:
