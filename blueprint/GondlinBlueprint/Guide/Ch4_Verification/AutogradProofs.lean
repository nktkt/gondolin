...
JAX made it practical at enormous scale. Gondlin keeps the same programmer intuition and puts an
explicit theorem underneath it.

The difference is not that Gondlin "has autograd" while PyTorch and JAX do not. They do, and they
...
Gondlin's proof tree asks a more explicit question:

> For the supported graph fragment, does the reverse pass compute the adjoint of the derivative of
> the forward denotation?

That is the proof architecture in [NN/Proofs/Autograd](https://github.com/nktkt/gondlin/tree/main/NN/Proofs/Autograd/). We built it in layers so the
...
Gondlin does not claim those systems are careless. We use the same mental model because it is
...
[NN.Proofs.Autograd.Tape.Algebra.Soundness API](https://github.com/nktkt/gondlin/blob/main/NN/Proofs/Autograd/Tape/Algebra/Soundness.lean).
...
walk over a dynamic graph and accumulates cotangents into inputs. Gondlin's algebraic graph does
...
[Frechet derivative bridge](https://github.com/nktkt/gondlin/blob/main/NN/Proofs/Autograd/Tape/Core/FDeriv.lean) provides that link.
...
division, logarithms, and square roots all have domain or nondifferentiability issues. Gondlin
...
- [NN.Proofs.Autograd.FDeriv.Softmax API](https://github.com/nktkt/gondlin/blob/main/NN/Proofs/Autograd/FDeriv/Softmax.lean)
- [NN.Proofs.Autograd.FDeriv.LogSoftmax API](https://github.com/nktkt/gondlin/blob/main/NN/Proofs/Autograd/FDeriv/LogSoftmax.lean)
...
the derivative law for Gondlin's mathematical definition.

# MLP And MSE Gradients: A Concrete Scalar Loss Story

The [MLP/MSE derivative API](https://github.com/nktkt/gondlin/blob/main/NN/Proofs/Autograd/FDeriv/MlpMse.lean)
...
convention at zero; Gondlin's real analysis statement names the differentiability condition
instead.
...
[runtime approximation proof API](https://github.com/nktkt/gondlin/tree/main/NN/Proofs/RuntimeApprox/).
...
- [NN.Proofs.Autograd.Tape.Ops.Attention.ScaledDotProduct API](https://github.com/nktkt/gondlin/blob/main/NN/Proofs/Autograd/Tape/Ops/Attention/ScaledDotProduct.lean)
- [NN.Proofs.Autograd.Tape.Ops.Attention.MultiHeadSelfAttention API](https://github.com/nktkt/gondlin/blob/main/NN/Proofs/Autograd/Tape/Ops/Attention/MultiHeadSelfAttention.lean)
- [NN.Proofs.Autograd.Tape.Ops.Transformer.PostNorm API](https://github.com/nktkt/gondlin/blob/main/NN/Proofs/Autograd/Tape/Ops/Transformer/PostNorm.lean)
- [NN.Proofs.Autograd.Tape.Ops.Transformer.ResidualAttention API](https://github.com/nktkt/gondlin/blob/main/NN/Proofs/Autograd/Tape/Ops/Transformer/ResidualAttention.lean)
- [NN.Proofs.Autograd.Tape.Ops.Transformer.FeedForward API](https://github.com/nktkt/gondlin/blob/main/NN/Proofs/Autograd/Tape/Ops/Transformer/FeedForward.lean)
- [NN.Proofs.Autograd.Tape.Ops.Recurrent.ElmanCell API](https://github.com/nktkt/gondlin/blob/main/NN/Proofs/Autograd/Tape/Ops/Recurrent/ElmanCell.lean)
...
[NN.Proofs.Autograd.Training.StepAlgebra API](https://github.com/nktkt/gondlin/blob/main/NN/Proofs/Autograd/Training/StepAlgebra.lean).
...
1. Start with the [tape algebra soundness API](https://github.com/nktkt/gondlin/blob/main/NN/Proofs/Autograd/Tape/Algebra/Soundness.lean)
   and find `Graph.backprop_correct`.
2. Move to the [Fréchet derivative tape API](https://github.com/nktkt/gondlin/blob/main/NN/Proofs/Autograd/Tape/Core/FDeriv.lean) and find
   `Graph.backpropVec_eq_adjoint_fderiv`.
3. Read a local derivative file such as
   the [softmax derivative API](https://github.com/nktkt/gondlin/blob/main/NN/Proofs/Autograd/FDeriv/Softmax.lean) or
   the [log-softmax derivative API](https://github.com/nktkt/gondlin/blob/main/NN/Proofs/Autograd/FDeriv/LogSoftmax.lean).
4. Read a file for a model block, such as
   the [Transformer post-norm API](https://github.com/nktkt/gondlin/blob/main/NN/Proofs/Autograd/Tape/Ops/Transformer/PostNorm.lean).
5. Finish with the [training step algebra API](https://github.com/nktkt/gondlin/blob/main/NN/Proofs/Autograd/Training/StepAlgebra.lean)
...
Gondlin turns those engineering moves into named Lean statements instead of leaving them as a
...
- that every Gondlin runtime path uses only proved nodes;
