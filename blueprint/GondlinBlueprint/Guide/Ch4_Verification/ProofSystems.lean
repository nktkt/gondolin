...
PyTorch, JAX, TensorFlow, XLA, TVM, CUDA, Gymnasium, and α,β-CROWN are excellent tools. Gondlin is
not treating those tools as a problem. The important difference is where the mathematical claim is
stated.

- *PyTorch autograd*: ordinary use relies on the dynamic autograd engine and kernels; Gondlin proves
...
- *`torch.compile` and graph lowering*: ordinary use relies on compiler passes plus regression tests;
  Gondlin proves that successful lowering from IR to executable graph preserves denotation for
  supported ops.
- *CUDA kernels*: ordinary use relies on native code, vendor libraries, and tests; Gondlin keeps
...
- *α,β-CROWN*: ordinary use relies on external optimization/search or checks solver outputs informally;
  Gondlin parses or recomputes selected certificate steps and names the remaining oracle boundary.
- *Gymnasium/RL environments*: ordinary use relies on Python environment records; Gondlin can check
  emitted transition records against a Lean boundary contract.
- *Float32 execution*: ordinary use relies on backend arithmetic; Gondlin connects executable
  binary32 semantics, intervals, and explicit approximation theorems where the bridge is present.

That is the recurring Gondlin pattern:
...
[IR execution correctness API](https://github.com/nktkt/gondlin/blob/main/NN/Runtime/Autograd/Compiled/IRExec/Correctness/SemanticEquivalence.lean).
...
- In Gondlin's supported IRExec fragment, the compiler's forward result is tied to the IR
...
- The [IRExec common API](https://github.com/nktkt/gondlin/blob/main/NN/Runtime/Autograd/Compiled/IRExec/Correctness/Common.lean) contains shared
  infrastructure for dynamic values, compiled contexts, and finishing a node step.
- The [semantic equivalence common API](https://github.com/nktkt/gondlin/blob/main/NN/Runtime/Autograd/Compiled/IRExec/Correctness/SemanticEquivalenceCommon.lean)
  contains helper lemmas used by the recursive proof.
- The [semantic equivalence op cases API](https://github.com/nktkt/gondlin/blob/main/NN/Runtime/Autograd/Compiled/IRExec/Correctness/SemanticEquivalenceOpCases.lean)
...
- `Correctness/Ops/*` contains smaller branches by op family: activations, constants, elementwise,
  ...
  ops.
- The [semantic equivalence theorem API](https://github.com/nktkt/gondlin/blob/main/NN/Runtime/Autograd/Compiled/IRExec/Correctness/SemanticEquivalence.lean)
  ties the cases together into `execGraphOfIR_semantics_eq`.

This is one of the places where Gondlin is genuinely doing something stronger than a normal ML
...
Gondlin splits autograd into proof layers:
...
[real autograd correctness](https://github.com/nktkt/gondlin/blob/main/NN/Proofs/Autograd/Core/RealCorrectness.lean) and
[semiring autograd correctness](https://github.com/nktkt/gondlin/blob/main/NN/Proofs/Autograd/Core/SemiringCorrectness.lean), prove
primitive derivative rules. The global tape APIs, such as
[tape algebra soundness](https://github.com/nktkt/gondlin/blob/main/NN/Proofs/Autograd/Tape/Algebra/Soundness.lean) and
[Fréchet derivative soundness](https://github.com/nktkt/gondlin/blob/main/NN/Proofs/Autograd/Tape/Core/FDeriv.lean), lift those rules
to whole graphs. Model and training APIs, such as
[MLP/MSE derivative facts](https://github.com/nktkt/gondlin/blob/main/NN/Proofs/Autograd/FDeriv/MlpMse.lean) and
[training step algebra](https://github.com/nktkt/gondlin/blob/main/NN/Proofs/Autograd/Training/StepAlgebra.lean), specialize the theorem
to concrete losses and optimizer style updates.

A helpful way to say the contrast is:

> PyTorch gives us `loss.backward()`. Gondlin proves, for supported graph fragments,
...
- The [softmax derivative API](https://github.com/nktkt/gondlin/blob/main/NN/Proofs/Autograd/FDeriv/Softmax.lean) proves the derivative and adjoint
  identities behind softmax.
- The [log-softmax derivative API](https://github.com/nktkt/gondlin/blob/main/NN/Proofs/Autograd/FDeriv/LogSoftmax.lean) records the log-softmax
  derivative account used by stable losses.
- The [attention proof API](https://github.com/nktkt/gondlin/tree/main/NN/Proofs/Models/Attention/) contains attention invariants such as
  weight normalization and causal mask properties.
- The [Transformer post-norm API](https://github.com/nktkt/gondlin/blob/main/NN/Proofs/Autograd/Tape/Ops/Transformer/PostNorm.lean) contains
  post-norm Transformer sublayer VJP theorems and the bridge theorem for two sublayers with post norm
  blocks.
- The [Elman cell API](https://github.com/nktkt/gondlin/blob/main/NN/Proofs/Autograd/Tape/Ops/Recurrent/ElmanCell.lean) gives a
...
the same as all of GPT-2. The value is that Gondlin makes those boundaries precise instead of
...
code as if no bridge were needed. Gondlin has a separate runtime approximation tree because we do
...
The [runtime approximation core](https://github.com/nktkt/gondlin/tree/main/NN/Proofs/RuntimeApprox/Core/) defines tolerances and
spec/runtime approximation relations. The
[forward graph approximation API](https://github.com/nktkt/gondlin/blob/main/NN/Proofs/RuntimeApprox/Graph/ForwardApprox.lean) and
[backward graph approximation API](https://github.com/nktkt/gondlin/blob/main/NN/Proofs/RuntimeApprox/Graph/BackwardApprox.lean)
propagate those relations through graph execution. The
[normal form operator API](https://github.com/nktkt/gondlin/blob/main/NN/Proofs/RuntimeApprox/NF/Ops.lean),
[convolution forward API](https://github.com/nktkt/gondlin/blob/main/NN/Proofs/RuntimeApprox/NF/ConvForward.lean), and
[convolution backward API](https://github.com/nktkt/gondlin/blob/main/NN/Proofs/RuntimeApprox/NF/ConvBackward.lean) give the operator
cases. The [scale approximation API](https://github.com/nktkt/gondlin/tree/main/NN/Proofs/RuntimeApprox/Scale/) adds absolute and
relative tolerance tracking, and the
[FP32 CROWN bridge API](https://github.com/nktkt/gondlin/blob/main/NN/Proofs/RuntimeApprox/FP32/CROWN.lean) connects approximation
...
The RL stack is another place where Gondlin should say more than "we implemented PPO."
The codebase has both runtime RL tools and MDP facts in Lean.

Runtime side:

- [NN.Runtime.RL.Core API](https://github.com/nktkt/gondlin/blob/main/NN/Runtime/RL/Core.lean) defines transition and rollout data.
- [NN/Runtime/RL/PPO](https://github.com/nktkt/gondlin/tree/main/NN/Runtime/RL/PPO/) covers PPO rollout collection.
- [NN/Runtime/RL/PolicyGradient](https://github.com/nktkt/gondlin/tree/main/NN/Runtime/RL/PolicyGradient/) contains policy gradient and
  PPO objective code.
- [NN.Runtime.RL.Replay API](https://github.com/nktkt/gondlin/blob/main/NN/Runtime/RL/Replay.lean) defines replay buffers.
- [NN/Runtime/RL/Gymnasium](https://github.com/nktkt/gondlin/tree/main/NN/Runtime/RL/Gymnasium/) is the external Python environment
  boundary.

Proof side:

- [NN.Proofs.RL.MDP API](https://github.com/nktkt/gondlin/blob/main/NN/Proofs/RL/MDP.lean) and
  the [finite stochastic MDP API](https://github.com/nktkt/gondlin/blob/main/NN/Proofs/RL/FiniteStochasticMDP.lean) prove monotonicity,
  contraction, uniqueness, and fixed point/error facts for Bellman operators.
- [NN.Proofs.RL.Envs.GridWorld API](https://github.com/nktkt/gondlin/blob/main/NN/Proofs/RL/Envs/GridWorld.lean) proves simple
  GridWorld transition facts and bridges deterministic successors to finite stochastic MDP rows.
- [NN.Proofs.RL.Replay API](https://github.com/nktkt/gondlin/blob/main/NN/Proofs/RL/Replay.lean) proves structural properties of replay
  buffer push/size behavior.
- [NN/Proofs/RL/Floats](https://github.com/nktkt/gondlin/tree/main/NN/Proofs/RL/Floats/) connects discounted backups and TD residuals to
  executable float32 formulas under explicit finiteness checks.
- [NN.Proofs.RL.Boundary API](https://github.com/nktkt/gondlin/blob/main/NN/Proofs/RL/Boundary.lean) proves facts about checked boundary
  contracts.

Compared with ordinary Gymnasium usage, the difference is again the boundary. A Python environment
can return an observation and reward; Gondlin can additionally check that a transition record has
...
Gondlin has generative model examples and theory declarations, with a deliberately narrow claim:
...
- [NN.MLTheory.Generative.Diffusion API](https://github.com/nktkt/gondlin/blob/main/NN/MLTheory/Generative/Diffusion.lean) collects the
  diffusion theory surface.
- The [forward Gaussian API](https://github.com/nktkt/gondlin/blob/main/NN/MLTheory/Generative/Diffusion/ForwardGaussian.lean) states that
  affine forward noising of a standard Gaussian remains Gaussian.
- The [diffusion samplers API](https://github.com/nktkt/gondlin/blob/main/NN/MLTheory/Generative/Diffusion/Samplers.lean) records boundary and
  dynamics adapter facts for DDPM/DDIM/probability flow style samplers.
- The [VAE theory API](https://github.com/nktkt/gondlin/blob/main/NN/MLTheory/Generative/Latent/VAE.lean) proves ELBO/KL style structural
  facts such as nonnegativity of the diagonal Gaussian KL to a standard normal.
- The [VQ-VAE theory API](https://github.com/nktkt/gondlin/blob/main/NN/MLTheory/Generative/Latent/VQVAE.lean) proves nearest code and
  quantization loss facts.
- The [GAN theory API](https://github.com/nktkt/gondlin/blob/main/NN/MLTheory/Generative/Latent/GAN.lean) records generator/discriminator
  loss decompositions.

Executable examples:

- [NN.Examples.Models.Generative.Diffusion API](https://github.com/nktkt/gondlin/blob/main/NN/Examples/Models/Generative/Diffusion.lean)
- [autoencoder example API](https://github.com/nktkt/gondlin/blob/main/NN/Examples/Models/Generative/Autoencoder.lean)
- [VAE example API](https://github.com/nktkt/gondlin/blob/main/NN/Examples/Models/Generative/Vae.lean)
- [VQ-VAE example API](https://github.com/nktkt/gondlin/blob/main/NN/Examples/Models/Generative/VqVae.lean)
- [GAN example API](https://github.com/nktkt/gondlin/blob/main/NN/Examples/Models/Generative/Gan.lean)

The precise wording is:

> Gondlin can run small generative demos and prove selected objective/sampler facts. That is not a
...
README prose. The
[differential privacy core](https://github.com/nktkt/gondlin/blob/main/NN/MLTheory/LearningTheory/DifferentialPrivacy/Core.lean) defines
...
[stability core](https://github.com/nktkt/gondlin/blob/main/NN/MLTheory/LearningTheory/Stability/Core.lean) defines supervised learning
stability over typed datasets. The
[ridge regression theorem API](https://github.com/nktkt/gondlin/tree/main/NN/MLTheory/LearningTheory/Stability/RidgeRegression1D/)
gives a worked stability theorem with real and `IEEE32Exec` versions. The
[robustness spec API](https://github.com/nktkt/gondlin/blob/main/NN/MLTheory/LearningTheory/Robustness/Spec.lean) names perturbation and
robustness predicates, while the
[robustness runtime API](https://github.com/nktkt/gondlin/blob/main/NN/MLTheory/LearningTheory/Robustness/Runtime.lean) gives
...
BugZoo is one of the most teachable parts of Gondlin because it connects formal methods to bugs
ML engineers already recognize.

The [BugZoo API](https://github.com/nktkt/gondlin/tree/main/NN/Examples/BugZoo/) and
[BugZoo overview](https://github.com/nktkt/gondlin/blob/main/NN/Examples/BugZoo/README.md) map each card to real bug families.
...
- Gondlin turns the intended behavior into a small spec or theorem so the failure mode has a
...
The [widgets API](https://github.com/nktkt/gondlin/tree/main/NN/Widgets/) gives human-readable views of proof and runtime artifacts:
...
- *BugZoo Catalog* gives the real bug cards and their checked Gondlin boundaries.
