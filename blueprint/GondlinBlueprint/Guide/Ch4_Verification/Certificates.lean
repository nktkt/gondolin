...
Gondlin does not vendor the Two-Stage / α,β-CROWN repository. The core Lean build does not
...
certificate* for each terminal subdomain. Gondlin can parse that JSON and validate several
...
file to Gondlin's checker.

# How To Check In Lean

Use the unified `verify` CLI tool `abcrown-leaf` to check a JSON certificate against Gondlin's
semantics.
...
End-to-end Python plus Lean post-check: *Two-Stage Workflows*. What Gondlin proves vs imports:
*Verification*.
