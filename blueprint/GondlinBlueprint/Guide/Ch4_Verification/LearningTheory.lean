...
Learning theory belongs in Gondlin because many ML claims become mathematical before they become
...
This is the same discipline used elsewhere in Gondlin. Specifications say what the claim means,
...
[NN.MLTheory.LearningTheory.DifferentialPrivacy.Core API](https://github.com/nktkt/gondlin/blob/main/NN/MLTheory/LearningTheory/DifferentialPrivacy/Core.lean).
...
input again. Gondlin states that as a Lean theorem rather than relying on a comment.
...
- [Robustness.Spec](https://github.com/nktkt/gondlin/blob/main/NN/MLTheory/LearningTheory/Robustness/Spec.lean) defines the mathematical
  predicates.
- [Robustness.Runtime](https://github.com/nktkt/gondlin/blob/main/NN/MLTheory/LearningTheory/Robustness/Runtime.lean) defines executable
  `Float` diagnostics.

The spec side works over Gondlin tensors without committing to one runtime scalar. It defines:
...
the model. Gondlin names it as an empirical runtime quantity unless a separate proof connects it
to a certified bound.

# Algorithmic Stability

The [algorithmic stability API](https://github.com/nktkt/gondlin/blob/main/NN/MLTheory/LearningTheory/Stability/Core.lean) uses one
...
In mainstream ML code, "replace one example and retrain" is usually an experiment. In Gondlin, it
...
[NN.MLTheory.LearningTheory.Stability.Dynamics API](https://github.com/nktkt/gondlin/blob/main/NN/MLTheory/LearningTheory/Stability/Dynamics.lean).
...
[Dynamics.Spec](https://github.com/nktkt/gondlin/blob/main/NN/MLTheory/LearningTheory/Stability/Dynamics/Spec.lean) names predicates
...
[Dynamics.Runtime](https://github.com/nktkt/gondlin/blob/main/NN/MLTheory/LearningTheory/Stability/Dynamics/Runtime.lean) provides
...
[NN.MLTheory.LearningTheory.Stability.RidgeRegression1D.Real API](https://github.com/nktkt/gondlin/blob/main/NN/MLTheory/LearningTheory/Stability/RidgeRegression1D/Real.lean).
...
[RidgeRegression1D/IEEE32Exec](https://github.com/nktkt/gondlin/blob/main/NN/MLTheory/LearningTheory/Stability/RidgeRegression1D/IEEE32Exec.lean).
The IEEE32Exec side implements the algorithm with Gondlin's executable binary32 model and states
the bridge to a proof level round after each primitive semantics under explicit finiteness
conditions.

That is the same numerics philosophy used elsewhere in Gondlin. A theorem over the reals is not
...
- If a script says an optimizer is differentially private because it used a DP library, the
  Gondlin guidepost is to define a mechanism and prove the DP event inequality or import a
  checked theorem.
...
This does not make Gondlin automatically prove every learning theory theorem. It makes the boundary
...
1. Read the [differential privacy core API](https://github.com/nktkt/gondlin/blob/main/NN/MLTheory/LearningTheory/DifferentialPrivacy/Core.lean)
   for the smallest complete example of a reusable measure theoretic definition plus closure
   lemmas.
2. Read the [robustness spec API](https://github.com/nktkt/gondlin/blob/main/NN/MLTheory/LearningTheory/Robustness/Spec.lean) beside
   the [robustness runtime API](https://github.com/nktkt/gondlin/blob/main/NN/MLTheory/LearningTheory/Robustness/Runtime.lean) to see the
   spec/runtime split.
3. Read the [stability core API](https://github.com/nktkt/gondlin/blob/main/NN/MLTheory/LearningTheory/Stability/Core.lean) for datasets,
   replace one perturbations, learning maps, and loss/error definitions.
4. Read the [ridge regression real theorem API](https://github.com/nktkt/gondlin/blob/main/NN/MLTheory/LearningTheory/Stability/RidgeRegression1D/Real.lean)
   for a complete theorem development with explicit constants.
5. Read the [ridge regression IEEE32Exec API](https://github.com/nktkt/gondlin/blob/main/NN/MLTheory/LearningTheory/Stability/RidgeRegression1D/IEEE32Exec.lean)
...
attached runtime checks and proof obligations to them. That is how Gondlin lets ML engineering
