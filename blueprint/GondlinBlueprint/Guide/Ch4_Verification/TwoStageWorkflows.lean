...
Gondlin does not vendor the Two-Stage / α,β-CROWN repository. The core Gondlin library and
`lake build` do not require that Python stack. If you want to run the external producer workflow,
clone the Two-Stage repository separately. Gondlin no longer carries that producer as a submodule.
...
- Python training, Julia simulation, or α,β-CROWN produces a checkpoint, JSON artifact, or bound
  report.
- Gondlin parses the artifact into typed Lean data.
- A Gondlin checker replays the supported computation, checks structure, or records an explicit
  producer hypothesis.
...
Below is how Gondlin connects Python training and α,β-CROWN runs to Lean replay and
checking. For flags and solver options on the Python side, follow the Two-Stage repository; those
details change independently of Gondlin.

To run a verifier tool, use `lake exe verify -- list` and the tool names in
[NN.Verification.CLI API](https://github.com/nktkt/gondlin/blob/main/NN/Verification/CLI.lean), then read the sections here for
what Lean actually checks.

# What Gondlin Provides

Gondlin supports the Python-first verification pattern: an external producer writes an artifact,
...
Gondlin does not try to own that environment. Local α,β-CROWN runs follow those repositories'
instructions; artifact export is described in *Certificates*.

# What Is Actually Checked

The Lean pipelines re-evaluate models against Gondlin's semantics, including the chosen scalar
domain such as `IEEE32Exec`, and run Gondlin's bound propagation and checkers on the resulting
graphs. The `abcrown-leaf` JSON artifacts are structural leaf artifacts today: Gondlin can parse
...
For the Lean declarations, open the [verification CLI](https://github.com/nktkt/gondlin/blob/main/NN/Verification/CLI.lean) and the
[α,β-CROWN leaf checker](https://github.com/nktkt/gondlin/blob/main/NN/Verification/Cert/AbCrownLeafCert.lean). The external producer
...
- Gondlin paper (Two-Stage case study context): https://arxiv.org/abs/2602.22631
