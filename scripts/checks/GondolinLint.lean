...
Gondolin linter driver.

This is wired into Lake as `lintDriver := "gondolin_lint"`, so you can run:

* `lake lint` locally
* (optionally) the same command in CI

Unlike mathlib’s `#lint`-heavy pipeline, Gondolin’s “lint” is primarily **repo policy**:
forbidden constructs (unverified proof stubs, unexpected axioms, native decision procedures, etc.),
whitespace hygiene, and consistent file headers. This keeps the signal high while the library is
...
This helper is used by the `gondolin_lint` executable entrypoint, so it must not be `private`
