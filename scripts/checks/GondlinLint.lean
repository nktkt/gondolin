...
Gondlin linter driver.

This is wired into Lake as `lintDriver := "gondlin_lint"`, so you can run:

* `lake lint` locally
* (optionally) the same command in CI

Unlike mathlib’s `#lint`-heavy pipeline, Gondlin’s “lint” is primarily **repo policy**:
...
This helper is used by the `gondlin_lint` executable entrypoint, so it must not be `private`
